...
public class PltPayDAO {
  ...
  public static List<PayTypeView> listViewBySupplier(Integer supplier) {
    return Query.SELECT(PltPay.class).WHERE(T.SUPPLIER, "=?", supplier).queryList().stream()
        .map(
            bean ->
                new PayTypeView() {
                  {
  ...
  public static class pageSelect extends IduOther<IduOther, PltPay> {
    ...
    public List getPaySettingBySupplierId(long id, int version)
      ...
      List<Map> resultList =
          sql.castListMap(
              BeanBase.list(sql.buildSql(), sql.getParms(parmList)),
              PltConfigDAO.supplierLanguage(Math.toIntExact(id)));
      if (version == 2) {
        ...
        for (Map o : resultList) {
          ...
          JSONObject jsonObject =
              new JSONObject(String.valueOf(o.get(PltPay.T.PAYSETTING.getFld().getCodeSqlField())));
  ...
  public static class Ins extends IduIns<Ins, PltPay> {
    ...
    public void valid() {
      ...
      //	     vn.validIntegerPositive(PltPay.T.MIN_COST, PltPay.T.MAX_COST, PltPay.T.EXTRA_COSTS,
      // PltPay.T.POUNDAGE);
      super.valid();
    }
  }
  ...
  public static class Upd extends IduUpd<Upd, PltPay> {
    ...
    public void before() {
      ...
      PropertyUtils.copyPropertiesWithout(
          bean, getB(), PltPay.T.PKEY, PltPay.T.ROW_VERSION, PltPay.T.SUPPLIER
          //                    T.PAYSETTING,
          //                    T.ENABLED
          );
      setB(bean);
  ...
  public static void init(UsrSupplier supplier) {
    ...
    // 支付设置
    // bean.setPaysetting(paySetting4TT(supplier.getSettlementBank(), "", supplier.get, bankAddress,
  ...
  private static final JSONObject paySetting4TT(
  ...
  public static List<PayTypeView> getPayMethodBySupplier(Integer supplier) {
    return Query.SELECT(T.PKEY, T.MODE).FROM(PltPay.class).WHERE(T.SUPPLIER, "=?", supplier)
