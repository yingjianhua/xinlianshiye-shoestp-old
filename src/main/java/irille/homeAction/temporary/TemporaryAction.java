...
public class TemporaryAction extends HomeAction<O2O_Product> {
  ...
  public void randomO2oList() throws IOException {
    ...
    if (getPurchase() != null) {
      childrenQuery.SELECT(new IEnumFld[]{T.PKEY}).FROM(UsrFavorites.class)
          .WHERE(T.PURCHASE, "=?", new Serializable[]{getPurchase().getPkey()})
          .WHERE(T.PRODUCT, "=", irille.shop.pdt.PdtProduct.T.PKEY);
    }

    sql.SELECT(new IEnumFld[]{irille.shop.pdt.PdtProduct.T.PKEY, irille.shop.pdt.PdtProduct.T.NAME,
        irille.shop.pdt.PdtProduct.T.PICTURE, irille.Entity.O2O.O2O_Product.T.PRICE,
        irille.Entity.O2O.O2O_Product.T.MIN_OQ});
    if (getPurchase() != null) {
      sql.SELECT(childrenQuery, "ismyfavorite");
    }

    sql.FROM(O2O_Product.class).LEFT_JOIN(PdtProduct.class, irille.shop.pdt.PdtProduct.T.PKEY,
        irille.Entity.O2O.O2O_Product.T.PRODUCT_ID)
        .LEFT_JOIN(O2O_Activity.class, irille.Entity.O2O.O2O_Activity.T.PKEY,
        ...
        .WHERE(irille.Entity.O2O.O2O_Product.T.VERIFY_STATUS, "=?",
            new Serializable[]{O2O_ProductStatus.PASS.getLine().getKey()}).WHERE(
        "(" + O2O_Product.class.getSimpleName() + "." + irille.Entity.O2O.O2O_Product.T.STATUS
            .getFld().getCodeSqlField() + "=? OR " + O2O_Product.class.getSimpleName() + "."
    ...
    for (int i = 0; o2os.size() != 0; ++i) {
      ...
      Integer pdtPkey = (Integer) GetValue
          .get(pdt, irille.shop.pdt.PdtProduct.T.PKEY, Integer.class, -1);
      if (!map.containsKey(pdtPkey)) {
        ...
        item.setImage(GetValue.getFirstImage(
            (String) GetValue.get(pdt, irille.shop.pdt.PdtProduct.T.PICTURE, String.class, null)));
        item.setId((long) pdtPkey);
        item.setMin_order(
            (Integer) GetValue.get(pdt, irille.Entity.O2O.O2O_Product.T.MIN_OQ, Integer.class, -1));
        item.setPrice((BigDecimal) GetValue
            .get(pdt, irille.Entity.O2O.O2O_Product.T.PRICE, BigDecimal.class, BigDecimal.ZERO));
        String name = (String) GetValue
            .get(pdt, irille.shop.pdt.PdtProduct.T.NAME, String.class, "");
        item.setTitle(name);
        item.setRewrite(
  ...
  public void generalList() throws IOException {
    ...
    sql.SELECT(new IEnumFld[]{irille.shop.pdt.PdtProduct.T.PKEY, irille.shop.pdt.PdtProduct.T.NAME,
        irille.shop.pdt.PdtProduct.T.CUR_PRICE, irille.shop.pdt.PdtProduct.T.PICTURE,
        irille.shop.pdt.PdtProduct.T.MIN_OQ}).FROM(PdtProduct.class)
        .WHERE(irille.shop.pdt.PdtProduct.T.PRODUCT_TYPE, " =? ",
            new Serializable[]{OProductType.GENERAL.getLine().getKey()});
    if (this.cat != null) {
      String pkeys = (String) this.productDao.getCatsNodeByCatId(this.getCat()).stream()
          .map((id) -> {
    ...
    for (int i = 0; pdts.size() != 0; ++i) {
      ...
      if (!map.containsKey(pdtPkey)) {
        ...
        item.setImage(GetValue.getFirstImage((String) GetValue
            .get(pdt, irille.shop.pdt.PdtProduct.T.PICTURE, String.class,  null)));
        item.setId((long) pdtPkey);
        item.setMin_order(
            (Integer) GetValue.get(pdt, irille.shop.pdt.PdtProduct.T.MIN_OQ, Integer.class, -1));
        item.setPrice((BigDecimal) GetValue
            .get(pdt, irille.shop.pdt.PdtProduct.T.CUR_PRICE, BigDecimal.class, BigDecimal.ZERO));
        String name = (String) GetValue
            .get(pdt, irille.shop.pdt.PdtProduct.T.NAME, String.class, "");
        item.setTitle(name);
        item.setRewrite(
