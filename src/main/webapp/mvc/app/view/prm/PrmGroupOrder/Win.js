Ext.define('mvc.view.prm.PrmGroupOrder.Win',{
...
initComponent : function(){
		...
		anchor : '100%',
		plain : true,
		xtype : Ext.create('mvc.view.prm.PrmGroupOrder.Form',{	insFlag : this.insFlag})
	},{
		xtype : Ext.create('mvc.view.prm.PrmGroupOrder.ListForm',{height : 300,border : false })
	}];
...
setActiveRecord : function(record){
		...
		if (record || this.form.activeRecord) {
			...
			this.lineTable.store.filter([{'id':'filter','property':'orderid','value':record.get('bean.pkey')}]);
		} else {
