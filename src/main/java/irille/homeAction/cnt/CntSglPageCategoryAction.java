...
public class CntSglPageCategoryAction extends HomeAction<CntSglPageCategory> {
  ...
  public void listAll() throws Exception {
    List<CntSglPageCategoryView> pcvs =
        (List<CntSglPageCategoryView>)
            CacheUtils.cache.get(
                "CntSglPageCategoryAction_listAll"
                    + ServletActionContext.getRequest().getSession().getId(),
                o -> {
                  List<CntSglPageCategory> cspc =
                      BeanBase.list(CntSglPageCategory.class, null, false);
