...
public class UsrProductCategoryDAO {
  ...
  public static void updateEnabled(
      final Integer pkey, final boolean enabeld, final Integer supplier) {
    if (Query.SELECT(UsrProductCategory.class)
            .WHERE(T.PKEY, "=?", pkey)
    ...
    Query.UPDATE(UsrProductCategory.class)
        .SET(T.ENABLED, enabeld ? OEnabled.TRUE : OEnabled.FALSE)
        .WHERE(
            T.PKEY,
            "in ("
                + allCategory.stream().map(p -> String.valueOf(p)).collect(Collectors.joining(","))
                + ")")
  ...
  public static class UpperAndLowerFrame extends IduOther<UpperAndLowerFrame, UsrProductCategory> {
    public void valid() {
      if (getB().gtCategoryUp() != null
          && Integer.valueOf(getB().gtCategoryUp().getEnabled())
              .equals(Integer.valueOf(Sys.OEnabled.FALSE.getLine().getKey()))) {
    ...
    List<PdtProduct> pdtList =
        BeanBase.list(
            ...
            PdtProduct.T.SUPPLIER.getFld().getCodeSqlField()
                ...
                + " <> "
                + Pdt.OState.DELETE.getLine().getKey(),
            false,
            SellerAction.getSupplier().getPkey());
  ...
  public static class Update extends IduOther<Update, UsrProductCategory> {
    public void before() {
      ...
      PropertyUtils.copyProperties(
          ...
          UsrProductCategory.T.SEO_DESCRIPTION_EN,
          UsrProductCategory.T.SEO_KEYWORD_EN,
          UsrProductCategory.T.SEO_TITLE_EN);
      setB(cat);
  ...
  //		public void before() {
  //			catList =
  // BeanBase.list(UsrProductCategory.class,UsrProductCategory.T.PKEY.getFld().getCodeSqlField() + "
  // in(" + pkeys + ")",false);
  //		}
  //
  //		public void valid() {
  //			for(UsrProductCategory cat : catList){
  //				List<UsrProductCategory> childrenCat =
  // BeanBase.list(UsrProductCategory.class,UsrProductCategory.T.CATEGORY_UP.getFld().getCodeSqlField() + " = ? ",false,cat.getPkey());
  ...
  public static class Sellect extends IduOther<Sellect, UsrProductCategory> {
    ...
    /** 查询某商家下所有分类 */
    public static List<UsrProductCategory> getAllCatBySup(
        FldLanguage.Language lang, Serializable supplier) {
      SQL sql =
          new I18NSQL(lang) {
            {
              SELECT(UsrProductCategory.class)
    ...
    /** 查询某商家下所有启用的一级分类 */
    public static List<UsrProductCategory> getTopCat(
        FldLanguage.Language lang, Serializable supplier) {
      SQL sql =
          new I18NSQL(lang) {
            {
  ...
  /** 查询所有分类 */
  public static Page getUsrProductCategory(
      Integer start,
      Integer limit,
      String name,
      Integer category,
      ...
      Integer enabled,
      String seoKeyword) {
    List<UsrProductCategory> listCat =
        irille.pub.bean.Query.SELECT(UsrProductCategory.class).queryList();
    ...
    List<UsrProductCategoryView> list =
        Query.sql(sql.LIMIT(start, limit)).queryMaps().stream()
            .map(
                o -> {
                  ...
                  view.setName(
                      String.valueOf(o.get(UsrProductCategory.T.NAME.getFld().getCodeSqlField())));
                  view.setName(String.valueOf(o.get("names")));
                  view.setSupplierId(
                      (Integer) o.get(UsrProductCategory.T.SUPPLIER.getFld().getCodeSqlField()));
                  view.setSupplier(
                      String.valueOf(o.get(UsrSupplier.T.NAME.getFld().getCodeSqlField())));
                  view.setCategoryId(
                      (Integer) o.get(UsrProductCategory.T.CATEGORY_UP.getFld().getCodeSqlField()));
                  view.setEnabled(
                      Integer.valueOf(
                          String.valueOf(o.get(PdtCat.T.ENABLED.getFld().getCodeSqlField()))));
                  view.setSeoKeyword(
                      String.valueOf(
                          o.get(UsrProductCategory.T.SEO_KEYWORD_EN.getFld().getCodeSqlField())));
                  view.setSeoTitle(
                      String.valueOf(
                          o.get(UsrProductCategory.T.SEO_TITLE_EN.getFld().getCodeSqlField())));
                  view.setSeoDescription(
                      String.valueOf(
                          o.get(
                              UsrProductCategory.T.SEO_DESCRIPTION_EN.getFld().getCodeSqlField())));
                  Integer id =
                      (Integer) o.get(UsrProductCategory.T.CATEGORY_UP.getFld().getCodeSqlField());
                  if (id != null) {
