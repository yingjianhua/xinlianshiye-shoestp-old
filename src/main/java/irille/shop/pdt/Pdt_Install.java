...
public class Pdt_Install extends InstallBase {
  ...
  private void initPdtColor() {
    if (BeanBase.list(PdtColor.class, null, false).size() > 0) return;
    PdtColorDAO.InsInit act = new PdtColorDAO.InsInit();
    List<PdtColor> list = new ArrayList<>();
    List<String> l =
        Arrays.asList(
            "红色", "蓝色", "緑色", "黃色", "橙色", "紫色", "黑色", "白色", "棕色", "酒紅色", "桃紅色", "深緑色", "深紫色", "鮮紅色",
            "灰色", "杏色", "深藍色", "米色", "驼色", "金色", "银色", "军绿色", "古铜色", "深粉", "深灰", "粉色", "深棕", "淺棕",
            "深灰", "卡其色", "黑粗紋", "黑細紋", "紅棕", "黃褐色", "槍色", "淺藍", "咖啡色", "紅棕色", "灰褐色", "淡紫色", "亮黑色",
            "玫瑰金", "紫紅色", "淡粉色", "红白色", "黄金", "蓝白色", "黑红", "裸色", "银粉", "黄金白银", "黑白", "黄蓝", "玫瑰金",
            "枚红色", "黑金", "藏蓝");
    for (String o : l) {
  ...
  private void initPdtSize() {
    if (BeanBase.list(PdtSize.class, null, false).size() > 0) return;
  ...
  private void initPdtCat() {
    if (BeanBase.list(PdtCat.class, null, false).size() > 0) return;
    ...
    category
        .get("Men Collections")
        .put("Mens boots", Arrays.asList("Safety Shoes", "Dress  boots"));
    category.get("Men Collections").put("Mens sandals", Arrays.asList());
    category
        .get("Womens Collections")
        .put(
            "Womens shoes",
            Arrays.asList(
                "Sports shoes",
    ...
    category
        ...
        .put(
            "Womens boots",
            Arrays.asList("Ankle Boots", "Mid calf boots", "Knee high boots", "Casual boots"));
    category
        .get("Womens Collections")
        .put(
            "Womens sandals",
            Arrays.asList("Flat sandals", "Wedge sandals", "Heel sandals", "Slippers"));
    category
        .get("Kids Collections")
        .put(
            "Boys Collections",
            Arrays.asList(
                "Boys boots",
  ...
  private void initPdtAttr() {
    if (BeanBase.list(PdtAttrCat.class, null, false).size() > 0) return;
    ...
    attrMap.put("风格类型", Arrays.asList("青春潮流", "商务", "休闲", "运动", "民族风", "韩版", "英伦", "欧美", "复古"));
    attrMap.put("适合季节", Arrays.asList("春秋", "夏季", "冬季", "四季", "秋冬季", "秋季"));
    attrMap.put(
        "内里材料",
        Arrays.asList(
            "半粒面", "檫色皮", "樹膏皮", "磨砂皮", "壓花皮", "漆皮", "粒面皮", "搖粒絨", "PU", "毛里", "仿羊羔毛", "纺织品",
            "MESH", "Fabric", "Fur", "布", "网布", "帆布", "短毛绒", "绒布", "绒毛", "佳積布", "頭層豬皮", "金絲絨",
            "二層豬皮", "超纖布", "天鵝絨", "超纖", "頭層豬皮", "漆皮", "鹿皮"));
    attrMap.put(
        "鞋底材料",
        Arrays.asList(
            "EVA", "EVA發泡膠", "MD", "PVC", "TPR(牛筋）", "TPU", "橡膠發泡", "煙膠", "PU", "橡胶", "组合底",
            "聚氨酯"));
    attrMap.put(
        "鞋面材料",
        Arrays.asList(
    ...
    attrMap.put(
        "鞋垫材料",
        Arrays.asList(
            "PU", "二層豬皮", "人造短毛絨", "人造長毛絨", "頭層牛皮", "頭層豬皮", "布", "純羊毛", "織布", "佳績布", "仿羊羔毛",
            "天鵝絨"));
    attrMap.put("高跟鞋", Arrays.asList("低跟（1--3cm）", "平跟（小於等於1cm）", "中跟（3--5cm）", "高跟（5cm及以上）"));
    attrMap.put("鞋头", Arrays.asList("圓頭", "方頭", "尖頭", "扁頭", "魚嘴"));
    attrMap.put("闭合方式", Arrays.asList("拉鏈", "繫帶", "魔術貼", "套腳", "搭扣", "鬆緊帶"));
    attrMap.put("制作工艺", Arrays.asList("固特異", "縫製鞋", "膠粘鞋", "硫化鞋", "注壓鞋", "冷粘"));
    attrMap.put("根底", Arrays.asList("平底", "內增高", "厚底", "坡跟", "高跟", "摇摇底"));
    attrMap.put("图案", Arrays.asList("純色", "格子", "拼色", "手繪"));
    attrMap.put(
        "流行元素",
        Arrays.asList(
            "奢華馬毛", "皮草裝飾", "素面", "編制", "馬銜扣", "圖騰", "鉚釘", "抽條", "飾釦", "流蘇", "鬆緊拼接", "透明跟", "電秀",
            "金屬跟"));
    attrMap.put("功能", Arrays.asList("矯正", "透氣", "輕質", "防水", "保暖", "耐磨", "減震", "增高"));
    ...
    attrMap.put(
        "生产周期", Arrays.asList("70天", "30天", "约50天", "两个月", "一个月", "45天", "7天", "15天", "25天"));
