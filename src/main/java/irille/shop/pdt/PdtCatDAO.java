...
public class PdtCatDAO {
  ...
  /** 获取产品类目 */
  public static Page listPdtCat(
      Integer start, Integer limit, Integer enabled, String name, Integer subjectionCat) {
                    ...
                    new PdtCatView() {
                      {
                        ...
                        setEnabled(
                            Integer.valueOf(
                                        String.valueOf(
                                            bean.get(T.ENABLED.getFld().getCodeSqlField())))
                                    == 1
                                ? true
                                : false);
                        ...
                        setCreateBy(
                            BeanBase.load(
                                    ...
                                    Integer.valueOf(
                                        String.valueOf(
                                            bean.get(T.CREATE_BY.getFld().getCodeSqlField()))))
                                .getLoginName());
                        setSeoDescription(
                            (String) bean.get(T.SEO_DESCRIPTION_EN.getFld().getCodeSqlField()));
                        setSeoKeyword(
                            (String) bean.get(T.SEO_KEYWORD_EN.getFld().getCodeSqlField()));
  ...
  public static List<CategoryView> listTopCatView(Language lang) throws JSONException {
    List<PdtCat> list =
        irille.pub.bean.Query.SELECT(PdtCat.class)
  ...
  public static class Query extends IduOther<Query, PdtCat> {
    public static List<PdtCat> listSub(Integer pkey) {
      return BeanBase.list(
          PdtCat.class,
          T.CATEGORY_UP.getFld().getCodeSqlField()
              + " = ? AND "
              + T.DELETED.getFld().getCodeSqlField()
              + " = ?",
          false,
          pkey,
  ...
  public static class Upd extends IduUpd<Upd, PdtCat> {
    ...
    public void before() {
      ...
      PropertyUtils.copyPropertiesWithout(
          dbBean, getB(), T.PKEY, T.CREATE_BY, T.CREATE_TIME, T.DELETED);
  ...
  // 修改
  public static void updPdtCat(
      String name,
      Integer enabled,
      Integer subjectionCat,
  ...
  public static List<PdtCat> listAll() {
    List<PdtCat> listCat =
        irille.pub.bean.Query.SELECT(PdtCat.class).WHERE(T.DELETED, " = ?", OYn.NO).queryList();
    return listCat;
  }
