...
(function($){
	...
	$(function(){
		...
		$container.find('div.menu_list .son>a').on('click tap', function(){
			var par = $(this).parent();
			if(par.hasClass('open')){
				par.removeClass('open');
				par.find('.menu_son').stop(true, false).slideUp();
			}else{
				par.addClass('open').find('.menu_son').stop(true, false).slideDown();
		...
		//选择语言
		$('footer .list_lang, .lang_cr .lang').on('click', function(e){
			$height=$(window).height();
