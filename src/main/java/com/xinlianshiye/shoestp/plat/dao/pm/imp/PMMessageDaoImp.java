...
public class PMMessageDaoImp implements IPMMessageDao{
	...
	public Page<MessageView> list(MessageView view,Integer start, Integer limit) {
		...
		List<MessageView> items = Query.sql(sql).queryList(PMMessage.class).stream().map(item->{
	...
	public Page<MessageView> list(Integer rec,ORCVRType type, Integer start, Integer limit) {
		...
		String sql = " SELECT " + 
					 ...
					 " FROM p_m_read rea " + 
					 " WHERE rea.reader = " + rec + 
					 " GROUP BY rea.read_message " + 
					 " ) AS r " + 
					 " ON r.m = message.pkey " + 
					 " WHERE " + 
					 " ( message.rcvr = " + rec + " OR ( message.rcvr_type = " + type.getLine().getKey() + " AND message.rcvr = - 1 ) ) " + 
		...
		List<MessageView> items = Query.sql(sql).queryMaps().stream().map(item->{
			MessageView model = new MessageView();
