...
public class PdtSizeDAO {
	 ...
	 * @date 2019/3/1 15:30
	 */
	public static Page newListSize(String name, String productCategory, Integer start, Integer limit) {
		if (null == start) {
			start = 0;
		}
		if (null == limit) {
	...
	public static class UpdSize extends IduUpd<PdtSizeDAO.Upd, PdtSize> {
		...
		public void before() {
			...
			PropertyUtils.copyPropertiesWithout(dbBean, getB(), PdtSize.T.PKEY, PdtSize.T.SUPPLIER, PdtSize.T.CREATE_BY,
					PdtSize.T.CREATE_TIME, PdtSize.T.DELETED, PdtSize.T.TYPEVER, PdtSize.T.ROW_VERSION);
			setB(dbBean);
		}
	}
	...
	public static List<PdtSize> listSummary(Language lang, Integer type) {
		...
		SQL sql = new I18NSQL(lang) {
			{
				...
				FROM(PdtSize.class).WHERE(T.DELETED, "=?", OYn.NO).WHERE(T.SUPPLIER, " is null ").WHERE(T.TYPEVER,
						" =? ", Pdt.OVer.ELSE.getLine().getKey());
			}
		};
		...
		if (type == 1) {
			SQL supSql = new I18NSQL(lang) {
				{
					SELECT(T.PKEY, T.NAME, T.CREATE_BY, T.ROW_VERSION);
					FROM(PdtSize.class).WHERE(T.DELETED, "=?", OYn.NO)
	...
	public static List<PdtSize> newListSummary(UsrSupplier supplier, Language lang, Integer type, Integer cat) {

		/*
		 * SQL sql = new I18NSQL(lang) {{ SELECT(T.PKEY, T.NAME, T.CREATE_BY,
		 * T.ROW_VERSION); FROM(PdtSize.class).WHERE(T.DELETED, "=?",
		 * OYn.NO).WHERE(T.SUPPLIER, " is null "); }}; List<PdtSize> sysSize =
		 * Query.sql(sql).queryList(PdtSize.class); if (type == 1) { SQL supSql = new
		 * I18NSQL(lang) {{ SELECT(T.PKEY, T.NAME, T.CREATE_BY, T.ROW_VERSION);
		 * FROM(PdtSize.class).WHERE(T.DELETED, "=?", OYn.NO).WHERE(T.SUPPLIER, " =? ",
		 * SellerAction.getSupplier().getPkey()); }}; List<PdtSize> supSize =
		 * Query.sql(supSql).queryList(PdtSize.class); sysSize.addAll(supSize); } return
		 * sysSize;
		 */
		if (null == cat) {
		...
		SQL sql = new SQL() {
			{
				...
				FROM(PdtSize.class).WHERE(T.DELETED, "=" + OYn.NO.getLine().getKey()).WHERE(T.TYPEVER,
						" =" + Pdt.OVer.NEW_1.getLine().getKey());
				if (type.equals(1)) {
					WHERE("(" + PdtSize.class.getSimpleName() + "." + PdtSize.T.SUPPLIER.getFld().getCodeSqlField()
							+ " IS NULL OR " + PdtSize.class.getSimpleName() + "."
							+ PdtSize.T.SUPPLIER.getFld().getCodeSqlField() + " =" + supplier.getPkey() + " )");
					WHERE("(" + PdtSize.class.getSimpleName() + "."
							+ PdtSize.T.PRODUCT_CATEGORY.getFld().getCodeSqlField() + " in("
							+ String.join(",", pdtProductDao.getParent(cat)) + ") OR " + PdtSize.class.getSimpleName()
							+ "." + PdtSize.T.PRODUCT_CATEGORY.getFld().getCodeSqlField() + " IS NULL )")
		...
		if (null != sysSize && sysSize.size() > 0) {
			sysSize = sysSize.stream().filter(b -> {
				try {
	...
	public static class Upd extends IduUpd<Upd, PdtSize> {
		...
		public void before() {
			super.before();
			PdtSize dbBean = loadThisBeanAndLock();
			PropertyUtils.copyPropertiesWithout(dbBean, translateUtil.autoTranslateByManageLanguage(getB(), true),
					T.PKEY, T.SUPPLIER, T.CREATE_BY, T.CREATE_TIME, T.DELETED);
			setB(dbBean);
		}
	}
	...
	public static PdtSize insSize(Byte type, String name, Integer supplier, Language lag) {
		...
		List<PdtSize> attrs = BeanBase.list(PdtSize.class,
				PdtSize.T.NAME.getFld().getCodeSqlField() + "->'$.en' = ? AND "
						+ PdtSize.T.SUPPLIER.getFld().getCodeSqlField() + " =? AND "
						+ PdtSize.T.DELETED.getFld().getCodeSqlField() + " =? AND "
						+ PdtSize.T.TYPE.getFld().getCodeSqlField() + " =? ",
				false, engName, supplier, OYn.NO.getLine().getKey(), type);
	...
	public static void updSize(Integer supplier, Integer pkey, String name) {
		...
		List<PdtSize> attrs = BeanBase.list(PdtSize.class,
				...
				false, engName, supplier, OYn.NO.getLine().getKey());
		if (null != attrs && attrs.size() > 0) {
			throw new WebMessageException(ReturnCode.failure, "尺码【" + engName + "】已存在,请勿重复添加");
		}
		SQL sql = new SQL();
		...
		if (size.getSupplier() == null || !size.getSupplier().equals(supplier))
	...
	public static void plaInsSize(SysUser user, Byte type, String name, Integer pdtCate) {
		...
		if (pdtCate != null) {
			...
			if (cat != null)
				size.setProductCategory(pdtCate);
		}
		size.setDeleted(OYn.NO.getLine().getKey());
	...
	public static void checkNameInUse(PdtSize size, String name) {
		...
		sql.WHERE("JSON_EXTRACT(PdtSize.name,?) = ?", "$." + PltConfigDAO.manageLanguage().toString(), str);
	...
	public static void chooseSizePdt(PdtSize size, Integer type) {
		...
		sql1.WHERE("size_attr like (?) or size_attr like (?) or size_attr like (?) or size_attr = ?",
				"%," + size.getPkey(), "%," + size.getPkey() + ",%", size.getPkey() + ",%", size.getPkey());
