...
class OssUtil {
     ...
     * @author yingjianhua
     */
    public static <T extends BeanMain<?, ?>> String upload(Class<T> beanClass, String filename, File file) {
    	String fileKey = pathPattern.replaceAll("%m", moduleName(beanClass)).replaceAll("%b", beanName(beanClass))+filename;
...
//    	String url = cname==null?("http://"+bucketName+"."+endpoint+"/"+fileKey):("http://"+cname+"/"+fileKey);
    	return "/"+fileKey;
    }
    //取bean的模块名
