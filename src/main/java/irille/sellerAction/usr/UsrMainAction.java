...
public class UsrMainAction extends SellerAction<UsrMain> {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(irille.homeAction.usr.UsrMainAction.class);
      ...
      BufferedReader reader =
          new BufferedReader(
              new InputStreamReader(
                  irille.homeAction.usr.UsrMainAction.class.getResourceAsStream(
                      "/mailPage/" + fileName + ".html")));
  ...
  public void subValid() throws IOException {
    ...
    if (cache.getIfPresent(getCode()) != null && value.equalsIgnoreCase(getEmail())) {
      write();
    } else {
  ...
  public void updEmail() throws Exception {
    ...
    if (!ValidRegex.regMarch(Regular.REGULAR_EMAIL, getNewEmail())
        || !ValidRegex.regMarch(Regular.REGULAR_EMAIL, getSureEmail())) {
  ...
  public void getTimeZone() throws IOException, URISyntaxException {
    URL resource = UsrMainAction.class.getResource("/timeZone.properties");
    ...
    if (file.exists()) {
      ...
      for (Entry<Object, Object> entry : properties.entrySet()) {
        ...
        zt.setId(entry.getKey().toString());
        zt.setName(entry.getValue().toString());
        zones.add(zt);
  ...
  public void getTimeDifferent() throws Exception {
    ...
    int selectHour = c2.get(Calendar.HOUR_OF_DAY);
    int dif = selectHour - nowHour;
    JSONObject json = new JSONObject();
    SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy年MM月dd日," + dif + "小时 HH:mm:ss");
    json.put("ret", 1);
    json.put("different", simpleDateFormat.format(c2.getTime()));
    writerOrExport(json);
  }
}
