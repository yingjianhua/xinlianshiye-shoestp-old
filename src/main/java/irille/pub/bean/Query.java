...
public class Query {
  ...
  public static void main(String[] args) {
    try {
      ...
      String sql =
          "select b.name as cname,a.* from usr_consult a left join plt_country b on a.COUNTRY=b.PKEY where b.NAME='Zambia' AND TITLE like '%鞋%' order by CREATE_TIME desc LIMIT 0,2";
    ...
    // String sql2 = "select a.* from UsrConsult a left join UsrConsultRelation b on
    // a.pkey=b.CONSULT where b.SUPPLIER=?";
  }
  ...
  public static SqlQuery sql(SQL sql) {
    return new SqlQuery(
        sql.toString(), sql.PARAMS().toArray(new Serializable[sql.PARAMS().size()]));
  }
  ...
  public static <T extends BeanMain<?, ?>> T SELECT(
      Class<T> beanClass, Language lang, Integer pkey) {
    BeanQuery<?> q = new BeanQuery<>(lang);
    return q.SELECT(beanClass).FROM(beanClass).WHERE("pkey=?", pkey).query();
  }
}
