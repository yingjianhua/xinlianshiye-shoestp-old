...
public class UsrPurchaseAction extends HomeAction<UsrPurchase> implements IUsrPurchaseAction {
  ...
  @Inject private PdtproductPageselect pdtPageSelectt = new PdtproductPageselect();
  ...
  @Getter @Setter private IndexAdView4Mobile indexAdView4Mobile;
  ...
  public String execute() throws Exception {
    ...
    for (Map adId : adPageSelect.getHasAdCatInfos(page, CntAd.OAdLocation.PDT_CAT, osignage)) {
      ...
      long categoryId =
          Long.valueOf(String.valueOf(adId.get(CntAd.T.CATEGORY.getFld().getCodeSqlField())));
      ...
      if (!isMobile()) {
        ...
        adIndexCategory.setCategorys(
            pdtPageSelectt.getProductsCategorieNodesByCategory(categoryPage, categoryId));
      }
    ...
    indexAdView4PC.setMiddleAd(
        CntAdDAO.listViewBySignagePosition(osignage, OAdLocation.INDEX_BANNER_MIDDLE, 0, 3));
    indexAdView4PC.setProductCategoryAd(
        CntAdDAO.listViewBySignagePosition(osignage, OAdLocation.PDT_CAT, 0, 7));

    page.setLimit(5);
    purchaseIndexView.setHeadedSwiperAds(
        adPageSelect.getAdsInfo(page, CntAd.OAdLocation.INDEX_SWIPER, osignage));
    if (!isMobile()) {
      page.setLimit(1);
      purchaseIndexView.setHeadedRightTopAds(
          adPageSelect.getAdsInfo(page, CntAd.OAdLocation.INDEX_RIGHT_TOP, osignage));
      page.setLimit(2);
      purchaseIndexView.setHeadedBottomTopAds(
          adPageSelect.getAdsInfo(page, CntAd.OAdLocation.INDEX_RIGHT_BOTTOM, osignage));
    }
    page.setStart(0);
    ...
    indexAdView4Mobile.setProductCategoryAd(
        CntAdDAO.listViewBySignagePosition(osignage, OAdLocation.PDT_CAT, 0, 4));
    /**
    ...
    // 中间广告
    if (isMobile())
      purchaseIndexView.setMiddleAds(
          adPageSelect.getAdsInfo(page, CntAd.OAdLocation.INDEX_BANNER_MIDDLE, osignage));
  ...
  public void signIn() throws Exception {
    ...
    if (vcodeNum >= 3
        && (Str.isEmpty(verifyCode) || Str.isEmpty(_checkCode) || !verifyCode.equals(_checkCode))) {
    ...
    if (user.isPurchase()) {
      ...
      if (!Str.isEmpty(getJumpUrl())) json.put("msg", new JSONArray().put(getJumpUrl()));
      writeSuccess(json);
      return;
    }
  ...
  public String signOut() {
    setUser(null);
    if (isMobile()) setResult("/home/usr_UsrPurchase_sign", false);
    else {
  ...
  public String userIndex() {
    if (isMobile()) {
      ...
      num =
          BeanBase.list(
                  UsrCart.class,
                  UsrCart.T.PURCHASE.getFld().getCodeSqlField() + " = " + usrPurchase.getPkey(),
                  false)
              .size();
      orders =
          BeanBase.list(
                  OdrOrder.class,
                  OdrOrder.T.PURCHASE.getFld().getCodeSqlField() + " = " + usrPurchase.getPkey(),
                  false)
              .size();
      List<UsrPurchaseLine> li =
          BeanBase.list(
      ...
      String text =
          AppConfig.mail_template
              .replaceAll(
                  "\\{ForgotUrl\\}",
                  AppConfig.domain
                      + "/home/usr_UsrPurchase_forgetUI?randomNumber1="
                      + randomNumber1)
              .replaceAll(
                  "\\{Time\\}",
  ...
  public String usrSetting() {
    ...
    usrPurchase =
        BeanBase.load(
            UsrPurchase.class,
            getPurchase().getPkey() + " = " + HomeAction.getPurchase().getPkey());
  ...
  public void detailById() throws Exception {
    ...
    if (address != null) view.setDefaultAddress(AddressView.trans(address));
  ...
  public String center() {
    //    	List<Object[]> list = BeanBase.list("SELECT " +
    // UsrSupplier.T.PKEY.getFld().getCodeSqlField() + "," +
    // UsrSupplier.T.SHOW_NAME.getFld().getCodeSqlField() + "," +
    // UsrSupplier.T.COMPANY_PHOTO.getFld().getCodeSqlField() + " FROM " +
    // UsrSupplier.TB.getCodeSqlTb());
    //
    ...
    if (null == faceBookToken || "".equals(faceBookToken)) {
      ...
      String faceBook_checkUrl =
          "https://graph.facebook.com/debug_token?access_token="
              ...
              + "&input_token=";
      String checkResult =
          sendHttpsUtils.sendGet(faceBook_checkUrl + faceBookToken, "127.0.0.1", 1080);
