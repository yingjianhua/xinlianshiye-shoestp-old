...
public class RFQConsultServiceImpl implements RFQConsultService {
        ...
        query.queryMaps().stream()
            .map(
                map -> {
                  ...
                  quotation.setPkey(
                      GetValue.get(map, RFQConsultRelation.T.PKEY, Integer.class, null));
                  quotation.setTitle(
                      GetValue.get(map, RFQConsultRelation.T.TITLE, String.class, null));
                  quotation.setDescription(
                      GetValue.get(map, RFQConsultRelation.T.DESCRIPTION, String.class, null));
                  try {
                    String image =
                        GetValue.get(map, RFQConsultRelation.T.IMAGE, String.class, null);
                    if (image != null && !image.isEmpty()) {
                      quotation.setImages(
                          om.readValue(image, new TypeReference<List<RFQQuotationImageView>>() {}));
                    }
                  } catch (IOException e) {
                  ...
                  quotation.setCreateDate(
                      GetValue.get(map, RFQConsultRelation.T.CREATE_DATE, Date.class, null));
                  quotation.setSample(
                      BeanBase.byteToBoolean(
                          GetValue.get(map, RFQConsultRelation.T.SAMPLE, Byte.class, null)));
                  quotation.setIsNew(
                      BeanBase.byteToBoolean(
                          GetValue.get(map, RFQConsultRelation.T.IS_NEW, Byte.class, null)));
  ...
  public RFQQuotationView getQuotation(UsrPurchase purchase, Integer relationPkey) {
    BeanQuery<RFQConsultRelation> query =
    ...
    try {
      if (relation.getImage() != null && !relation.getImage().isEmpty()) {
        quotation.setImages(
            om.readValue(relation.getImage(), new TypeReference<List<RFQQuotationImageView>>() {}));
      }
    } catch (IOException e) {
    ...
    try {
      quotation.setThrowaways(
          om.readValue(
              relation.getThrowaway(), new TypeReference<List<RFQQuotationThrowawayView>>() {}));
    } catch (IOException e) {
                    ...
                    new RFQListView() {
                      {
                        ...
                        for (RFQConsultUnit value : RFQConsultUnit.values()) {
                          if (value.getLine().getKey()
                              == (Byte) o.get(RFQConsult.T.UNIT.getFld().getCodeSqlField())) {
  ...
  public RFQDetailsView getRFQDetails(Integer pkey) {
    ...
    view.setUserCountryImage(
        (String) userMap.get(PltCountry.T.NATIONAL_FLAG.getFld().getCodeSqlField()));
  ...
  public void deleteQuotation(UsrPurchase purchase, Integer relationPkey) {
    ...
    RFQConsultRelation relation = query.query();
    if (relation == null || relation.gtIsDeletedPurchase()) return;
  ...
  public RFQConsultView getDetail(UsrPurchase purchase, Integer consultPkey, Language language) {
    ...
    view.setImages(
        consult.getImage() == null
            ? new ArrayList<>()
            : Arrays.asList(consult.getImage().split(",")));
    return view;
  }
  ...
  public void addProductRequest(
    ...
    if (productRequest == null || productRequest.isEmpty()) {
      ...
      try {
        productRequestSet =
            om.readValue(
                consult.getProductRequest(), new TypeReference<Set<RFQConsultProductView>>() {});
      } catch (IOException e) {
        productRequestSet = new HashSet<>();
      }
    }
