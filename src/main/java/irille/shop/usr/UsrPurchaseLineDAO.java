...
public class UsrPurchaseLineDAO  {
	...
	public static List<UsrPurchaseLine> listByPurchaseAddrsstype(Integer purchase, OAddress stype) {
		return Query.SELECT(UsrPurchaseLine.class).WHERE(T.PURCHASE, "=?", purchase).WHERE(T.ADDRSSTYPE, "=?", stype.getLine().getKey()).queryList();
	}
	public static UsrPurchaseLine findDefaultByPurchase(Integer purchase) {
		return Query.SELECT(UsrPurchaseLine.class).WHERE(T.PURCHASE, "=?", purchase).WHERE(T.ADDRSSTYPE, "=?", OAddress.COMMON.getLine().getKey()).WHERE(T.ISDEFAULT, "=?", BeanBase.booleanToByte(true)).query();
	}
	
	public static class Ins extends IduIns<Ins, UsrPurchaseLine> {
		public void before() {
			String sql = "UPDATE " +  UsrPurchaseLine.TB.getCodeSqlTb() + " SET " + T.ISDEFAULT.getFld().getCodeSqlField() + "=? WHERE " + T.ISDEFAULT.getFld().getCodeSqlField() + "=? AND " + T.PURCHASE.getFld().getCodeSqlField() + "=?";
			UsrPurchaseLine.executeUpdate(sql,Sys.OYn.NO.getLine().getKey(), Sys.OYn.YES.getLine().getKey(),HomeAction.getPurchase().getPkey());
	...
	public  static class upd extends IduUpd<upd, UsrPurchaseLine> {
		public void before() {
            ...
            PropertyUtils.copyPropertiesWithout(dbbean, getB(),T.ISDEFAULT,T.PKEY,T.ADDRSSTYPE,T.ROW_VERSION,T.PURCHASE);
            setB(dbbean);
            super.before();
		}
	}
	...
	public  static class updisdefault extends IduUpd<upd, UsrPurchaseLine> {
		public void before() {
			String sql = "UPDATE " +  UsrPurchaseLine.TB.getCodeSqlTb() + " SET " + T.ISDEFAULT.getFld().getCodeSqlField() + "=? WHERE " + T.ISDEFAULT.getFld().getCodeSqlField() + "=? AND " + T.PURCHASE.getFld().getCodeSqlField() + "=? AND " + T.ADDRSSTYPE.getFld().getCodeSqlField() + " = ? ";
			UsrPurchaseLine.executeUpdate(sql,Sys.OYn.NO.getLine().getKey(), Sys.OYn.YES.getLine().getKey(),HomeAction.getPurchase().getPkey(),Usr.OAddress.COMMON.getLine().getKey());
			getB().stIsdefault(true);
			UsrPurchaseLine dbbean = loadThisBeanAndLock();
	...
	public  static class updAdd extends IduUpd<upd, UsrPurchaseLine> {
		public void before() {
			String sql = UsrPurchaseLine.T.PURCHASE.getFld().getCodeSqlField()+" in ("+HomeAction.getPurchase().getPkey()+")";
