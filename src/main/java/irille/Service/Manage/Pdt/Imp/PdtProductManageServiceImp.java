...
public class PdtProductManageServiceImp implements IPdtProductManageService, Job {
	...
	public Page getProductList(String name, String number, Integer supplierId, int cat, int start, int limit,
			Integer search, String data) {
		return pdtProductDao.getProductListManage(name, number, supplierId, cat, start, limit, search, data);
	}
	...
	public Integer saveProduct(String data, Integer supId) throws IOException, ExecutionException {
		...
		// 校验商品货号
		Integer verify = pdtProductDao.verifyCode(pdtProductSaveView.getNumber_right(), pdtProductSaveView.getId(),
				supId);
		if (verify == 0) {
			return -12;
		}
		...
		if (pdtProductSaveView.getTieredPricing() != null && !pdtProductSaveView.getTieredPricing().isEmpty()) {
			for (int i = 0; i < pdtProductSaveView.getTieredPricing().size(); i++) {
				...
				if (item.getCount() < 0 || item.getPrice().compareTo(BigDecimal.valueOf(999999999)) > 0
						|| item.getPrice().compareTo(BigDecimal.valueOf(0.01)) < 0) {
					return -14;
				}
				if (i != 0) {
					if (item.getCount() <= pdtProductSaveView.getTieredPricing().get(i - 1).getCount() || item
							.getPrice().compareTo(pdtProductSaveView.getTieredPricing().get(i - 1).getPrice()) > 0) {
						return -14;
					}
				}
			...
			for (int i = 0; i < pdtProductSaveView.getDesModule().length; i++) {
				...
				if (i == 0) {
					...
					if (pdtProduct.getDescribeModule1() == null)
						...
						pdtProduct.setDescribeModule2(des);
				} else if (i == 2) {
					if (pdtProduct.getDescribeModule1() == null)
						pdtProduct.setDescribeModule1(des);
					else if (pdtProduct.getDescribeModule2() == null)
						pdtProduct.setDescribeModule2(des);
					else
						pdtProduct.setDescribeModule3(des);
				} else {
					break;
				}
			}
		...
		if (pdtProductSaveView.getSelectcountry() != null || pdtProductSaveView.getSelectcountry().isEmpty()) {
		...
		pdtProduct.setTargetedMarket(country.stream().map(String::valueOf).collect(Collectors.joining(",")));
		...
		String attrs = pdtProductSaveView.getAttr().stream()
				.filter(o -> o != null && Integer.parseInt(o.toString()) > 0).map(String::valueOf)
				.collect(Collectors.joining(","));
		System.out.println(attrs);
		List<PdtAttrLine> lines = new ArrayList<>();
		if (attrs != null && !"".equals(attrs)) {
			lines = BeanBase.list(PdtAttrLine.class,
					PdtAttrLine.T.PKEY.getFld().getCodeSqlField() + " IN(" + attrs + ") ", false);
		}
		 ...
		 */
		pdtProduct.setColorAttr(colorSet.stream().map(String::valueOf).collect(Collectors.joining(",")));
		/*
	...
	@Override
	public Page getWarehouse(Integer supplierm, String productName, String productNum, Integer status, Integer start,
		...
		SQL sql = new SQL() {
			{
				SELECT(PdtProduct.class).FROM(PdtProduct.class).WHERE(PdtProduct.T.SUPPLIER, "=?", supplierm);
				if (type != null) {
					WHERE(PdtProduct.T.STATE, "=?", Pdt.OState.MERCHANTDEL.getLine().getKey());
				} else {
					WHERE(PdtProduct.class.getSimpleName() + "." + PdtProduct.T.STATE.getFld().getCodeSqlField()
		...
		List<WarehouseView> list = Query.sql(sql.LIMIT(start, limit)).queryMaps().stream()
				.map(o -> new WarehouseView() {
					{
						...
						if (o.get(PdtProduct.T.CATEGORY.getFld().getCodeSqlField()) != null) {
							setProductCate(BeanBase
									.load(PdtCat.class,
											(Integer) o.get(PdtProduct.T.CATEGORY.getFld().getCodeSqlField()))
									.getName());
						}
						if (o.get(PdtProduct.T.CATEGORY_DIY.getFld().getCodeSqlField()) != null) {
							setStoreCate(BeanBase
									.load(UsrProductCategory.class,
											(Integer) o.get(PdtProduct.T.CATEGORY_DIY.getFld().getCodeSqlField()))
									.getName());
						}
						...
						setStatus(Integer
								.valueOf(String.valueOf(o.get(PdtProduct.T.IS_VERIFY.getFld().getCodeSqlField()))));
					}
	...
	public void addSvs(PdtProduct pdtProduct, Integer supId) {
		if (pdtProduct.getState() == Pdt.OState.ON.getLine().getKey()
				...
				&& !pdtProduct.gtSoldInTime() && pdtProduct.getFirstPutaway() == OYn.NO.getLine().getKey()) {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		if (strList != null && !strList.isEmpty()) {
			Query.UPDATE(PdtProduct.class).SET(PdtProduct.T.FIRST_PUTAWAY, OYn.YES.getLine().getKey())
					.WHERE(PdtProduct.T.PKEY, SQL.getInSql(strList.size()),
