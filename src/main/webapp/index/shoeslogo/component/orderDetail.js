	...
	template:'<div :id="id" :key="id"><ul class="order_info">'+
			  	'<li v-for="(key,index) in orders">'+
			  		'<p class="order_info_detail" :class="[index%2==0?\'add\':\'even\']">'+
			  			...
			  			'<span class="money">{{key.type}}&nbsp;&nbsp;&nbsp;¥{{key.money}}</span>'+
//			  			'<span class="state">{{key.state}}</span>'+
	...
	mounted(){
		var self = this;
		/*this.$nextTick(()=>{
			jQuery("#"+this.id).slide({mainCell:"ul.order_info",interTime:self.time,autoPage:true,effect:"topLoop",autoPlay:true,vis:this.vis});
		})*/
	// setInterval(function(){self.doscroll();},5000);
	},
	watch:{
		orders:{
