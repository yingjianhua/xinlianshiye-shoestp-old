...
public class urlRewrite implements Filter {
  ...
  public void doFilter(
    ...
    if (request.getServletPath().startsWith("/country/") && isMobile(request)) {
      request
          .getRequestDispatcher(request.getServletPath().replaceFirst("/country/", "/m/country/"))
          .forward(request, response);
      return;
    }
    if (request.getServletPath().startsWith("/activity/html/romania/")
        && isMobile(request)
        && request.getServletPath().indexOf("mc") == -1) {
      String path =
          request
              .getServletPath()
              .substring(
                  "/activity/html/romania/".length(),
                  request.getServletPath().indexOf("/", "/activity/html/romania/".length()));
