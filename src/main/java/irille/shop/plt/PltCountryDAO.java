...
public class PltCountryDAO {
  ...
  public static class InsInit extends IduOther<InsInit, PltCountry> {
    ...
    public static PltCountry build(
        String name,
        String shortName,
        String zone,
        Integer currency,
  ...
  public static class Upd extends IduUpdLines<Upd, PltCountry, PltProvince> {
    ...
    public void before() {
      ...
      PropertyUtils.copyPropertiesWithout(
          dbBean, getB(), T.PKEY, T.ENABLED, T.HOT, T.CREATED_BY, T.CREATED_TIME);
      translateUtil.autoTranslateByManageLanguage(dbBean, true);
      setB(dbBean);
      String wheresql = PltProvince.T.MAIN.getFld().getCodeSqlField() + "=?";
      List<PltProvince> listOld =
          BeanBase.list(
              PltProvince.T.MAIN.getFld().getTb().getClazz(), wheresql, false, getB().pkeyValues());
      boolean insFlag;
      for (PltProvince formBean : getLines()) {
        ...
        for (PltProvince bean : listOld) {
          try {
            if (null != formBean.getPkey()
                && bean.getPkey().intValue() == formBean.getPkey().intValue()
                && !bean.getName(PltConfigDAO.manageLanguage()).equals(formBean.getName())) {
              insFlag = false;
      ...
      for (PltProvince bean : listOld) {
        if (getLines().contains(bean)) continue;
        bean.del();
      }
    }
  ...
  public static Page list(
      String countryName, String shortName, String zone, Integer start, Integer limit) {
    if (null == start) {
      start = 0;
    }
    if (null == limit) {
                    ...
                    new CountryView() {
                      {
                        ...
                        setCurrency(
                            (String) bean.get(PltErate.T.SYMBOL.getFld().getCodeSqlField()));
                        ...
                        setIshot(
                            Integer.valueOf(
                                        String.valueOf(bean.get(T.HOT.getFld().getCodeSqlField())))
                                    == 1
                                ? true
                                : false);
                        setIsdefault(
                            Integer.valueOf(
                                        String.valueOf(
                                            bean.get(T.ISDEFAULT.getFld().getCodeSqlField())))
                                    == 1
                                ? true
                                : false);
                      }
