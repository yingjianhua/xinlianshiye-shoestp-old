...
public class ActivityServiceImp implements IActivityService {
  ...
  @Inject private GoogleAnalyticsUtils googleAnalyticsUtils;

  @Inject private PkCompetitionDataDao pkCompetitionDataDao;
  ...
  @Override
  public PkCompetitionPageManageView getPkCompetitionData(
    ...
    switch (type) {
      case "trafficvolume":
        {
          globalDataView.setTop5(
              pkCompetitionDataDao.getTrafficvolumeTop5Count(startDate, endDate));
        ...
        for (ReportRow row : rows) {
          ...
          for (int j = 0; j < metrics.size(); j++) {
            ...
            for (int k = 0; k < values.getValues().size() && k < metricHeaders.size(); k++) {
              ...
              if (!keyword2.equalsIgnoreCase("(not set)")
                  || !keyword1.equalsIgnoreCase("(not set)")) {
          ...
          if (googleAnalyticsView == null) {
            ...
            googleAnalyticsView.setTrafficVolume(
                googleAnalyticsView.getTrafficVolume() + trafficvolume);
  ...
  private void getInq(Map<Integer, GoogleAnalyticsView> map, LocalDate startDate, LocalDate today) {
    Integer[] integers = {
      281, 298, 283, 318, 279, 295, 16, 291, 282, 13, 317, 23, 78, 301, 165, 292
    };
    for (Integer integer : integers) {
      ...
      googleAnalyticsView.setInquiry(
          pkCompetitionDataDao.getInquiry(
              Date.from(startDate.atStartOfDay().toInstant(ZoneOffset.UTC)),
              Date.from(today.atStartOfDay().toInstant(ZoneOffset.UTC)),
              integer));
  ...
  private GoogleAnalyticsView getId(String url) {
    ...
    if (url.indexOf("_p") != -1 && url.endsWith(".html")) {
      googleAnalyticsView.setId(
          Integer.valueOf(url.substring((url.lastIndexOf("_p") + 2), url.indexOf(".html"))));
    ...
    list.forEach(
        stringObjectMap -> {
          ...
          try {
            ...
            dateRange.setStartDate(startDateString);
            dateRange.setEndDate("today"); // TODO
            Metric sessions = new Metric().setExpression("ga:impressions");
            ReportRequest request =
                new ReportRequest()
                    .setViewId(viewId)
                    .setDateRanges(Arrays.asList(dateRange))
