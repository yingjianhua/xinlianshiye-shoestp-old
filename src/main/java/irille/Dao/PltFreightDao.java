...
public class PltFreightDao {
        ...
        Query.sql(sql).queryMaps().stream()
            .map(
                o -> {
                  ...
                  view.setCompany(
                      String.valueOf(o.get(PltFreight.T.COMPANY.getFld().getCodeSqlField())));
                  view.setLogo(String.valueOf(o.get(PltFreight.T.LOGO.getFld().getCodeSqlField())));
                  view.setEnable(
                      Byte.valueOf(
                          String.valueOf(o.get(PltFreight.T.ENABLED.getFld().getCodeSqlField()))));
                  view.setType(
                      Byte.valueOf(
                          String.valueOf(o.get(PltFreight.T.TYPE.getFld().getCodeSqlField()))));
                  return view;
                })
        ...
        Query.sql(sql).queryMaps().stream()
            .map(
                o -> {
                  ...
                  view.setInterval(
                      String.valueOf(o.get(PltFreightLine.T.SECTION.getFld().getCodeSqlField())));
                  return view;
                })
            .collect(Collectors.toList());
    return list;
  ...
  public PltFreight insShipping(FreightManagementView view) {
    ...
    if (pltFreight.getWeightMin().compareTo(pltFreight.getWeightMax()) >= 0)
      throw LOG.err(
          Msgs.priceErr,
          PltFreight.T.WEIGHT_MAX.getFld().getName(),
          PltFreight.T.WEIGHT_MIN.getFld().getName());
  ...
  public void delFreight(Integer pkey, Integer user) {
    ...
    if (BeanBase.list(
                PltFreight.class,
                PltFreight.T.USER_SYS.getFld().getCodeSqlField() + " =" + user,
                false)
            .size()
        == 0) {
