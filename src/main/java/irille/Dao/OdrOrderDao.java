...
/** Created by IntelliJ IDEA. User: lijie@shoestp.cn Date: 2018/12/14 Time: 15:50 */
public class OdrOrderDao {
  ...
  public PrmGroupPurchaseLine getPrmGroupPurchaseLine(Integer integer) {
    ...
    query
        .SELECT(PrmGroupPurchaseLine.T.COUNT, PrmGroupPurchaseLine.T.BOUGHT_COUNT)
        .FROM(PrmGroupPurchaseLine.class)
        .WHERE(PrmGroupPurchaseLine.T.PRODUCT, "=?", integer);
  ...
  public Integer getEasyOrder(Integer integer) {
    BeanQuery query = new BeanQuery();
    query
        .SELECT(EasyOdr.T.COUNYPD)
        .FROM(EasyOdr.class)
        .LEFT_JOIN(EasyOdrline.class, EasyOdrline.T.ORDER_ID, EasyOdr.T.PKEY)
        .LEFT_JOIN(PdtSpec.class, PdtSpec.T.PKEY, EasyOdrline.T.SPEC)
        .WHERE(PdtSpec.T.PRODUCT, "=?", integer);
    EasyOdr easyOdr = (EasyOdr) query.query(EasyOdr.class);
    if (easyOdr == null) return 0;
