...
public class PdtProductDAO {
	...
	public static Page listproduct(String fldvalue, String condition, Integer start, Integer limit) {
		...
		List<PdtProductView> list = irille.pub.bean.Query.sql(sql.LIMIT(start, limit)).queryMaps().stream()
				.map(bean -> new PdtProductView() {
					{
						...
						setCategory(Bean.load(PdtCat.class, (Integer) bean.get(T.CATEGORY.getFld().getCodeSqlField()))
								.getName());
						setSupplier(
								Bean.load(UsrSupplier.class, (Integer) bean.get(T.SUPPLIER.getFld().getCodeSqlField()))
										.getName());
					}
	...
	public static class Publish extends IduIns<Publish, PdtProduct> {
		...
		public void before() {
			...
			translateUtil.newAutoTranslate(getB(),
					translateUtil.buildFilter(getB().getName(), FldLanguage.Language.en));
			PropertyUtils.copyProperties(dbBean, getB(), T.NAME, // 名字
					T.IS_VERIFY, // 审核状态
					T.CATEGORY, // 产品类目
					...
					T.DESCRIPTION, // 详细介绍
					T.SEO_DESCRIPTION, T.SEO_KEYWORD, T.SEO_TITLE, T.PRODUCT_TYPE, T.TARGETED_MARKET);
	...
	public static class Upd extends IduUpd<Upd, PdtProduct> {
		...
		public void updateFavorite(int id) {
			String sql = "update " + FormaterSql.getTableName(T.PKEY) + " set "
					+ T.Favorite_Count.getFld().getCodeSqlField() + "=" + T.Favorite_Count.getFld().getCodeSqlField()
					+ "+1 where " + T.PKEY + " = ?";
	...
	public static class Select extends IduOther<Select, PdtProduct> {
		...
		public static List<PdtView> getIndexProduct(Integer supplier, FldLanguage.Language lang) {
			SQL sql = new SQL() {
				{
					SELECT(T.PKEY, T.PICTURE, T.NAME, T.CUR_PRICE).FROM(PdtProduct.class)
							...
							// TODO 这里缺少O2O的商品列表
							.WHERE(T.PRODUCT_TYPE, "=?", Pdt.OProductType.GENERAL).ORDER_BY(T.UPDATE_TIME, "DESC")
							.LIMIT(0, 8);
				}
			};

			return irille.pub.bean.Query.sql(sql).queryList(PdtProduct.class).stream().map(bean -> new PdtView() {
				{
	...
	public static Page getProducts(Integer start, Integer limit, SeartchView seartch) {
		...
		SQL sql = new SQL() {
			{
				SELECT(T.PKEY, "pkey").SELECT(T.PICTURE, "pdtImage").SELECT(T.NAME, "pdtName").SELECT(T.CODE, "pdtCode")
						.SELECT(PdtCat.T.NAME, "pdtCategory").SELECT(T.MKT_PRICE, "mktPrice")
						.SELECT(T.CUR_PRICE, "curPrice").SELECT(T.STOCK, "stock").SELECT(T.MIN_OQ, "minOq")
						.SELECT(UsrSupplier.T.NAME, "supplierName").SELECT(T.SOLD_TIME_B, "soldTimeB")
						.SELECT(T.IS_VERIFY, "isVerify").SELECT(T.STATE, "state").FROM(PdtProduct.class)
						...
						.WHERE(T.STATE, "<>?", OState.DELETE).WHERE(T.STATE, "<>?", OState.MERCHANTDEL);
				if (seartch != null) {
					WHERE(seartch.getProductType() != null, T.PRODUCT_TYPE, "=?", seartch.getProductType())
							.WHERE(null != seartch.getProductCode(), PdtProduct.T.CODE, " like ?",
									"%" + seartch.getProductCode() + "%")
							.WHERE(seartch.getShopName() != null, UsrSupplier.T.NAME, "like ?",
									"%" + seartch.getShopName() + "%")
							.WHERE(seartch.getProductClassification() != null, PdtCat.T.NAME, "like ?",
		...
		Integer count = irille.pub.bean.Query.sql(sql).queryCount();
		List<ProductsView> list = irille.pub.bean.Query.sql(sql.ORDER_BY(PdtProduct.T.SOLD_TIME_B, " DESC ")
				.ORDER_BY(PdtProduct.T.PKEY, " DESC ").LIMIT(start, limit)).queryMaps().stream()
				.map(o -> new ProductsView() {
					{
						SQL sql = new SQL() {
							{
								SELECT(O2O_Product.class).FROM(O2O_Product.class)
	...
	public static void review(Integer id, Byte status, String message) {
		...
		if (pp.getProductType() == Pdt.OProductType.O2O.getLine().getKey()) {
			SQL o2o = new SQL() {
				{
					SELECT(O2O_Product.class).FROM(O2O_Product.class).WHERE(O2O_Product.T.PRODUCT_ID, "=?", id);
				}
			};
	...
	public Integer productCount(Integer supplierpkey) {
		SQL pdt = new SQL().SELECT(PdtProduct.class).FROM(PdtProduct.class)
				.WHERE(PdtProduct.T.SUPPLIER, "=?", supplierpkey).WHERE(PdtProduct.T.STATE, "=?", OState.ON).or()
				.WHERE(PdtProduct.T.SUPPLIER, "=?", supplierpkey).WHERE(PdtProduct.T.STATE, "=?", OState.OFF)
	...
	public Integer privateproductCount(Integer supplierpkey) {
		SQL pdt = new SQL() {
			{
				SELECT(PdtProduct.T.PKEY).FROM(PdtProduct.class).WHERE(PdtProduct.T.SUPPLIER, "=?", supplierpkey)
						.WHERE(PdtProduct.T.PRODUCT_TYPE, "=?", Pdt.OProductType.PrivateExpo);
			}
		};
		return irille.pub.bean.Query.sql(pdt).queryCount();
	...
	public Integer wareHouseProductCount(Integer supplierpkey) {
		SQL pdt = new SQL() {
			{
				SELECT(PdtProduct.T.PKEY).FROM(PdtProduct.class).WHERE(PdtProduct.T.SUPPLIER, "=?", supplierpkey)
						.WHERE(PdtProduct.T.STATE, "=?", OState.OFF).WHERE(PdtProduct.T.SOLD_TIME_B, " IS NULL ");
			}
		};
		return irille.pub.bean.Query.sql(pdt).queryCount();
	}
	...
	public Integer verifyProductCount(Integer supplierpkey) {
		SQL priPdt = new SQL() {
			{
				SELECT(O2O_PrivateExpoPdt.T.PDT_ID).FROM(O2O_PrivateExpoPdt.class).WHERE(
						O2O_PrivateExpoPdt.T.VERIFY_STATUS, "=" + O2O_PrivateExpoPdtStatus._DEFAULT.getLine().getKey());
			}
		};

		SQL pdt = new SQL() {
			{
				SELECT(PdtProduct.T.PKEY).FROM(PdtProduct.class)
	...
	public Integer failedProductCount(Integer supplier) {
		SQL sql = new SQL() {
			{
				SELECT(PdtProduct.T.PKEY).FROM(PdtProduct.class).WHERE(PdtProduct.T.SUPPLIER, " =? ", supplier)
