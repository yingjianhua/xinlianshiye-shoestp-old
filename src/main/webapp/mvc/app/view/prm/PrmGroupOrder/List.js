Ext.define('mvc.view.prm.PrmGroupOrder.List',{
extend : 'Ext.panel.Panel',
oldId : 'PrmGroupOrder_list_',
loadMask : true,
multiSelect : true,
roles : '',
layout : 'border',
...
mainActs.push({
		...
		items : [{
				xtype : Ext.create('mvc.view.prm.PrmGroupOrder.ListMain',{
							title : '团购订单',
							...
							listeners : 							 {
				                ...
				                selectionchange: function(model, records) {
				                    if (records.length === 1){
        								...
        								this.mdLineTable.store.filter([{'id':'filter', 'property':'orderid','value':records[0].get('bean.pkey')}]);
    									if (this.roles.indexOf('upd') != -1)
				...
				xtype : Ext.create('mvc.view.prm.PrmGroupOrder.ListLinePrmGroupOrderLine',{
							title : '订单明细',
...
onIns : function(){
		var win = Ext.create('mvc.view.prm.PrmGroupOrder.Win',{
			title : this.title+'>新增'
		});
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/prm_PrmGroupOrder_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.prm.PrmGroupOrder.WinSearch',{
