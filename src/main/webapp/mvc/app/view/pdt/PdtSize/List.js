Ext.define('mvc.view.pdt.PdtSize.List',{
...
this.columns = [{text : '名称',width : 278,dataIndex : 'bean.name',sortable : true,renderer : mvc.Tools.JsonRenderer()}
	,{text : '产品类目',width : 100,dataIndex : 'bean.productCategory',sortable : true,renderer : mvc.Tools.JsonRenderer(),md : 'pdt',mn : 'view.pdt.PdtCat.List'}
	...
	,{text : '建档时间',width : 140,dataIndex : 'bean.createTime',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s')}
		...
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.pdt.PdtSize'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.pdt.PdtSize.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.pdt.PdtSize', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/pdt_PdtSize_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/pdt_PdtSize_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.pdt.PdtSize.WinSearch',{
