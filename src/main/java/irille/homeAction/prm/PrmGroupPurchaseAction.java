...
import irille.homeAction.usr.UsrSupplierAction;
...
public class PrmGroupPurchaseAction extends HomeAction<PrmGroupPurchase>{
	...
	public void getLineData() throws Exception{
		...
		String groupLineWhere = PrmGroupPurchaseLine.T.MAIN.getFld().getCodeSqlField() + " = " + getUnionPkey();
		...
		String where = PdtProduct.T.STATE.getFld().getCodeSqlField() + " = " + Pdt.OState.ON.getLine().getKey();
		if(category != null && category.trim() != ""){
			where += " AND "+ PdtProduct.T.CATEGORY.getFld().getCodeSqlField() + " in(" + getAllCat(category) + ") ";
		}
		if(sort != null && sort.trim() != ""){
				...
				for(PrmGroupPurchaseLine unionLine : unionLineList){
					if(catPdtList != null){
						if(unionLine.getProduct().equals(pdt.getPkey())){
							...
							if(unionLine.gtProduct().getProductType().equals(Pdt.OProductType.GROUP.getLine().getKey())){
								jsonLine.put("source", crtJsonByBean(translateUtil.getAutoTranslate(unionLine.gtProduct().gtSourceProduct(), HomeAction.curLanguage())));
								JSONObject jsonPro = crtJsonByBean(translateUtil.getAutoTranslate(unionLine.gtProduct(), HomeAction.curLanguage()));
	 ...
	 private Set<String> catSet = new HashSet<String>();
	    ...
	    public void getAllSubclassification(String category) {
	        List<PdtCat> catList = BeanBase.list(PdtCat.class, PdtCat.T.CATEGORY_UP.getFld().getCodeSqlField() + " in(" + category + ") ", false);
	        String childCat = "";
	 ...
	 * 获取联合采购商品详情
	 * getPkey() 		  --->   联合采购明细的pkey
	 * groupPurchaseLine  --->   用户点击的联合采购明细对象
	 * product  		  --->   此联合采购明细对象所对应的产品
	 * supplier			  --->   此产品的供应商
	 * pdtCatList		  --->   此产品的分类
	 * colorToSpec  	  --->   此产品包含的颜色及该颜色对应的规格集合
	 * colorToImg  		  --->   此产品包含的颜色及该颜色对应的产品图片
	 * colorShow   		  --->   该颜色及其首张对应产品图
	 * proAttribute		  --->   此产品的属性
	 * groupPurchase	  --->   联合采购对象
	 * recommendationPdt  --->   供应商的推荐商品
	 * purchase			  --->   登录的采购商
	 * @throws Exception 
	 */
	public String getGroupPdt() throws Exception{
		...
		product = translateUtil.getAutoTranslate(groupPurchaseLine.gtProduct(), curLanguage());//获取点击的联合采购产品
		if(getPurchase() != null){
...
//		List<PdtProduct> newProList =  BeanBase.list(PdtProduct.class,PdtProduct.T.SUPPLIER + " = " + supplier.getPkey() + " AND " + PdtProduct.T.IS_NEW + " = 1 ",false);
		List<PdtProduct> newProList =  BeanBase.list(PdtProduct.class,PdtProduct.T.STATE.getFld().getCodeSqlField() + " = " + Pdt.OState.ON.getLine().getKey() + " AND " + PdtProduct.T.IS_VERIFY.getFld().getCodeSqlField() + " = " + Sys.OYn.YES.getLine().getKey() + " AND " + PdtProduct.T.PRODUCT_TYPE.getFld().getCodeSqlField() + " = ? AND " + PdtProduct.T.SUPPLIER.getFld().getCodeSqlField() + "=? AND " + PdtProduct.T.IS_NEW.getFld().getCodeSqlField() + "=?", false, Pdt.OProductType.GENERAL.getLine().getKey(),supplier.getPkey(), Sys.OYn.YES.getLine().getKey());
		if(newProList.size() > 0) {
			Random r = new Random();
			for(int i = 0;i<5;i++){
				irille.homeAction.usr.dto.PdtView view = new irille.homeAction.usr.dto.PdtView();
				PdtProduct pdt = translateUtil.getAutoTranslate(newProList.get(r.nextInt(newProList.size())), curLanguage());
				view.setPdt(pdt);
		...
		for(int i =0 ; i<colorArr.length;i++){
			...
			List<PdtSpec> colorSpecList = BeanBase.list(PdtSpec.class, PdtSpec.T.PRODUCT.getFld().getCodeSqlField() + " = ? AND " + PdtSpec.T.COLOR.getFld().getCodeSqlField() + " = ? ",false,product.getPkey(),colorArr[i]);
	...
	public String send() throws Exception{
		...
		//获取该采购商的所有地址
		List<UsrPurchaseLine> purchaseLineList = BeanBase.list(UsrPurchaseLine.class,UsrPurchaseLine.T.PURCHASE.getFld().getCodeSqlField() + " = " + purchase.getPkey() + " AND " + UsrPurchaseLine.T.ADDRSSTYPE.getFld().getCodeSqlField() + " = " + Usr.OAddress.COMMON.getLine().getKey(),false);
		List<AddressView> address = AddressView.buildByPurchaseLine(purchaseLineList);
		List<UsrPurchaseLine> billAddressList = BeanBase.list(UsrPurchaseLine.class, UsrPurchaseLine.T.PURCHASE.getFld().getCodeSqlField() + " =? AND " + UsrPurchaseLine.T.ADDRSSTYPE.getFld().getCodeSqlField() + " = ? ",false,purchase.getPkey(),Usr.OAddress.BILLED.getLine().getKey());
		...
		List<PltPay> payMethod = BeanBase.list(PltPay.class,PltPay.T.SUPPLIER.getFld().getCodeSqlField() + " = " +supplier.getPkey() + " AND " + PltPay.T.ENABLED.getFld().getCodeSqlField() + " <> " + Sys.OEnabled.FALSE.getLine().getKey(),false);
		...
		List<PltPay> payList = BeanBase.list(PltPay.class,PltPay.T.SUPPLIER.getFld().getCodeSqlField() + " = " + supplier.getPkey() + " AND " + PltPay.T.ENABLED.getFld().getCodeSqlField() + " = " + Sys.OEnabled.TRUE.getLine().getKey(),false);
