...
public class UsrSupImDAO {
	 ...
	 * <script type="text/javascript">
		...
		s1.async=true;
		s1.src='https://embed.tawk.to/5b95d3eaf31d0f771d8496a4/default';
		s1.charset='UTF-8';
	...
	public static class UpdImSetting extends IduOther<UpdImSetting,UsrSupIm>{
		...
		public void run() {
			String where = UsrSupIm.T.SUPPLIER.getFld().getCodeSqlField() + " = " +SellerAction.getSupplier().getPkey();
			switch(Integer.valueOf(getB().getType())){
				case 0:
					where += " AND " + UsrSupIm.T.TYPE.getFld().getCodeSqlField() + " = " + Integer.valueOf(Usr.OIMType.PHONE.getLine().getKey());
					try{
				...
				case 1:
					where += " AND " + UsrSupIm.T.TYPE.getFld().getCodeSqlField() + " = " + Integer.valueOf(Usr.OIMType.COMPUTER.getLine().getKey());
					try{
		...
		private void updBean(){
			...
			getB().setAccount(getAccount());
			getB().setDemo(getB().getDemo().trim());
			PropertyUtils.copyPropertiesWithout(model, getB(),UsrSupIm.T.PKEY,UsrSupIm.T.SUPPLIER,UsrSupIm.T.ROW_VERSION);
	...
	public static List<UsrSupIm> getImSetting(Integer supId){
		List<UsrSupIm> imList = BeanBase.list(UsrSupIm.class,UsrSupIm.T.SUPPLIER.getFld().getCodeSqlField() + " =? ",false,supId);
		return imList;
	}
	public static List<UsrSupIm> getEnabledImSetting(Integer supId){
		List<UsrSupIm> imList = BeanBase.list(UsrSupIm.class,UsrSupIm.T.SUPPLIER.getFld().getCodeSqlField() + " =? AND " + UsrSupIm.T.ENABLED.getFld().getCodeSqlField() + " =? ",false,supId,Sys.OEnabled.TRUE.getLine().getKey());
		return imList;
	}
