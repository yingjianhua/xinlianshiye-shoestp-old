...
public class O2OActivityDao {
  ...
  public Page<O2OActivityView> pageView(Integer start, Integer limit, O2OActivityView condition) {
    if (null == condition) condition = new O2OActivityView();
    BeanQuery<O2O_Activity> query =
        Query.SELECT(O2O_Activity.class)
            // 起始时间
            .WHERE(
            ...
            // 活动名称
            .WHERE(
                condition.getName() != null && !"".equals(condition.getName().trim()),
                O2O_Activity.T.NAME,
                "like ?",
                "%" + condition.getName() + "%");
  ...
  public List<Map<String, Object>> privetePdtList(
      int start,
      int limit,
      Integer status,
  ...
  public void upd(List<O2O_Activity> activities) {
    BatchUtils.batchUpd(
        O2O_Activity.class,
        Arrays.asList(O2O_Activity.T.STATUS),
        Arrays.asList(O2O_Activity.T.PKEY),
        activities);
  }
}
