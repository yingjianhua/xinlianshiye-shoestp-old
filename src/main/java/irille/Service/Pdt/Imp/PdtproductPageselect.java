...
public class PdtproductPageselect {
  ...
  @Inject IPdtProductService iPdtProductServer;
  ...
  public Map getNewProducts(IduPage page) {
    ...
    sql.select(PdtProduct.T.PKEY, PdtProduct.T.NAME, PdtProduct.T.PICTURE, PdtProduct.T.CUR_PRICE)
        ...
        .desc(PdtProduct.T.UPDATE_TIME)
        .desc(PdtProduct.T.MY_ORDER);
    if (page.getWhere() != null)
      sql.eqAutoAnd(
          PdtProduct.T.CATEGORY,
          Integer.valueOf(page.getWhere()),
          number -> {
            return page.getWhere() != null
                && page.getWhere().length() > 0
  ...
  public PdtYouMayLikeView getProduct(int id) {
    ...
    sql.select(PdtProduct.T.PKEY, PdtProduct.T.NAME, PdtProduct.T.CUR_PRICE, PdtProduct.T.PICTURE)
        .from(PdtProduct.T.NAME)
        .eqAutoAnd(PdtProduct.T.PKEY, id)
  ...
  public long getProductAvgById(long id) {
    return FormaterSql.castLong(
        BeanBase.queryOneRowIsNull(
            "SELECT avg(product_satisfaction) FROM `pdt_comment` where product=?", id));
  }
  ...
  public String getCategorieNameById(long id) {
    ...
    return translateUtil.getAutoTranslate(
        sql.castString(BeanBase.queryOneRowIsNull(sql.buildSql(), id)), HomeAction.curLanguage());
  }
  ...
  public List getYouMayLikeByPdtId(int id) {
    ...
    formaterSql.select(
        PdtProduct.T.PKEY, PdtProduct.T.NAME, PdtProduct.T.CUR_PRICE, PdtProduct.T.PICTURE);
    List result = new ArrayList();
    List<Map> list =
        BeanBase.list(sql, id).stream()
            .map(
                objects -> {
   ...
   */
  public List getProductsCategorieNodesByCategory(IduPage iduPage, long category)
  ...
  public List getBreadcrumbNavByPdtId(Integer id) {
    ...
    return pdtProductDao.getBreadcrumbNav(
        sql.castLong(BeanBase.queryOneRowIsNull(sql.buildSql(), sql.getParms())));
  }
  ...
  public PdtProductSaveView sellerGetProductById(int id, int supId) throws JSONException {
    return sellerGetProductById(
        ...
        pdtSpec -> {
          ...
          saveView.setPrice(
              pdtSpec.getPrice().doubleValue() == -1 ? null : pdtSpec.getPrice().doubleValue());
   ...
   * @date 2018/9/25 9:59
   */
  public PdtProductSaveView sellerGetProductById(
      int id, int supId, Function<PdtSpec, PdtProductSpecSaveView> function) throws JSONException {
    ...
    view.setAttr(
        pdtProduct.getNormAttr() == null
            ? new ArrayList<>()
            : Stream.of(pdtProduct.getNormAttr().split(","))
                .map(
                    s -> {
                      if (s != null) {
    ...
    view.setSpecColor(
        Arrays.asList(pdtProduct.getColorAttr().split(",")).stream()
            .map(
                s -> {
    ...
    view.setSpecSize(
        Arrays.asList(pdtProduct.getSizeAttr().split(",")).stream()
            .map(
                s -> {
                  if (s == null) {
                    return null;
  ...
  private List<PdtProductSpecSaveView> getSpec(
      int pdtId, Function<PdtSpec, PdtProductSpecSaveView> function) {
    FormaterSql sql = FormaterSql.build();
    sql.eq(PdtSpec.T.PRODUCT);
    return PdtSpec.list(PdtSpec.class, sql.toWhereString(), false, pdtId).stream()
        .map(function)
        .collect(Collectors.toList());
  }
