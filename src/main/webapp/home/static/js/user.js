...
var user_obj = {
    ...
    user_login_binding: function () {
        ...
        frm_binding.submit(function () {
            return false;
        });
        frm_binding.find('button:submit').click(function () {
            if (global_obj.check_form(frm_binding.find('*[notnull]'))) {
                return false;
            }
            ;
            $(this).attr('disabled', true);
        ...
        $(".index_boxes.favorites").carousel(
            {
                itemsPerMove: 1,
                height: 174,
                width: ($(window).width() >= 1250 ? 568 : 470),
                duration: 200,
                vertical: !1,
                step: 1
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.ret == 1) {
                        ...
                        //提交编辑支付方式
                        $('form[name=pay_edit_form]').submit(function () {
                            return false;
                        });
    ...
    user_address: function () {
        ...
        $('a.chzn-single').off().on('click', function () {
            ...
            if ($(this).hasClass('chzn-single-with-drop')) {
                ...
                $(this).blur().addClass('chzn-single-with-drop').next().css({
                    'left': '0',
                    'top': '27px'
                }).parent().addClass('chzn-container-active').css('z-index', '10').find('li.result-selected').addClass('highlighted');
        ...
        $('.editAddr').on('click', 'button[id=useAddress]', function () {
            if (!check_form_address()) {
                return false;
            }
            //console.log(1111)
            var addressId = $("#setAddressId input[name='bean.pkey']").val();
            $.ajax({
                url: '/home/usr_UsrPurchaseLine_addAddress',
                type: 'post',
                data: $("#setAddressId").serialize(),
                dataType: 'json',
                success: function (data) {
                	if(data.ret == 1){
                		if(addressId == undefined || addressId == ""){
                			layer.msg(lang_obj.addressfrom.Inserted_successfully,{icon:1,time:2000},function(){
                				location.reload();
                			});
                		}else{
                			layer.msg(lang_obj.addressfrom.Successfully_modified,{icon:1,time:2000},function(){
                				location.reload();
                			});
                		}
                	}else{
...
//		});
        ...
        $('input[name=Email], input[name=FirstName], input[name=LastName], input[name=PhoneNumber]').focus(function () {
            $(this).next().next('p.errorInfo').text('');
        });
        $('input[name=AddressLine1], input[name=City], input[name=ZipCode]').focus(function () {
            $(this).next('p.errorInfo').text('');
        });
        ...
        function check_form_address() {
            //typeAddr=$('.editAddr form input[name="bean.name"]').val();
            firstname = $('.editAddr form input[name="bean.name"]');
            lastname = $('.editAddr form input[name="bean.surname"]');
            address = $('.editAddr form input[name="bean.address"]');
            city = $('.editAddr form input[name="bean.city"]');
            state_obj = $('.editAddr form #sldProvinceHiden');
            country = $('.editAddr form #sldCountryHiden');
            //taxcode=$('.editAddr form #taxCodeValue');
            //tariff=$('.editAddr form #tariffCodeValue');
            zipcode = $('.editAddr form input[name="bean.emailcode"]');
            phone = $('.editAddr form input[name="bean.phonenumber"]');
           ...
           /* else if (ueeshop_config.lang != 'zh_TW' && ueeshop_config.lang != 'jp' && lastname.val().length < 2)
            ...
            if (city.val() == '')
            //	alert("Cities must not be empty. Please choose cities.");
           ...
           /* else if (ueeshop_config.lang != 'zh_TW' && ueeshop_config.lang != 'jp' && city.val().length < 3)
            ...
            //	alert("Countries must not be empty. Please choose countries.");
...
//			}
            ...
            state_obj.size() && state_obj.siblings('p.errorInfo').text(stateTips);
            country.size() && country.siblings('p.errorInfo').text(countryTips);
            ...
            if (firstnameTips == '' && addressTips == '' && cityTips == '' && countryTips == '' && zipTips == '' && phoneTips == '')
                return true;
            else
                return false;
        }
    ...
    address_init: function () {
        ...
        $('.addAddress').click(function () {
            ...
            $('#addressForm').slideUp('fast', function () {
                $('.billing_addr, .shipping_addr').slideUp('fast', function () {
                    $('#addressForm').children('.big').text($(self).closest('.shipping_addr').children('.big').text());
                    $('#addressForm').slideDown('fast');
    ...
    inbox_init: function () {
        ...
        user_obj.inbox_ajax_list('inbox_list', 0);
        user_obj.inbox_ajax_list('Outbox_list', 1);
        user_obj.inbox_ajax_list('Outbox_list', 1);
        $("#ButtonReply").on("click", function () {
            $.ajax({
                url: "/home/usr_UsrMessages_replyMessage",
                data: {
                    id: $("input[class='id']", $("#reply")).val(),
                    message: $("textarea[name='Content']", $("#reply")).val()
                }, type: "POST",
                success: function (data) {
                    $("input[name='id']", $("#reply")).val("")
                    $("textarea[name='Content']", $("#reply")).val("")
                    user_obj.inbox_ajax_list('inbox_list', 0);
                }
        ...
        $("#submit_button").on("click", function () {
            if (global_obj.check_form($('div.inbox_form *[notnull]'))) {
                return false
            }
            $.ajax({
                url: "/home/usr_UsrMessages_writeMessage",
                data: {
    ...
    inbox_ajax_list: function (name, type) {
        var obj = $('#' + name),
            page = obj.attr('page');
        $.ajax({
            url: "/home/usr_UsrMessages_getMyInbox",
            data: {
                type: type,
                page: page
            },
            success: function (data) {
                renderInbox(data.result, $("#" + name))
            }

        })
    }
};
var renderInbox = function (data, dom) {
    var html = '';
    $("div.turn_page", dom).html("")
    $.each(data.items, function (i, v) {
        html += "     <li><i class='fl " + (v.status == 1 ? "read" : "") + "'></i><a class=\"fl\" href=\"javascript:void(0);\"  type='" + $(dom).attr("id") + "' id='" + v.id + "'\n" +
            "                                                                  title='" + v.title + "'>" + v.title + "</a><span class=\"time fr\">" + v.sendTime + "</span>\n" +
            "</li>";
    })
    $("ul", dom).html(html)
    $("ul li a", dom).on("click", function () {
        var self = this;
        var id = $(this).attr("id")
        $.ajax({
            ...
            success: function (data) {
                if ($(self).attr("type") === 'inbox_list') {
                    $('.inbox_menu .menu').eq(4).removeClass('hide').siblings().addClass('hide');
                    var doms = $("#reply")
                    $(".reply", doms).val("")
                } else {
                    var doms = $("#read")
                    $('.inbox_menu .menu').eq(3).removeClass('hide').siblings().addClass('hide');
                }
                $($("a", doms)).addClass('current').parent().siblings().children('a').removeClass('current');
    ...
    getPage($("div.turn_page", dom), data.currentPage, data.totalCount, data.limit, function () {
        $("div.turn_page a", dom).on("click", function () {
            $.ajax({
                url: "/home/usr_UsrMessages_getMyInbox",
                data: {
                    type: dom.attr("type"),
...
function getPage(div, curr, count, size, functions) {
    $("div.page ul").html("")
    var last = parseInt((count / size))
    ...
    for (i = 1; i <= last; i++) {
        ...
        $("div.page ul", div).append("<li><a action='" + i + "' href=\"javascript:void(0)\">" + i + "/" + last + "</a></li>")
    }
    $("div.page .cur", div).text(curr + "/" + last)
    if (functions != null) {
        functions();
    }
}
