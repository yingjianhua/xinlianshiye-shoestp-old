		...
		cart_checkout:function(){
			...
			*/
			...
			$('#cart_checkout').on('click touchstart', function(e){
				...
				var address = $("#PlaceOrderFrom input[name=address]").val();
				var carts = $("#PlaceOrderFrom #cartPkeys").val();
				var delivery = $("#PlaceOrderFrom select[name=express]").val();
				var payType = $("#PlaceOrderFrom select[name=payType]").val();
				if(payType == -1){
					$('html').tips_box('请选择支付方式', 'error');
					return false;
				}
			...
			/*$('#cart_checkout').on('click touchstart', function(e){
				...
				$.post('/?do_action=cart.placeorder', $('#PlaceOrderFrom').serialize()+Attr, function(data){
					if(data.ret==1){//成功
						window.location.href='/cart/complete/'+data.msg.OId+'.html';
