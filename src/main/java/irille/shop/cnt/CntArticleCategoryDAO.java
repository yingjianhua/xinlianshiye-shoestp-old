...
public class CntArticleCategoryDAO {
    ...
    public static class Ins extends IduIns<Ins, CntArticleCategory> {
        ...
        public void after() {
            ...
            PropertyUtils.copyPropertiesWithout(dbBean, getB(), CntArticleCategory.T.CREATE_BY, CntArticleCategory.T.CREATE_TIME);
            setB(dbBean);
            super.after();
        }
    }
...
//            Validate.regex(LOG, "[0-9]{1,}.*", getB().getName(), CntPageType.T.NAME.getFld().getName(), "首字母不能为数字");
//            Validate.hasa_Z_Length_Lt_8(LOG, getB().getLang(), CntPageType.T.LANG.getFld().getName(), "应为小于八位的英文字母");
//            for (CntPageTypeLine cntPageTypeLine : getLines()) {
//                Validate.regex(LOG, "[0-9]{1,}.*", cntPageTypeLine.getName(), CntPageTypeLine.T.NAME.getFld().getName(), "首位不能为数字");
//            }
//            super.valid();
        }
    }
