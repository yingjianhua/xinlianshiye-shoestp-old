...
public class SVSInfo extends BeanInt<SVSInfo> {
	...
	public enum T implements IEnumFld {
		PKEY(Tb.crtIntPkey()), SUPPLIER(UsrSupplier.fldOutKey()), RESEARCH(Sys.T.JSON, "研发能力"),
		PRODUCTION_CAPACITY(Sys.T.JSON, "生产能力"), REAL_FACTORY(Sys.T.JSON, "真实工厂"), PRODUCT_QUALITY(Sys.T.JSON, "产品质量"),
		FOREIGN_TRADE_TEAM(Sys.T.JSON, "外贸团队"), EXHIBITION_ATTENDED(Sys.T.JSON, "参加过的展会"), PARTNER(Sys.T.JSON, "合作商"),
		BASE_SCORE(Sys.T.INT, "基础分"), DYNAMIC_SCORE(Sys.T.INT, "动态分"), STATUS(Tb.crt(SVSAuthenticationStatus.DEFAULT)),
		GRADE(Tb.crt(SVSGradeType.DEFAULT)), APPLICATION_TIME(Sys.T.DATE_TIME, "申请认证时间"),
		AUTHENTICATION_TIME(Sys.T.DATE_TIME__NULL, "认证时间"), FAILURE_REASONS(Sys.T.STR__200_NULL, "认证失败原因"),
