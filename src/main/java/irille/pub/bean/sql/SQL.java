...
public class SQL {
  ...
  public <T extends BeanMain<?, ?>> SQL LEFT_JOIN(
      Class<T> beanClass, IEnumFld fld1, IEnumFld fld2) {
    return mybatisSQL.LEFT_OUTER_JOIN(beanClass, fld1, fld2);
  }

  public <T extends BeanMain<?, ?>> SQL LEFT_JOIN(
      SQL sql, Class<T> beanAlias, IEnumFld fld1, IEnumFld fld2) {
  ...
  public <T extends BeanMain<?, ?>> SQL WHERE(
  ...
  public <T extends BeanMain<?, ?>> SQL WHERE(
      boolean b, IEnumFld fld, String conditions, Serializable... params) {
    if (b) {
      mybatisSQL.WHERE(fld, conditions);
      mybatisSQL.params(params);
    }
  ...
  public <T extends BeanMain<?, ?>> SQL Mconditions(
      List<MconditionsView> list, Class<T> beanClass) {
    // 存放实体类字段名称
    Object[] ObEnums = null;
    // 循环实体类中字段
  ...
  public <T extends BeanMain<?, ?>> SQL orWhere(
      IEnumFld fld, String conditions, Serializable... params) {
  ...
  public <T extends BeanMain<?, ?>> SQL HAVING(
      IEnumFld fld, String condition, Serializable... params) {
  ...
  public String JSON_EXTRACT(IEnumFld fld, String key) {
    // return mybatisSQL.columnLabelWithAlias(fld)+"->"+key+" as
    // "+mybatisSQL.columnLabel(fld);//老版本mysql不支持的语法
    return "JSON_EXTRACT("
        + mybatisSQL.columnLabelWithAlias(fld)
        + ", \"$."
        + key
        + "\") as "
        + mybatisSQL.columnLabel(fld);
  }
  ...
  class MybatisSQL extends AbstractSQL<SQL> {
    ...
    public <T extends BeanMain<?, ?>> SQL WHERE(IEnumFld fld, String conditions, IEnumFld fld2) {
      if (isSelect())
        return super.WHERE(
            columnLabelWithAlias(fld) + " " + conditions + " " + columnLabelWithAlias(fld2));
      else return super.WHERE(columnLabel(fld) + " " + conditions + " " + columnLabel(fld2));
    }

    public <T extends BeanMain<?, ?>> SQL WHERE(IEnumFld fld, String conditions) {
      if (isSelect()) return super.WHERE(columnLabelWithAlias(fld) + " " + conditions);
      else return super.WHERE(columnLabel(fld) + " " + conditions);
    }
    ...
    public String toString() {
      ...
      if (isSelect() && limit > 0) sql += " LIMIT " + start + "," + limit;
