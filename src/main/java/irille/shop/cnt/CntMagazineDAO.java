  ...
  public static class Upd extends IduUpd<Upd, CntMagazine> {
    ...
    public void before() {
      ...
      if (getB().getSpecialPictures().indexOf("https://image.shoestp.com") != -1) {
        getB()
            .setSpecialPictures(
                getB().getSpecialPictures().replace("https://image.shoestp.com", ""));
      }
      // getB().setCreatedTime(Env.getSystemTime());//自动生成修改时间
      PropertyUtils.copyPropertiesWithout(
          dbBean, getB(), CntMagazine.T.CREATED_BY, CntMagazine.T.CREATED_TIME);
      setB(dbBean);
    }
  }
                    ...
                    new magazineView() {
                      {
                        ...
                        setSpecialPictures(
                            (String)
                                bean.get(
                                    CntMagazine.T.SPECIAL_PICTURES.getFld().getCodeSqlField()));
                        setTime((Date) bean.get(CntMagazine.T.TIME.getFld().getCodeSqlField()));
                        setCycle(
                            (Integer) bean.get(CntMagazine.T.CYCLE.getFld().getCodeSqlField()));
                        setContent(
                            (String) bean.get(CntMagazine.T.CONTENT.getFld().getCodeSqlField()));
                        setUrl(
                            (String) bean.get(CntMagazine.T.CONTENTURL.getFld().getCodeSqlField()));
                        setCreatedby(
                            BeanBase.load(
                                    SysUser.class,
                                    Integer.valueOf(
                                ...
                                .getLoginName());
                        setCreatedtime(
                            (Date) bean.get(CntMagazine.T.CREATED_TIME.getFld().getCodeSqlField()));
                      }
                    })
