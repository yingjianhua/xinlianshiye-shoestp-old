...
public class ColorView {
	...
	public ColorView(String name,Integer id, String color, String img, Integer proId, long qty,BigDecimal totalPrice) {
		this.name = name;
		this.id = id;
		this.color = color;
		this.img = img;
	...
	public static List<ColorView> buildColor2(List<OdrOrder> orderList){
		...
		for(OdrOrder order : orderList){
			List<OdrOrderLine> orderLineList = BeanBase.list(OdrOrderLine.class,OdrOrderLine.T.MAIN.getFld().getCodeSqlField() + " = " + order.getPkey(),false);
	...
	public static ColorView buildColor(Integer pdtPkey,String color){
		...
		try {
			...
			PdtSpec spec = BeanBase.list(PdtSpec.class, PdtSpec.T.PRODUCT.getFld().getCodeSqlField() + " = " + pdtPkey + " AND " + PdtSpec.T.COLOR.getFld().getCodeSqlField() + " = " + color,false).get(0);
