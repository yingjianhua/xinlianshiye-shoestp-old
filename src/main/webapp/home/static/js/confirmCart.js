...
var cart_obj={
	cart_init:{
		...
		total_price:function(){ //价格显示
			...
			console.log(CartProductPrice+'__'+DiscountPrice+'__'+shipPrice+'__'+insurancePrice+'__'+couponPrice);
...
//			console.log($(this).attr("data"));
			$.ajax({
				...
				type:'post',
				data:{"bean.pkey":$(this).attr("data")},
				dataType:'json',
				success:function(data){
					$('#lib_address>li').hide();
					$('#addressForm').slideDown(500);
					$("#setAddressId input[name='bean.pkey']").val(data.editAddress.pkey)
					$("#setAddressId input[name='bean.name']").val(data.editAddress.name)
					$("#setAddressId input[name='bean.surname']").val(data.editAddress.surname)
					$("#setAddressId input[name='bean.emailcode']").val(data.editAddress.emailcode)
					$("#setAddressId input[name='bean.country']").val(getId(data.editAddress.country))
					$("#setAddressId input[name='bean.region']").val(getId(data.editAddress.region))
					var country = $("#country_chzn .chzn-drop .chzn-results li[value='"+getId(data.editAddress.country)+"']").text();
					$("#country_chzn .chzn-single").text(country);
					var province = $("#province_chzn .chzn-drop ul[id=getProvinces"+getId(data.editAddress.country)+"] li[value='"+getId(data.editAddress.region)+"']").text();
					$("#province_chzn .chzn-single").text(province);
					$("#setAddressId input[name='bean.city']").val(data.editAddress.city)
					$("#setAddressId input[name='bean.address']").val(data.editAddress.address)
					$("#setAddressId input[name='bean.phonenumber']").val(data.editAddress.phonenumber)
				}
			})
//			user_obj.set_default_address($(this).prev().prev().val());
//			$('#lib_address>li').hide();
//			$('#addressForm').slideDown(500);
//			$('#setAddressId #usrpurphaseLinepkey').val(this.id)
//			$.post('/home/usr_UsrPurchaseLine_getAddInfo',$.param({'bean.pkey':this.id}),function(data){
//				var da = JSON.parse(data);
//			    $("#setAddressId input[name='FirstName']").val(da.editAddress.name)
//				$("#setAddressId input[name='LastName']").val(da.editAddress.surname)
//				$("#setAddressId input[name='ZipCode']").val(da.editAddress.emailcode)
//				$("#sldCountry").text(da.country)
//				$("#sldCountryHiden").val(da.countrypkey)
//				$("#sldProvince").text(da.province)
//				$("#sldProvinceHiden").val(da.provincepkey)
//				$("#setAddressId input[name='City']").val(da.editAddress.city)
//				$("#setAddressId input[name='AddressLine1']").val(da.addressa)
//				$("#setAddressId input[name='AddressLine2']").val(da.addressb)
//				$("#setAddressId input[name='PhoneNumber']").val(da.editAddress.phonenumber)
//				$.post("/home/plt_PltProvince_getAllProByCtr",{"countryPkey":da.countrypkey},function(data){
//				  var da = JSON.parse(data);
...
//	  	  })
		...
		$('#cancelAddr').click(function(){
			...
			$("#setAddressId input[name='bean.pkey']").val("0");
			...
			$("#province_chzn .chzn-single").text("");
		...
		*/
		...
		$('#orderFormSubmit').click(function(){
			...
			setTimeout(function(){
				...
				var pament=$("#_payment_method").val()
				var select = $("#PlaceOrderFrom select[name='bean.payType']").val();
				if(select == -1){
					$('body,html').animate({
					      scrollTop: $('#paymentObj').offset().top
					    }, 500);
					 global_obj.win_alert(lang_obj.cart.payment_error);
					    $obj.attr('id', 'orderFormSubmit');
					    $obj.removeAttr('disabled');
				...
				$.post('/home/odr_OdrOrder_staOrder',$("#PlaceOrderFrom").serialize(), function(data){
					//console.log(data);
//					if(data.data==1){
//						console.log(data)
//						alert("成功")
						window.top.location.href='/home/odr_OdrOrder_payOrder?orderNumber='+data.orderNum;
//					}else{
//						alert("提交订单失败")
...
//			$.post('/home/odr_OdrOrder_appConfirmPay',$.param({'bean.Currency':1})+'&'+$.param({'bean.paycontent':payContent})+'&'+$(this).serialize(), function(data){
//				if(data.success){
//					window.top.location.reload();
//				}
//			}, 'json');
