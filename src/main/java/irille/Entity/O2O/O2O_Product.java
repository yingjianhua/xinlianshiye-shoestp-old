...
public class O2O_Product extends BeanInt<O2O_Product> {
  ...
  static { // 在此可以加一些对FLD进行特殊设定的代码
    O2O_Product.T.PKEY.getFld().getTb().lockAllFlds(); // 加锁所有字段,不可以修改
  }
  ...
  private Date _upperDate;	// 日期时间  TIME
  private Date _lowerDate;	// 日期时间  TIME
  ...
  public O2O_Product init(){
    ...
    _upperDate=Env.getTranBeginTime();	// 日期时间  TIME
    _lowerDate=Env.getTranBeginTime();	// 日期时间  TIME
