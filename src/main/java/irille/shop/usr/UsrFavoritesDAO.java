...
public class UsrFavoritesDAO {
  ...
  public static Integer countByPurchase(Integer purchase) {
    return Query.SELECT(UsrFavorites.class)
        .WHERE(UsrFavorites.T.PURCHASE, "=?", purchase)
        .WHERE(T.SHOW_STATE, "=?", BeanBase.booleanToByte(true))
        .queryCount();
  }

  /** @author yingjianhua */
  public static Page<FavoriteView> pageByPurchase(Integer purchase, Integer start, Integer limit) {
    BeanQuery<UsrFavorites> q =
        Query.SELECT(UsrFavorites.class)
            .WHERE(UsrFavorites.T.PURCHASE, "=?", purchase)
            .WHERE(T.SHOW_STATE, "=?", BeanBase.booleanToByte(true));
    ...
    public void run() {
      if (bkeys == null || bkeys.trim() == "") {
        ...
        BeanBase.executeUpdate(
            " UPDATE "
                + UsrFavorites.TB.getCodeSqlTb()
                + " SET "
                + UsrFavorites.T.SHOW_STATE.getFld().getCodeSqlField()
                + " = "
                + Sys.OYn.NO.getLine().getKey()
                + " WHERE "
                + UsrFavorites.T.PKEY.getFld().getCodeSqlField()
                + " in ( "
                + bkeys
                + " ) ");
        List<UsrFavorites> favoriteList =
            BeanBase.list(
                UsrFavorites.class,
                UsrFavorites.T.PKEY.getFld().getCodeSqlField() + " in ( " + bkeys + " ) ",
                false);
        PdtProductDAO.Upd upd = new PdtProductDAO.Upd();
  ...
  public static class DelFavorite extends IduOther<DelFavorite, UsrFavorites> {
    ...
    public void run() {
      if (bkeys == null || bkeys.trim() == "") {
        ...
        BeanBase.executeUpdate(
            " DELETE FROM " + UsrFavorites.TB.getCodeSqlTb() + " WHERE pkey in ( " + bkeys + " ) ");
      }
    }
  }
