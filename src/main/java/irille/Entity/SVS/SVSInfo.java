...
@Variable(
    group = {OTempType.SVS_APPR_NOTICE},
    enumType = SVSInfo.T.class,
    clazz = SVSInfo.class,
    attributes = {
...
public class SVSInfo extends BeanInt<SVSInfo> {
  ...
  public SVSInfo init() {
    ...
    _status =
        SVSAuthenticationStatus.DEFAULT
            .getLine()
            .getKey(); // 认证结果类型 <SVSAuthenticationStatus>  BYTE
  ...
  public JSONObject gtProductionCapacity() throws JSONException {
    return getProductionCapacity() == null
        ? new JSONObject()
        : new JSONObject(getProductionCapacity());
  }
  ...
  public JSONObject gtExhibitionAttended() throws JSONException {
    return getExhibitionAttended() == null
        ? new JSONObject()
        : new JSONObject(getExhibitionAttended());
  }
