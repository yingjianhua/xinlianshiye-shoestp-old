...
public class PdtSpecDAO {
	...
	public static class Query extends IduOther<Query, PdtSpec> {
		...
		public List<PdtSpec> listByProduct(Integer pkey) {
			String where = PdtSpec.T.PRODUCT.getFld().getCodeSqlField()+"="+pkey+" AND "+ PdtSpec.T.DELETED.getFld().getCodeSqlField() +" = "+OYn.NO.getLine().getKey() ;
	...
	public static class upd extends IduUpd<upd, PdtSpec>{
		...
		public void before() {
			...
			PropertyUtils.copyProperties(spec,getB(),T.PRODUCT,T.COLOR,T.SIZE,T.PRICE,T.MARKUP,T.STORE_COUNT,T.WEIGHT,T.PICS);
			setB(spec);
		}
	}
