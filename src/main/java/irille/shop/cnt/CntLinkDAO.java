...
public class CntLinkDAO {
	...
	public static class Ins extends IduIns<Ins, CntLink>{
		...
		public void valid() {
			...
			Validate.validNumber(new Integer[]{getB().getSort(),getB().getCreatedBy()}, CntLink.T.SORT.getFld().getName(),CntLink.T.CREATED_BY.getFld().getName());
			Validate.validUniqueErr(new Object[]{CntLink.chkUniqueName(false, getB().getName())}, CntLink.T.NAME.getFld().getName());
			//验证数值不能为null
			Validate.validNotNull(new Object[]{getB().getUpdatedTime(),getB().getCreatedTime()},CntLink.T.UPDATED_TIME.getFld().getName(),ErrMsgs.notNull,CntLink.T.CREATED_TIME.getFld().getName());
			super.valid();
		}
	}
	...
	public static class Upd  extends IduUpd<Upd, CntLink>{
		...
		public void before() {
			...
			getB().setUpdatedBy(Idu.getUser().getPkey());//更新员
			PropertyUtils.copyPropertiesWithout(dbBean, getB(),CntLink.T.CREATED_BY
					,CntLink.T.CREATED_TIME);
			setB(dbBean);
