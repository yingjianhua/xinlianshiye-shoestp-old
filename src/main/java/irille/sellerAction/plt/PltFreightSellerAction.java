...
public class PltFreightSellerAction  extends SellerAction<PltFreightSeller> implements IPltFreightSellerAction {
	...
	public void listfreightlike() throws Exception{
		Page<FreightSellerlistView> freight = PltFreightSellerDAO.listfreightlike(getSupplier().getPkey(),selectcompany,getStart(), getLimit());
		write(freight);
	}
	...
	public void recovery() throws Exception{
		String sql=PltFreightSeller.T.SUPPLIER.getFld().getCodeSqlField()+"="+SellerAction.getSupplier().getPkey();
		List<PltFreightSeller> list=BeanBase.list(PltFreightSeller.class, sql,false);
		for (PltFreightSeller pfs : list) {
			BeanBase.executeUpdate("delete from "+PltFreightSellerLine.TB.getCodeSqlTb()+" where "+PltFreightSellerLine.T.MAIN+"=?",pfs.getPkey());
		}
		BeanBase.executeUpdate("delete from "+PltFreightSeller.TB.getCodeSqlTb()+" where "+PltFreightSeller.T.SUPPLIER+"=?",SellerAction.getSupplier().getPkey());
		BeanBase.executeUpdate("delete from "+PltCountryFreight.TB.getCodeSqlTb()+" where "+PltFreightSeller.T.SUPPLIER+"=?",SellerAction.getSupplier().getPkey());
		try{
