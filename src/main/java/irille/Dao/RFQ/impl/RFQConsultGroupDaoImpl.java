...
public class RFQConsultGroupDaoImpl implements RFQConsultGroupDao {
	...
	public Integer countBySupplier_PkeyName(Integer supplierId, String name) {
		return Query.SELECT(RFQConsultGroup.class).WHERE(RFQConsultGroup.T.SUPPLIER, "=?", supplierId).WHERE(RFQConsultGroup.T.NAME, "=?", name).queryCount();
	}

	@Override
	public RFQConsultGroup findByPkeySupplier_Pkey(Integer pkey, Integer supplierId) {
		return Query.SELECT(RFQConsultGroup.class).WHERE(RFQConsultGroup.T.PKEY, "=?", pkey).WHERE(RFQConsultGroup.T.SUPPLIER, "=?", supplierId).query();
	}
