...
public class SVSNewestPdtServiceImpl implements SVSNewestPdtService {
    ...
    for (int i = 0; i < listSize && i + 1 != listSize; i++) {
      ...
      if ((Byte) map.get(RFQConsultMessage.T.P2S.getFld().getCodeSqlField()) == 1) {
        if ((Byte) maps.get(RFQConsultMessage.T.P2S.getFld().getCodeSqlField()) == 0) {
          ...
          calendar.setTime(
              (Date) map.get(RFQConsultMessage.T.SEND_TIME.getFld().getCodeSqlField()));
          calendar.add(Calendar.DATE, +1);
          if (calendar.getTime().getTime()
              >= ((Date) maps.get(RFQConsultMessage.T.SEND_TIME.getFld().getCodeSqlField()))
                  .getTime()) {
            num++;
          }
        }
  ...
  public List<Integer> getSupDiamondsAndGold() {
    String sql =
        " select supplier from s_v_s_info where status = "
            + SVSAuthenticationStatus.SUCCESS.getLine().getKey()
            + " OR ( grade ="
            + SVSGradeType.GOLD.getLine().getKey()
            + " OR grade = "
            + SVSGradeType.DIAMONDS.getLine().getKey()
            + ")";
    List<Integer> supIds =
  ...
  public SQL getSVSInfos(String supIds) {
    SQL sql = new SQL();
    sql.SELECT(SVSInfo.class).FROM(SVSInfo.class).WHERE(SVSInfo.T.SUPPLIER, "in(" + supIds + ")");
    return sql;
  }
}
