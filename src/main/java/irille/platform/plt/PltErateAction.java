...
public class PltErateAction extends MgtAction<PltErate> {
  @Inject private PltErateDAO pltErateDAO;
  ...
  public void inspltErate() throws IOException {
    ...
    list.add(
        PltErateDAO.InsInit.build(
            ...
            pe.getCurName(),
            pe.getSymbol(),
            pe.getEnabled() == 1 ? true : false,
            pe.getSiteCur() == 1 ? true : false,
            pe.getRate(),
            pe.getSupCur() == 1 ? true : false,
            pe.getNowrate(),
            lu.get_user().getPkey()));
    ins.setList(list);
    ins.commit();
  ...
  public void upload() throws Exception {
    if (getLoginSys() == null) {
      ...
      if (width != 42 && height != 30) throw LOG.err("wrongSize", "仅限上传42*30PX的图片!");
  ...
  /**
   * 获取已开启的所有货币 @Date 2019/01/11 16:21
   *
   * @author zjl
   */
