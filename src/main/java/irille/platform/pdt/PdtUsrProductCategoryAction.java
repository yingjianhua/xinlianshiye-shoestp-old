...
public class PdtUsrProductCategoryAction extends ActionBase<UsrProductCategory> {
  ...
  @Getter @Setter private Integer category; // 查询所属分
  @Getter @Setter private Integer supplier; // 查询供应商
  @Getter @Setter private Integer enabled; // 查询是否启用
  @Getter @Setter private String seoKeyword; // 查询关键词

  /**
   * 查询所有分类 *@anthor kouhanyang
   *
   * @throws java.io.IOException
   */
  public void getUsrProductCategory() throws IOException {
    write(
        UsrProductCategoryDAO.getUsrProductCategory(
            getStart(), getLimit(), name, category, supplier, enabled, seoKeyword));
  }
}
