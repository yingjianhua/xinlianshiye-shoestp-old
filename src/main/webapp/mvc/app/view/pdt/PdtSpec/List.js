Ext.define('mvc.view.pdt.PdtSpec.List',{
...
this.columns = [{text : '产品颜色关联图片',width : 100,dataIndex : 'bean.pics',sortable : true,renderer : mvc.Tools.imgRenderer(80)}
...
onIns : function(){
		var win = Ext.create('mvc.view.pdt.PdtSpec.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.pdt.PdtSpec', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/pdt_PdtSpec_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/pdt_PdtSpec_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.pdt.PdtSpec.WinSearch',{
