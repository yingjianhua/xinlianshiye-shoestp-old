...
public class O2OMapServerImp implements IO2OMapServer {
  ...
  @Inject private O2OMapDao o2OMapDao;
  ...
  public JSONObject load(String address) {
    ...
    try {
      ...
      String result = null;
      if (AppConfig.dev) {
        result = sendHttpsUtils.get(builder.build().toString(), "127.0.0.1", 1080);
      } else {
  ...
  public void del(Integer id) {
    if (null == id) throw new WebMessageException(ReturnCode.failure, "请选择地址");
    O2O_Map map = BeanBase.chk(O2O_Map.class, id);
    if (null == map) throw new WebMessageException(ReturnCode.failure, "地址不存在");
