		...
		try {
			...
			if (invocation.getAction() instanceof HomeAction) {
				...
				if (method.getReturnType().equals(String.class)) {
					...
					irille.Filter.svr.SessionMsg sessionmsg = (irille.Filter.svr.SessionMsg) session
							.getAttribute(irille.Filter.svr.SessionMsg.session_key);
			...
			try {
				...
				irille.Filter.svr.SessionMsg sessionmsg = (irille.Filter.svr.SessionMsg) session
						.getAttribute(irille.Filter.svr.SessionMsg.session_key);
				if (sessionmsg != null)
					LgAccessDAO.add(sessionmsg.getUser(), ServletActionContext.getRequest(), date2 - date1, isSuccess,
							e2);
