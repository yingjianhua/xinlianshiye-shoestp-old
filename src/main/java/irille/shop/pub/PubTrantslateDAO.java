...
public class PubTrantslateDAO {
    public static class Select extends IduOther<Select, PubTrantslate> {
        public TranslateBean getTransLatesByHashCode(String sourceText, String  targetLanguage) {
            ...
            sql.eqAutoAnd(PubTrantslate.T.HASHCODE, sourceText.hashCode()).eqAutoAnd(PubTrantslate.T.TARGET, targetLanguage);
            List<PubTrantslate> list = PubTrantslate.list(PubTrantslate.class, sql.toWhereString(), false, sql.getParms());
            if (list.size() > 0) {
