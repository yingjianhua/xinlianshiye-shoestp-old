...
public class PdtProductServiceImp implements IPdtProductService {
	...
	@Override
	public Map getProductListByCategory(IduPage page, String[] flds, boolean order, int category, String spec,
			String onlyFld, String keyword, Integer type) {
		PdtProductView pdtProductView = new PdtProductView();
		pdtProductView.setPage(page).setFlds(flds).setOrder(order).setCategory(category).setSpec(spec)
				.setKeyword(keyword);
		if (type == null)
			type = 0;
		switch (type) {
	...
	public List<PdtNewPdtInfo> getRandomPdt(Integer limit, int cat, UsrPurchase purchase) {
		...
		while (true) {
			...
			if (object instanceof PdtProduct) {
				...
				newPdtInfo.setRewrite(SEOUtils.getPdtProductTitle(pdtProduct.getPkey(), pdtProduct.getName()));
				...
				newPdtInfo.setPrice(GetValue.get(pdtProduct, "cur_price", BigDecimal.class, BigDecimal.ZERO));
				...
				newPdtInfo
						.setRewrite(SEOUtils.getPdtProductTitle(newPdtInfo.getId().intValue(), newPdtInfo.getTitle()));
			}
	...
	// TODO 变成DTO返回
	@Override
	public Map getProductListByCategoryV2(IduPage iduPage, String[] orderfld, boolean order, int cated, String spec,
			String onlyFld, String keyword, Integer type) {
		PdtProductView pdtProductView = new PdtProductView();
		pdtProductView.setPage(iduPage).setFlds(orderfld).setOrder(order).setCategory(cated).setSpec(spec)
				.setKeyword(keyword);
		pdtProductView.setProductType(Pdt.OProductType.GENERAL);
		if (onlyFld != null) {
			try {
