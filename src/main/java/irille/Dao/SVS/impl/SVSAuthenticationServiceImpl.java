...
public class SVSAuthenticationServiceImpl implements SVSAuthenticationService {
  ...
  private SVSAuthenticationView CreateView(SVSInfo info) throws Exception {
    ...
    view.setCapacity(
        om.readValue(info.getProductionCapacity(), SVSInfoView.productionCapacity.class));
    view.setExhibition(
        om.readValue(info.getExhibitionAttended(), SVSInfoView.exhibitionAttended.class));
  ...
  public void Authentication(
      UsrSupplier supplier, Integer status, Integer grade, String reasons, Integer pkey)
      throws Exception {
    SVSInfo svs = SVSAuthenticationDao.querySVS(pkey);
    int score =
        GetBaseScoreUtils.getBaseScore(
            svs.getResearch(),
            svs.getProductionCapacity(),
            svs.getRealFactory(),
            svs.getProductQuality(),
            svs.getForeignTradeTeam(),
            svs.getExhibitionAttended(),
            svs.getPartner());
    if (score < 30) {
