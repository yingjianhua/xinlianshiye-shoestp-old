      ...
      if (getLines() != null)
        for (int i = 0; i < s.size(); i++) {
          ...
          if (s.get(i).getAggravatePrice().compareTo(BigDecimal.ZERO) < 0) {
            throw LOG.err(
                ErrMsgs.lowPriceErr, PltFreightLine.T.AGGRAVATE_PRICE.getFld().getName(), 0);
          }
        }
    }
  }
  ...
  public static class Upd extends IduUpdLines<Upd, PltFreight, PltFreightLine> {
    ...
    public void before() {
      ...
      PropertyUtils.copyPropertiesWithout(
          ...
          getB(),
          PltFreight.T.PKEY,
          PltFreight.T.ENABLED,
          PltFreight.T.CREATED_BY,
          PltFreight.T.CREATED_TIME,
          PltFreight.T.USER_SYS);
      setB(dbBean);
