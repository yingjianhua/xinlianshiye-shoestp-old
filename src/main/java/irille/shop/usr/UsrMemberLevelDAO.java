...
public class UsrMemberLevelDAO {
  ...
  public static class Upd extends IduUpd<Upd, UsrMemberLevel> {
    ...
    public void before() {
      ...
      PropertyUtils.copyPropertiesWithout(
          model, getB(), UsrMemberLevel.T.CREATED_BY, UsrMemberLevel.T.CREATED_TIME);
      setB(model);
    }
  ...
  private static void toValid(UsrMemberLevel bean) {
    ...
    new ValidNumber(bean)
        .validBigDecimalRange(BigDecimal.ZERO, BigDecimal.valueOf(100), UsrMemberLevel.T.DISCOUNT);
  }
  ...
  public static List<UsrmemberlistView> listnumer() {
    ...
    List<UsrmemberlistView> list =
        Query.sql(sql).queryMaps().stream()
            .map(
                bean ->
                    new UsrmemberlistView() {
                      {
                        setId((Integer) bean.get(UsrMemberLevel.T.PKEY.getFld().getCodeSqlField()));
                        setName(
                            (String) bean.get(UsrMemberLevel.T.NAME.getFld().getCodeSqlField()));
                        setIcon(
                            (String) bean.get(UsrMemberLevel.T.ICON.getFld().getCodeSqlField()));
                        if ((byte) bean.get(UsrMemberLevel.T.ENABLED.getFld().getCodeSqlField())
                            == 1) {
                          setEnabled(true);
                        } else {
                          setEnabled(false);
                        ...
                        setDiscount(
                            (BigDecimal)
                                bean.get(UsrMemberLevel.T.DISCOUNT.getFld().getCodeSqlField()));
                        setLevelUp(
                            (BigDecimal)
                                bean.get(UsrMemberLevel.T.LEVEL_UP.getFld().getCodeSqlField()));
                        ...
                        setCreatedby(
                            BeanBase.load(
                                    ...
                                    (Integer)
                                        bean.get(
                                            UsrMemberLevel.T.CREATED_BY.getFld().getCodeSqlField()))
                                .getName());
                        setCreatetime(
                            (Date)
                                bean.get(UsrMemberLevel.T.CREATED_TIME.getFld().getCodeSqlField()));
                        setUpdby(
                            BeanBase.load(
                                    SysUser.class,
                                    (Integer)
                                        bean.get(
                                            UsrMemberLevel.T.UPDATED_BY.getFld().getCodeSqlField()))
                                .getName());
                        setUpdtime(
                            (Date)
                                bean.get(UsrMemberLevel.T.UPDATED_TIME.getFld().getCodeSqlField()));
                      }
                    })
