...
public class PMMessageServiceImp implements IPMMessageService{
	...
	private void sendPm(OTempType type,UsrSupplier supplier,UsrPurchase purchase,String title, String content) {
	...
	private void sendEmail(OTempType type,UsrSupplier supplier,UsrPurchase purchase,String title,String content) {
		PMMessage message = initMessage(type,supplier,purchase,title, content);
		...
		if(message.getRcvrType().equals(ORCVRType.PURCHASE.getLine().getKey()) && !message.getRcvr().equals(-1)) {
			mail.setReceiver(purchase.getEmail());
		}else if(message.getRcvrType().equals(ORCVRType.SUPPLIER.getLine().getKey()) && !message.getRcvr().equals(-1)){
	...
	private PMMessage initMessage(OTempType type,UsrSupplier supplier,UsrPurchase purchase,String title, String content) {
		...
		message.setType(type.getLine().getKey());
		if(type.getRcvrType().getLine().getKey() == ORCVRType.PURCHASE.getLine().getKey() && purchase != null) {
			message.setRcvr(purchase.getPkey());
		}else if(type.getRcvrType().getLine().getKey() == ORCVRType.SUPPLIER.getLine().getKey() && supplier != null){
