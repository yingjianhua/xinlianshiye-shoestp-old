...
public class UsrPurchaseLineAction extends HomeAction<UsrPurchaseLine> {
	...
	public String addmanagement() {
		...
		setShippingAddr(AddressView.buildByPurchaseLine(BeanBase.list(UsrPurchaseLine.class,T.PURCHASE.getFld().getCodeSqlField() + " =? AND " + T.ADDRSSTYPE.getFld().getCodeSqlField() + "=? ORDER BY " + T.ISDEFAULT + " DESC " , false,getPurchase().getPkey(), Usr.OAddress.COMMON.getLine().getKey())));
	...
	public String addressform() {
		UsrPurchaseLine.TB.getCode();
		setShippingAddr(AddressView.buildByPurchaseLine(BeanBase.list(UsrPurchaseLine.class,T.PURCHASE.getFld().getCodeSqlField() + " =? AND " + T.ADDRSSTYPE.getFld().getCodeSqlField() + "=?" , false,getPurchase().getPkey(), Usr.OAddress.COMMON.getLine().getKey())));
		setBillAddr(AddressView.buildByPurchaseLine(BeanBase.list(UsrPurchaseLine.class, T.PURCHASE.getFld().getCodeSqlField() + " =? AND " + T.ADDRSSTYPE.getFld().getCodeSqlField() + "=?" , false,getPurchase().getPkey(), Usr.OAddress.BILLED.getLine().getKey())));
		setCountries(translateUtil.getAutoTranslateList(PltCountry.list(PltCountry.class, "1=1", false), HomeAction.curLanguage()));
	...
	public void initializing() throws JSONException, IOException{
		List<PltCountry> pc= translateUtil.getAutoTranslateList(PltCountry.list(PltCountry.class, "1=1", false), HomeAction.curLanguage());
		List<PltProvince> pr= translateUtil.getAutoTranslateList(Idu.getLines(PltProvince.T.MAIN, pc.get(0).getPkey()), HomeAction.curLanguage());
		JSONObject json = new JSONObject();
	...
	public void listProvinces() throws Exception {
		List<PltProvince> list = translateUtil.getAutoTranslateList(Idu.getLines(PltProvince.T.MAIN, getCountryId()), HomeAction.curLanguage());
		JSONObject json = new JSONObject();
