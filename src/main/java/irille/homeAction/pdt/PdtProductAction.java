...
//        新版API
  ...
  public String gtProductsInfo() throws Exception {
    ...
    if (!isMobile()) {
      ProductInfoView infoView = pdtpageSelect
          .getProductsById(Integer.valueOf(getId().toString()), Sys.OYn.YES, Pdt.OState.ON,
              getPurchase() != null ? getPurchase().getPkey() : -1, HomeAction.curCurrency());
      if (infoView == null) {
        throw LOG.err("not exists", "产品id[{0}]不存在", getId());
      }
      seoView = new SEOView();
    ...
    for (PdtProduct line : list) {
      ...
      for (String sizepk : line.getSizeAttr().split(",")) {
        ...
        for (PdtSize pdtSize : sizeList) {
          ...
          sizeJson
              .put(PdtSize.T.NAME.getFld().getCode(), pdtSize.getName(HomeAction.curLanguage()));
      ...
      for (String colorPk : line.getColorAttr().split(",")) {
        ...
        for (PdtColor pdtColor : colorList) {
          ...
          colorJson
              .put(PdtColor.T.NAME.getFld().getCode(), pdtColor.getName(HomeAction.curLanguage()));
  ...
  public String viewComments() {
    ...
    pdtCommentViewPageView
        .setPdtCommentViews(commentPageSelect.getCommentListByProId(null, getId()));
  ...
  public String writeComment() throws IOException {
    ...
    if (getPurchase() == null) {
      ...
      pdtCommentViewPageView
          .setCommentPermissionMessage(I18NUtil.getBundle("WriteComment_Nocomment"));
    } else {
      ...
      if (commentPageSelect.hasComment(getId(), getPurchase().getPkey())) {
        pdtCommentViewPageView.setCommentPermission(false);
        pdtCommentViewPageView
            .setCommentPermissionMessage(I18NUtil.getBundle("WriteComment_Commented"));
  ...
  public void addcomment() throws IOException {
    ...
    PdtCommentSatisFactionView pdtCommentSatisFactionView = objectMapper
        .readValue(getSatisfaction(), PdtCommentSatisFactionView.class);
    pdtComment.setProductSatisfaction((byte) pdtCommentSatisFactionView.getOverall_rating());
    objectMapper
        .setFilterProvider(new SimpleFilterProvider().addFilter("PdtCommentSatisFactionView",
  ...
  public void gtSupPro() throws Exception {
    String where =
        PdtProduct.T.IS_VERIFY.getFld().getCodeSqlField() + " = " + Sys.OYn.YES.getLine().getKey()
            + " AND " + PdtProduct.T.SUPPLIER.getFld().getCodeSqlField() + " = " + getId() + " AND "
            + PdtProduct.T.STATE.getFld().getCodeSqlField() + " = " + Pdt.OState.ON.getLine()
            .getKey() + " AND " + PdtProduct.T.PRODUCT_TYPE.getFld().getCodeSqlField() + " = "
    ...
    List<PdtProduct> pdtList = translateUtil
        .getAutoTranslateList(BeanBase.list(PdtProduct.class, false, where, idx, getLimit()),
  ...
  public void getProductBySup() throws Exception {
    ...
    Map map = PdtProductDAO.Select
        .getProductBySup(curLanguage(), getPkey(), getStart(), getLimit(), getCated(),
            getRankingBasis(), getBasis());
