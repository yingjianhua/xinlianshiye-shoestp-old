Ext.define('mvc.view.usr.UsrSupplierCategory.List',{
extend : 'Ext.grid.Panel',
oldId : 'btn_UsrSupplierCategory',
lock : true,
...
this.columns = [{text : '名称',width : 100,dataIndex : 'bean.name',sortable : true}
	,{text : '网站显示名称',width : 100,dataIndex : 'bean.showName',sortable : true,renderer : mvc.Tools.JsonRenderer()}
	,{text : '上级分类',width : 100,dataIndex : 'bean.categoryUp',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'usr',mn : 'view.usr.UsrSupplierCategory.List'}
	...
	,{text : '建档时间',width : 140,dataIndex : 'bean.createTime',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s')}
		...
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.usr.UsrSupplierCategory'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.usr.UsrSupplierCategory.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.usr.UsrSupplierCategory', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/usr_UsrSupplierCategory_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/usr_UsrSupplierCategory_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.usr.UsrSupplierCategory.WinSearch',{
