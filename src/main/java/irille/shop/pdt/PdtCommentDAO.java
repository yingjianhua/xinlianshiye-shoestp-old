...
public class PdtCommentDAO {
  ...
  public static class pageSelect extends IduOther<pageSelect, PdtComment> {
    ...
    public boolean hasComment(Integer id, Integer userid) {
      ...
      return sql.castLong(BeanBase.queryOneRowIsNull(sql.buildSql(), sql.getParms())) > 0
          ? true
          : false;
    }
  }
  ...
  public static class Upd extends IduUpd<Upd, PdtComment> {
    ...
    public void before() {
      ...
      PropertyUtils.copyPropertiesWithout(
          model, getB(), PdtComment.T.PRODUCT, PdtComment.T.USERS_PKEY);
      setB(model);
    }
    ...
    public int replay(PdtComment pdtComment, int supId) {
      ...
      if (sql.castLong(
              BeanBase.queryOneRow(
                  sql.from(PdtProduct.T.SUPPLIER).eq(PdtProduct.T.SUPPLIER).buildCountSql(), 1))
          > 0) {
        ...
        sql.clean()
            .update(PdtComment.T.REPLY, PdtComment.T.ROW_VERSION)
            .eqAutoAnd(PdtComment.T.ROW_VERSION)
            .eqAutoAnd(PdtComment.T.PKEY)
            .and()
            .isNull(PdtComment.T.REPLY);

        return BeanBase.executeUpdate(
            sql.buildSql(),
            sql.getParms(
                Arrays.asList(
                    pdtComment.getReply(), row_version + 1, row_version, pdtComment.getPkey())));
      }
      return -1;
    }
    ...
    public int unuseful(int pkey, int type) {
      if (type > -1 && type < 2) {
        ...
        int row_version = (int) map.get(PdtComment.T.ROW_VERSION.getFld().getCodeSqlField());
        sql.clean()
            .update(iEnumFld, PdtComment.T.ROW_VERSION)
            .eqAutoAnd(iEnumFld)
            .eqAutoAnd(PdtComment.T.ROW_VERSION)
            .eqAutoAnd(PdtComment.T.PKEY);
