...
public class PdtSizeDAO {
    ...
    public static List<PdtSize> listSummary(Language lang, Integer type) {
        ...
        SQL sql = new I18NSQL(lang) {{
            ...
            FROM(PdtSize.class).WHERE(T.DELETED, "=?", OYn.NO).WHERE(T.SUPPLIER, " is null ").WHERE(T.TYPEVER, " =? ",Pdt.OVer.ELSE.getLine().getKey());
        }};
        List<PdtSize> sysSize = Query.sql(sql).queryList(PdtSize.class);
        if (type == 1) {
            SQL supSql = new I18NSQL(lang) {{
                SELECT(T.PKEY, T.NAME, T.CREATE_BY, T.ROW_VERSION);
                FROM(PdtSize.class).WHERE(T.DELETED, "=?", OYn.NO).WHERE(T.SUPPLIER, " =? ", SellerAction.getSupplier().getPkey()).WHERE(T.TYPEVER, " =? ",Pdt.OVer.ELSE.getLine().getKey());
            }};
    ...
    public static List<PdtSize> newListSummary(Language lang, Integer type) {
    	...
    	SQL sql = new I18NSQL(lang) {{
            ...
            FROM(PdtSize.class).WHERE(T.DELETED, "=?", OYn.NO).WHERE(T.SUPPLIER, " is null ").WHERE(T.TYPEVER, " =? ",Pdt.OVer.NEW_1.getLine().getKey());
        }};
        List<PdtSize> sysSize = Query.sql(sql).queryList(PdtSize.class);
        if (type == 1) {
            SQL supSql = new I18NSQL(lang) {{
                SELECT(T.PKEY, T.NAME, T.CREATE_BY, T.ROW_VERSION,T.SUPPLIER,T.TYPE);
                FROM(PdtSize.class).WHERE(T.DELETED, "=?", OYn.NO).WHERE(T.SUPPLIER, " =? ", SellerAction.getSupplier().getPkey()).WHERE(T.TYPEVER, " =? ",Pdt.OVer.NEW_1.getLine().getKey());
            }};
            List<PdtSize> supSize = Query.sql(supSql).queryList(PdtSize.class);
            sysSize.addAll(supSize);
        }
