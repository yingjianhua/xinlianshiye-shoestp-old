...
public class UsrConsultRelationDAO {
  ...
  public static List<UsrConsultRelation> listByConsult(Integer pkey) {
    return BeanBase.list(
        UsrConsultRelation.class, UsrConsultRelation.T.CONSULT + "=?", false, pkey);
  }
   ...
   * @author yingjianhua
   */
  public static Page<ConsultView> pagePrivate(
      int start, int limit, int supplier, String countryName, String title, Language lang)
      throws JSONException {
    BeanQuery<UsrConsultRelation> q = Query.SELECT(UsrConsultRelation.class);
  ...
  public static int countByConsult(Integer pkey) {
    String sql =
        "SELECT count(*) FROM "
            + UsrConsultRelation.TB.getCodeSqlTb()
            + " WHERE "
            + UsrConsultRelation.T.CONSULT
            + "=?";
  ...
  public static boolean isOwner(int consult, int supplier) {
    return UsrConsultRelation.chkUniqueConsult_supplier(false, consult, supplier) == null
        ? false
        : true;
  }
   ...
   * @author yingjianhua
   */
  public static ConsultView load(Integer consult, Integer supplier, Language lang)
      throws JSONException {
    UsrConsultRelation relation =
        UsrConsultRelation.chkUniqueConsult_supplier(false, consult, supplier);
    if (relation == null) {
      return null;
    }
  ...
  public static Integer countPurchaseNewMsg(Integer pkey) {
    return Query.SELECT(UsrConsultRelation.class)
        .WHERE(T.S_TO_P_NEW_MSG, "=?", BeanBase.booleanToByte(true))
        .queryCount();
  }
}
