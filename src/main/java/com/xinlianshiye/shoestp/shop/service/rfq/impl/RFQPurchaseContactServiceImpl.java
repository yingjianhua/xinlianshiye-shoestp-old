...
public class RFQPurchaseContactServiceImpl implements RFQPurchaseContactService {
  ...
  public Page<RFQPurchaseContactView> page(
    ...
    query.LEFT_JOIN(
        RFQConsultRelation.class, RFQPurchaseContact.T.SUPPLIER, RFQConsultRelation.T.SUPPLIER_ID);
        ...
        query.queryMaps().stream()
            .map(
                map -> {
                  ...
                  contact.setPkey(
                      GetValue.get(map, RFQPurchaseContact.T.PKEY, Integer.class, null));
                  contact.setCreatedDate(
                      GetValue.get(map, RFQPurchaseContact.T.CREATED_TIME, Date.class, null));
                  ...
                  contact.setRelation(
                      listConsultRelation(
                          purchase.getPkey(),
                          supplier.getPkey(),
                          (keyword == null || supplier.getName().contains(keyword))
                              ? null
                              : keyword));
                  return contact;
                })
   ...
   * @author Jianhua Ying
   */
  public List<RFQConsultRelationView> listConsultRelation(
      Integer purchasePkey, Integer supplierPkey, String keyword) {
  ...
  public List<RFQPurchaseContactGroupView> listGroup(UsrPurchase purchase) {
    List<RFQPurchaseContactGroup> list =
        Query.selectFrom(RFQPurchaseContactGroup.class)
            .WHERE(RFQPurchaseContactGroup.T.PURCHASE, "=?", purchase.getPkey())
            .queryList();
    ...
    all.setName("all");
    all.setCount(0);
    List<RFQPurchaseContactGroupView> result =
        list.stream()
            .map(
                map -> {
