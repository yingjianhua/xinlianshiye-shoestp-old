Ext.define('mvc.view.pdt.PdtAsk.List',{
...
this.columns = [{text : '标题',width : 100,dataIndex : 'bean.title',sortable : true}
	...
	,{text : '图片',width : 100,dataIndex : 'bean.photo',sortable : true,renderer : mvc.Tools.imgRenderer()}
	...
	,{text : '日期时间',width : 140,dataIndex : 'bean.time',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s')}
	...
	,{text : '内容',width : 100,dataIndex : 'bean.content',sortable : true}
		...
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.pdt.PdtAsk'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.pdt.PdtAsk.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.pdt.PdtAsk', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/pdt_PdtAsk_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/pdt_PdtAsk_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
		...
		if (selection) {
			...
			Ext.MessageBox.confirm(msg_confirm_title, '您确认要启用所选的记录吗?',
					function(btn) {
						...
						Ext.Ajax.request({
							url : base_path + '/pdt_PdtAsk_doEnabled?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
							success : function(response, options) {
								var result = Ext.decode(response.responseText);
								if (result.success) {
									me.getStore().reload();
		...
		if (selection) {
			...
			Ext.MessageBox.confirm(msg_confirm_title, '您确认要停用所选的记录吗?',
					function(btn) {
						...
						Ext.Ajax.request({
							url : base_path + '/pdt_PdtAsk_unEnabled?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
							success : function(response, options) {
								var result = Ext.decode(response.responseText);
								if (result.success) {
									me.getStore().reload();
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.pdt.PdtAsk.WinSearch',{
