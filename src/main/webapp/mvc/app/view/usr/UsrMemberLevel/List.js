Ext.define('mvc.view.usr.UsrMemberLevel.List',{
extend : 'Ext.grid.Panel',
oldId : 'btn_UsrMemberLevel',
lock : true,
...
this.columns = [{text : '名称',width : 100,dataIndex : 'bean.name',sortable : true}
	,{text : '图片',width : 100,dataIndex : 'bean.icon',sortable : true,renderer : mvc.Tools.imgRenderer(16)}
	,{text : '启用标志',width : 75,dataIndex : 'bean.enabled',sortable : true,renderer : mvc.Tools.optRenderer('sys','Sys','OEnabled')}
	,{text : '会员折扣',width : 100,dataIndex : 'bean.discount',sortable : true,renderer : mvc.Tools.numberRenderer(),align : 'right'}
	,{text : '升级条件',width : 100,dataIndex : 'bean.levelUp',sortable : true,renderer : mvc.Tools.numberRenderer(),align : 'right'}
	...
	,{text : '更新时间',width : 140,dataIndex : 'bean.updatedTime',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s')}
		...
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.usr.UsrMemberLevel'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.usr.UsrMemberLevel.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.usr.UsrMemberLevel', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/usr_UsrMemberLevel_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/usr_UsrMemberLevel_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
		...
		if (selection) {
			...
			Ext.MessageBox.confirm(msg_confirm_title, '您确认要启用所选的记录吗?',
					function(btn) {
						...
						Ext.Ajax.request({
							url : base_path + '/usr_UsrMemberLevel_doEnabled?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
							success : function(response, options) {
								var result = Ext.decode(response.responseText);
								if (result.success) {
									me.getStore().reload();
		...
		if (selection) {
			...
			Ext.MessageBox.confirm(msg_confirm_title, '您确认要停用所选的记录吗?',
					function(btn) {
						...
						Ext.Ajax.request({
							url : base_path + '/usr_UsrMemberLevel_unEnabled?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
							success : function(response, options) {
								var result = Ext.decode(response.responseText);
								if (result.success) {
									me.getStore().reload();
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.usr.UsrMemberLevel.WinSearch',{
