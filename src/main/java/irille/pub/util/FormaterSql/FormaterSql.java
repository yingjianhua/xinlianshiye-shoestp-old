...
public class FormaterSql {
    ...
    /**
     * 参数 目标表，目标表中字段 本表关联字段
     *
     * @Description: 参数 目标表，目标表中字段 本表关联字段
     ...
     *
     * @author lijie@shoestp.cn
     * @Description: *基类中已有count()和countWhere() 方法不需要另外查询
     * @date 2018/8/1 15:01
     */
    ...
    public String buildSql() {
        ...
        do {
            ...
            if (type == UPDATE) {
                sqlStringBuffer.append(UPDATE.toString())
                        .append(formaterSqlBean.getTableName()).append(" Set ");
                int length = sqlStringBuffer.length();
                for (FormaterSqlFldBean s : formaterSqlBean.getFldlist()) {
                    if (sqlStringBuffer.length() > length) {
                        sqlStringBuffer.append(",");
        ...
        if (formaterSqlBean.getShowItem() != 0) {
            sqlStringBuffer.append(" limit ").append(formaterSqlBean.getOffset()).append(",").append(formaterSqlBean.getShowItem());
        }
     ...
     */
    public FormaterSql eqAutoAnd(IEnumFld fld, Number serializable, Function<Number, Boolean> function) {
    ...
    /***
     * 临时所用，转换为List Map 方便jsp迭代
     * 放入Dto更优,但Dto赋值模块 待测试
     * 但是不应写在这里，不符合单一原则。
     * @return
     */
        ...
        for (Object[] obj : objects) {
            if (obj.length == fldlist.size()) {
                ...
                for (int i = 0; i < obj.length; i++) {
                    ...
                    setvalue = fldBean.isMultiLanguageFld() ? translateUtil.getLanguage(obj[i], language) : obj[i];
    ...
    public Map castMap(Object[] obj) {
        ...
        for (int i = 0; i < obj.length; i++) {
            if (obj[i] != null) {
                FormaterSqlFldBean fldBean = fldlist.get(i);
                Object setvalue = null;
                setvalue = fldBean.isMultiLanguageFld() ? translateUtil.getLanguage(obj[i], HomeAction.curLanguage()) : obj[i];
                map.put(fldBean.getAlias() == null ? fldBean.getFldName() : fldBean.getAlias(), setvalue);
            }
        }
        return map;
    ...
    public Map castMapAddFld(Object[] obj) {
        Map map = castMap(obj);
        map.put("rewrite", SEOUtils.getPdtProductTitle(Integer.parseInt(String.valueOf(map.get("pkey"))), String.valueOf(obj[1])));
        return map;
    }
