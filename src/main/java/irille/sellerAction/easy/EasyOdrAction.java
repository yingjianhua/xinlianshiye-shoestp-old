...
public class EasyOdrAction extends SellerAction<EasyOdr> implements IEasyOdrAction {
  ...
  @Getter @Setter private OrderSearchView search;
  @Inject private EasyOdrDao easyOdrDao;

  @Getter @Setter private Integer eastOdrId;
  @Getter @Setter private String eastOdrIds;
  @Getter @Setter private String path;

  public void newOrderlist() throws IOException {
    Page<EasyodrView> page =
        easyOdrDao.sellerlist(getStart(), getLimit(), search, getSupplier().getPkey());
    write(page);
  }
  ...
  public void oneexport() throws Exception {
    sendOutPuiStreanm(
        easyOdrDao.oneexport(eastOdrId, getSupplier().getPkey()), new Date() + ".xlsx");
  }
  ...
  public void manyexports() {
    try {
      sendOutPuiStreanm(
          easyOdrDao.manyexport(eastOdrIds, getSupplier().getPkey()), new Date() + ".zip");
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
