...
public class Scanner {
	 ...
	 * @return
	 */
	public static <T extends Annotation> List<Class<?>> findClassByAnnotationSkipInitialize(Class<T> annotationClass,String rootPackage) {
	...
	public static <T extends Annotation> List<Class<?>> findClassByAnnotation(Class<T> annotationClass, String rootPackage,boolean initialize,  boolean noCache) {
		...
		if(noCache||TYPE_ANNOTATION_MAPS.containsKey(annotationClass)) {
			...
			.map(className -> {
				try {
					if(!initialize) {
						Class<?> cla = Class.forName(className,false,Scanner.class.getClassLoader());
						if(className.indexOf("$") != -1 && className.substring(className.lastIndexOf("$")+1).equals("T")) {
							String pClassName = className.substring(0, className.lastIndexOf("$"));
