...
//		countRequest();
     ...
     * ——————————————————————分割线(新平台)————————————————————————————
     */
    public static Page getAccessList(Integer start, Integer limit, String user, String loginName, String requestUrl) {
        if (start == null) {
            start = 0;
        }
        if (limit == null) {
        ...
        List<LgAccessView> list = Query.sql(sql.LIMIT(start, limit)).queryMaps().stream().map(o -> new LgAccessView() {{
            setId((Integer) o.get(LgAccess.T.PKEY.getFld().getCodeSqlField()));
            setUser((String) o.get(LgAccess.T.USER.getFld().getCodeSqlField()));
            setLoginName((String) o.get(LgAccess.T.LOGINNAME.getFld().getCodeSqlField()));
            setRemoteAddr((String) o.get(LgAccess.T.REMOTEADDR.getFld().getCodeSqlField()));
            setRequestUrl((String) o.get(LgAccess.T.REQUESTURL.getFld().getCodeSqlField()));
            setParams((String) o.get(LgAccess.T.PARAMS.getFld().getCodeSqlField()));
            setSuccess(Byte.valueOf(String.valueOf(o.get(LgAccess.T.SUCCESS.getFld().getCodeSqlField()))));
            setRemark((String) o.get(LgAccess.T.REMARK.getFld().getCodeSqlField()));
            setProcessTime((Long) o.get(LgAccess.T.PROCESSTIME.getFld().getCodeSqlField()));
            setRequestTime((Date) o.get(LgAccess.T.REQUESTTIME.getFld().getCodeSqlField()));
    ...
    public static List<statisticsView> getStatisticsList(Integer id) {
        SQL sql = new SQL();
        if (id == 1) {
            sql.SELECT("requesturl, AVG(processtime) as processtime, count(1) as count from lg_access GROUP BY requesturl ORDER BY processtime desc");
        } else {
            sql.SELECT("requesturl, AVG(processtime) as processtime, count(1) as count from lg_access GROUP BY requesturl ORDER BY count desc");
        }
        List<statisticsView> list = Query.sql(sql).queryMaps().stream().map(o -> new statisticsView() {{
