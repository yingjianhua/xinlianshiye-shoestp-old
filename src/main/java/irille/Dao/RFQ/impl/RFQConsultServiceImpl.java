...
public class RFQConsultServiceImpl implements RFQConsultService {
  ...
  public RFQConsultView detail(RFQConsultView condition) {
    ...
    if (id == null) throw new WebMessageException(ReturnCode.valid_notnull, "请选择询盘");
  ...
  public void approve(RFQConsultView view, Boolean verify) {
    ...
    if (consult.gtStatus().equals(RFQConsultStatus.ready)
        && consult.gtVerifyStatus().equals(RFQConsultVerifyStatus.UNAUDITED)) {
      if (verify) {
        // 审核通过
  ...
  public void delete(RFQConsultView view) {
    ...
    if (consult.gtStatus() != RFQConsultStatus.close
        && consult.gtStatus() != RFQConsultStatus.complete) {
  ...
  public void recommend(RFQConsultView view) {
    ...
    consult.setRecommend(
        (view.getRecommend() == null || view.getRecommend() == 0)
            ? RFQConsultRecommend.RECOMMENDED.getLine().getKey()
