...
public class RFQConsultRelationDaoImpl implements RFQConsultRelationDao {
    ...
    return query.queryMaps().stream()
        .map(
            map -> {
              ...
              view.setPurchaseId(
                  (Integer) map.get(RFQConsultRelation.T.PURCHASE_ID.getFld().getCodeSqlField()));
              //
              //	view.setFavorite((Byte)map.get(RFQConsultRelation.T.FAVORITE.getFld().getCodeSqlField()));
              view.setRowVersion(
                  (Short) map.get(RFQConsultRelation.T.ROW_VERSION.getFld().getCodeSqlField()));
  ...
  @Override
  public List<RFQConsultRelation> findAllByConsult_PkeySupplier_Pkey(
    ...
    return Query.SELECT(RFQConsultRelation.class)
        ...
        .WHERE(
            RFQConsultRelation.T.CONSULT,
            "in ("
                + Stream.of(consultPkeys.split(",")).map(i -> "?").collect(Collectors.joining(","))
                + ")",
            Stream.of(consultPkeys.split(",")).map(Integer::new).toArray(Serializable[]::new))
        .queryList();
  }
  ...
  @Override
  public RFQConsultRelation findByConsult_PkeySupplier_Pkey(
