...
public class RFQConsultGroupRelationDaoImpl implements RFQConsultGroupRelationDao {
	...
	@Override
	public Page<InquiryMessageView> message(Integer rec,boolean supplier,Integer start,Integer limit){
		limit = limit == 0?15:limit;
		String sql = " SELECT relation.pkey,consult.title,message.re,consult.create_time " + 
						...
						" WHERE relation.supplier_id = " + rec;
		...
		List<InquiryMessageView> items = Query.sql(sql).queryMaps().stream().map(item->{
