        ...
        new SQL() {
          {
            ...
            FROM(EasyOdrline.class).WHERE(EasyOdrline.T.ORDER_ID, "=?", eastOdrId);
          }
        };
    List<EolineView> list =
                    ...
                    new EolineView() {
                      {
                        ...
                        setProductname(
                            translateUtil.getLanguage(
                                bean.get(EasyOdrline.T.PRODUCTNAME.getFld().getCodeSqlField()),
                                language));
                        setColor(
                            translateUtil.getLanguage(
                                bean.get(EasyOdrline.T.COLOR.getFld().getCodeSqlField()),
                                language));
                        setSize(
                            translateUtil.getLanguage(
                                bean.get(EasyOdrline.T.SIZE.getFld().getCodeSqlField()), language));
                        setNum(
                            String.valueOf(bean.get(EasyOdrline.T.NUM.getFld().getCodeSqlField())));
                        setRemarks(
                            (String) bean.get(EasyOdrline.T.REMARKS.getFld().getCodeSqlField()));
                      }
                    })
