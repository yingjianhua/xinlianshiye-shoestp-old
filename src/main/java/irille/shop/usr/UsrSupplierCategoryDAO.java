...
public class UsrSupplierCategoryDAO {
  ...
  public static List<SupplierCategoryView> listViewIsTop(Language lang) {
    ...
    return Query.SELECT(lang, T.PKEY, T.SHOW_NAME).FROM(UsrSupplierCategory.class).queryList()
        .stream()
        .map(
            bean ->
                new SupplierCategoryView() {
                  {
                    setId(bean.getPkey());
  ...
  public static List<irille.sellerAction.view.SupplierCategoryView> listViews() {
    return Query.SELECT(T.PKEY, T.SHOW_NAME).FROM(UsrSupplierCategory.class).queryList().stream()
        .map(
            bean ->
                new irille.sellerAction.view.SupplierCategoryView() {
                  {
                    setId(bean.getPkey());
  ...
  public static List<SupplierCategoryView> listView() {
    return Query.SELECT(T.PKEY, T.NAME).FROM(UsrSupplierCategory.class).queryList().stream()
        .map(
            bean ->
                new SupplierCategoryView() {
                  {
  ...
  /** —————————————————分割线(新平台)——————————————————————————— */
  public static Page getUsrSupplierCategoryList(
      Integer start, Integer limit, String name, Integer supplierCategory) {
    if (start == null) {
      start = 0;
    }
    if (limit == null) {
    ...
    List<UsrSupplierCategoryView> list =
        Query.sql(sql.LIMIT(start, limit)).queryMaps().stream()
            .map(
                o ->
                    new UsrSupplierCategoryView() {
                      {
                        ...
                        if (o.get(T.CATEGORY_UP.getFld().getCodeSqlField()) != null) {
                          setCategoryUp(
                              BeanBase.load(
                                      ...
                                      (Integer) o.get(T.CATEGORY_UP.getFld().getCodeSqlField()))
                                  .getName());
                        }
                        setCreateBy(
                            Byte.valueOf(
                                String.valueOf(o.get(T.CREATE_BY.getFld().getCodeSqlField()))));
  ...
  private static void valids(UsrSupplierCategory b) {
    if (b.getCategoryUp() != null) {
      ...
      UsrSupplierCategory usrSupplierCategory =
          BeanBase.load(UsrSupplierCategory.class, b.getCategoryUp());
