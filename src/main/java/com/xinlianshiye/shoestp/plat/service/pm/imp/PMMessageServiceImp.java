...
public class PMMessageServiceImp implements IPMMessageService{
	...
	@Inject
	private IPMMessageDao messageDao;
	...
	@Override
	public Page list(UsrSupplier supplier,UsrPurchase purchase,ORCVRType type,Integer start,Integer limit) {
	...
	public void send(OTempType tempType,UsrSupplier supplier,UsrPurchase purchase,Object... objects) {
		...
		if(template.getEmailStatus().equals(OYn.YES.getLine().getKey())) {
			...
			if(tempType.getRcvrType().equals(ORCVRType.SUPPLIER)) {
				sendEmail(tempType.getRcvrType(),supplier,null,template.getMailTitle(),content);
			}else if(tempType.getRcvrType().equals(ORCVRType.PURCHASE)) {
				sendEmail(tempType.getRcvrType(),null,purchase,template.getMailTitle(),content);
			}else {
				sendEmail(tempType.getRcvrType(),null,null,template.getMailTitle(),content);
			}
		}
		...
		if(template.getPmStatus().equals(OYn.YES.getLine().getKey())) {
			...
			if(tempType.getRcvrType().equals(ORCVRType.SUPPLIER)) {
				sendPm(tempType.getRcvrType(),supplier,null,template.getTitle(),content);
			}else if(tempType.getRcvrType().equals(ORCVRType.PURCHASE)) {
				sendPm(tempType.getRcvrType(),null,purchase,template.getTitle(),content);
			}else {
				sendPm(tempType.getRcvrType(),null,null,template.getTitle(),content);
			}
		}
	}
	
	
	private void sendPm(ORCVRType type,UsrSupplier supplier,UsrPurchase purchase,String title, String content) {
		PMMessage message = initMessage(type,supplier,purchase,title, content);
		message.setMsgType(OMessageType.PM.getLine().getKey());
		Integer count = usrService.count(type);
		message.setCount(count + "/" + count);
		message.ins();
	}


	private void sendEmail(ORCVRType type,UsrSupplier supplier,UsrPurchase purchase,String title,String content) {
		PMMessage message = initMessage(type,supplier,purchase,title, content);
		...
		int i = 0;
		List<String> mails = usrService.getMailsBy(type);
		if(flag) {
			//邮件群发
			if(null != mails) {
	...
	private PMMessage initMessage(ORCVRType type,UsrSupplier supplier,UsrPurchase purchase,String title, String content) {
