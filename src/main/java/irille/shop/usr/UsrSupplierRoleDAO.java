...
public class UsrSupplierRoleDAO {
  ...
  public UsrSupplierRole getDefaultRole() {
    List<UsrSupplierRole> list =
        UsrSupplierRole.list(
            UsrSupplierRole.class,
            UsrSupplierRole.T.IS_DEFAULT.getFld().getCodeSqlField() + "=?",
            false,
            BeanBase.booleanToByte(true));
    if (list.size() == 0) {
      return null;
    } else {
      return list.get(0);
  ...
  public static UsrSupplierRole getDefault() {
    return Query.SELECT(UsrSupplierRole.class)
        .WHERE(T.IS_DEFAULT, "=?", BeanBase.booleanToByte(true))
        .query();
  }
  ...
  public static class SetDefault extends IduOther<SetDefault, UsrSupplierRole> {
    ...
    public void run() {
      ...
      for (UsrSupplierRole line : list) {
        if (line.getPkey().equals(getB().getPkey())) line.stIsDefault(true);
        else line.stIsDefault(false);
        line.upd();
      }
    }
  }
  ...
  public static class Ins extends IduIns<Ins, UsrSupplierRole> {
    ...
    public void before() {
      ...
      if (UsrSupplierRole.chkUniqueCode(false, getB().getCode()) != null) {
        throw LOG.err("notFound", "角色代码[{0}]已存在,不可重名!", getB().getCode());
      }
  ...
  public static class Del extends IduDel<Del, UsrSupplierRole> {
    ...
    public void before() {
      ...
      int count =
          BeanBase.list(
                  UsrSupplier.class,
                  UsrSupplier.T.ROLE.getFld().getCodeSqlField() + "= " + getB().getPkey(),
                  false)
              .size();
      if (count > 0) {
  ...
  public static HashSet<String> listByRole(Integer pkey, String module) {
    ...
    if (module != null && !module.equals("")) {
      List<UsrAccess> list =
          BeanBase.list(
              UsrAccess.class, UsrAccess.T.MODULE.getFld().getCodeSqlField() + "=?", false, module);
      Set<String> acts = new HashSet<String>();
  ...
  public static class ProvCtrl {
    ...
    public ProvCtrl() {
      /** 初始化数据>>菜单与菜单功能 避免重复读取数据库 */
      List<SysMenuAct> list = BeanBase.list(SysMenuAct.class, "", false);
      for (SysMenuAct line : list) _actMap.put(line.getPkey(), line);
      List<SysMenu> list2 = BeanBase.list(SysMenu.class, "", false);
      for (SysMenu line : list2) _menuMap.put(line.getPkey(), line);
    }
    ...
    public String initActSet() {
      ...
      if (_isActSet.contains(new Integer(supplier)) == false) {
        HashSet<Integer> actSet =
            UsrSupplierRoleActDAO.listByRole(SellerAction.getSupplier().getRole());
        ...
        String types = "";
        for (String std : typeSet) types += "," + std;
    ...
    public static JSONArray crtRes(Integer pkey, String type) throws JSONException {
      ...
      if (role.getActs() != null) for (String act : role.getActs().split(",")) roleActs.add(act);
    ...
    public class ProvLine {
      ...
      public ProvLine addMenu(SysMenu menu) {
        ...
        // 二级菜单
        if (level == 2) {
          if (_menuTwoList == null) _menuTwoList = new ArrayList<SysMenu>();
          _menuTwoList.add(menu);
          return this;
        }
        // 三级菜单
        if (level == 3) {
          if (_menuThrMap == null) _menuThrMap = new HashMap<Integer, ArrayList<SysMenu>>();
      ...
      private void setActs(SysMenu menu, String acts) {
        if (acts == null) return;
        if (_actsMap == null) _actsMap = new HashMap<Integer, String>();
        _actsMap.put(menu.getPkey(), acts);
      }

      // 上下级菜单排序错误检查
      private int validate(SysMenu menu) {
        if (menu.getMenuUp().intValue() == _menuOne.getPkey().intValue()) return 2;
        if (_menuTwoList != null) {
          for (SysMenu line : _menuTwoList)
            if (menu.getMenuUp().intValue() == line.getPkey().intValue()) return 3;
        }
        throw LOG.err("errOrder", "上下级菜单排序错误[" + menu.getName() + "]");
      }

      public JSONObject toJson(String type) throws JSONException {
        if (_actsMap == null) return null; // 没有任务权限，无此交易
        boolean isleaf = true;
        if (_menuTwoList != null) isleaf = false;
        String act1 = _actsMap.get(_menuOne.getPkey());
        JSONObject json = new JSONObject();
        json.put("id", getMenuId(_menuOne))
            .put("text", _menuOne.getName())
            .put("leaf", isleaf + "")
            .put("cls", isleaf ? "file" : "floder")
            .put("url", _menuOne.getUrl())
            .put("beanType", type)
            .put("roles", act1);
        // 下面是二级菜单的处理
        if (isleaf == false) {
          ...
          for (SysMenu line2 : _menuTwoList) {
            ...
            if (_menuThrMap != null && _menuThrMap.containsKey(line2.getPkey())) isleaf2 = false;
            String acts2 = _actsMap.get(line2.getPkey());
            JSONObject json2 = new JSONObject();
            json2
                .put("id", getMenuId(line2))
                .put("text", line2.getName())
                .put("leaf", isleaf2 + "")
                .put("cls", isleaf2 ? "file" : "floder")
                .put("url", line2.getUrl())
                .put("beanType", type)
                .put("roles", acts2);
            ...
            if (isleaf2 == false) {
              ...
              for (SysMenu line3 : list3) {
                ...
                if (acts3 == null) continue;
                JSONObject json3 = new JSONObject();
                json3
                    .put("id", getMenuId(line3))
                    .put("text", line3.getName())
                    .put("leaf", "true")
                    .put("cls", "file")
                    .put("url", line3.getUrl())
                    .put("beanType", type)
                    .put("roles", acts3);
                ja3.put(json3);
              }
            ...
            // 二级节点无权限并无下级菜单时跳过
            if (acts2 == null && json2.has("children") == false) continue;
            ja2.put(json2);
          }
          if (ja2.length() > 0) json.put("children", ja2);
        }
        // 一级节点无权限并无下级菜单时跳过
        if (act1 == null && json.has("children") == false) return null;
        return json;
      }
