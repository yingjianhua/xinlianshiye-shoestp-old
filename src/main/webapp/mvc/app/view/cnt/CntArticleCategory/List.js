Ext.define('mvc.view.cnt.CntArticleCategory.List',{
extend : 'Ext.panel.Panel',
oldId : 'CntArticleCategory_list_',
loadMask : true,
multiSelect : true,
roles : '',
layout : 'border',
...
mainActs.push({
		...
		items : [{
				xtype : Ext.create('mvc.view.cnt.CntArticleCategory.ListMain',{
							title : '文章类型',
							...
							listeners : 							 {
				                ...
				                selectionchange: function(model, records) {
				                    if (records.length === 1){
        								...
        								this.mdLineTable.store.filter([{'id':'filter', 'property':'rootid','value':records[0].get('bean.pkey')}]);
    									if (this.roles.indexOf('upd') != -1)
				...
				xtype : Ext.create('mvc.view.cnt.CntArticleCategory.ListLineCntArticleCategory',{
							title : '文章信息',
...
onIns : function(){
		var win = Ext.create('mvc.view.cnt.CntArticleCategory.Win',{
			title : this.title+'>新增'
		});
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/cnt_CntArticleCategory_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.cnt.CntArticleCategory.WinSearch',{
