...
//				}
	...
	o.attr_show.on("click ontouchstart", "span", function(e){//增加ipad触屏事件
		...
		if($this.hasClass("selected")){//取消操作
			if($("#goods_form .xmg-huodong .xmg-r").children().hasClass("selectd") == false){
				$("#goods_form .xmg-huodong .xmg-r .xmg-t").siblings(".xmg-t").hide();
				$("#goods_form .xmg-huodong .xmg-r .xmg-t:first").show();
				$("#goods_form .prod_info_line .prod_info_qty .txt .stock").children("b").hide();
				$("#goods_form .prod_info_line .prod_info_qty .txt .stock").children("b:first").show();
			}
	...
	$(".add_favorite").on('click', function(){
		...
		$.ajax({
         ...
         success: function (data) {
        	 if(data.ret==-1){
         		//$("#ficon3").click();
         		window.location.href = "/home/usr_UsrPurchase_sign?jumpUrl=/home/usr_UsrPurchase_userIndex";
        		return;
         	}
        	 if(data.type == 1){
        		 $(favorite).find("img").attr("src","../home/static/themes/default/mobile/images/like.png");
        	 }else{
        		 $(favorite).find("img").attr("src","../home/static/themes/default/mobile/images/liked.png");
        	 }
