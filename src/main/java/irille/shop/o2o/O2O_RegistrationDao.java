...
public class O2O_RegistrationDao {
  ...
  public Page list(
      ...
      Integer marketing,
      Integer buyerType,
      Integer exhibitionCountry,
      Integer start,
      Integer limit) {
    SQL sql = new SQL();
    sql.SELECT(O2oRegistration.class);
    if (gender != null) {
      if (gender == 1) sql.WHERE(O2oRegistration.T.GENDER, " =? ", O2O_Sex.men.getLine().getKey());
      else sql.WHERE(O2oRegistration.T.GENDER, " =? ", O2O_Sex.women.getLine().getKey());
    }
    if (marketing != null) {
      sql.WHERE(
          "( O2oRegistration.marketing = "
              + marketing
              + " OR O2oRegistration.marketing LIKE '%,"
              + marketing
              + "' OR O2oRegistration.marketing LIKE '%,"
              + marketing
              + ",%' OR O2oRegistration.marketing LIKE '"
    ...
    if (buyerType != null) sql.WHERE(O2oRegistration.T.BUYER_TYPE, " =? ", buyerType);
    if (exhibitionCountry != null) {
      sql.WHERE(
          "( O2oRegistration.exhibition_country = "
              + exhibitionCountry
              + " OR O2oRegistration.exhibition_country LIKE '%,"
              + exhibitionCountry
              + "' OR O2oRegistration.exhibition_country LIKE '%,"
              + exhibitionCountry
              + ",%' OR O2oRegistration.exhibition_country LIKE '"
    ...
    List<O2oRegistration> queryList =
        Query.sql(sql.LIMIT(start, limit)).queryList(O2oRegistration.class);
    List<O2o_RegistrationView> listView =
        queryList.stream()
            .map(
                bean ->
                    new O2o_RegistrationView() {
                      {
                        setId(bean.getPkey());
                        ...
                        if (marketing != null && marketing.length > 0) {
                          for (String item : marketing) {
                            for (O2O_Marketing en : O2O_Marketing.values()) {
                              if (Integer.valueOf(en.getLine().getKey())
                                  .equals(Integer.parseInt(item))) {
                          ...
                          if (marketingBuff.lastIndexOf(",") + 1 == marketingBuff.length()) {
                            marketingBuff.delete(
                                marketingBuff.length() - 1, marketingBuff.length());
                          }
                        }
                        ...
                        if (exhibitionCountry != null && exhibitionCountry.length > 0) {
                          ...
                          if (exhibitionCountryBuff.lastIndexOf(",") + 1
                              == exhibitionCountryBuff.length()) {
                            exhibitionCountryBuff.delete(
                                exhibitionCountryBuff.length() - 1, exhibitionCountryBuff.length());
                          }
                        }
