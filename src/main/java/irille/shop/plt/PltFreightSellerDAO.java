...
public class PltFreightSellerDAO {
	...
	public static class Ins extends IduInsLines<Ins, PltFreightSeller, PltFreightSellerLine>{
		...
		public void valid() {
			...
			if(getB().getWeightMin().compareTo(getB().getWeightMax())>=0)
				throw LOG.err(Msgs.priceErr, PltFreightSeller.T.WEIGHT_MAX.getFld().getName(), PltFreightSeller.T.WEIGHT_MIN.getFld().getName());
	        ...
	        if(PltFreightSeller.chkUniqueSupplier_company(true, SellerAction.getSupplier().getPkey(), getB().getCompany()) != null) 
	        ...
	        if(getLines()!= null)
	        	for (int i = 0; i < s.size(); i++) {
					...
					{
						throw LOG.err(ErrMsgs.lowPriceErr,PltFreightSellerLine.T.WEIGHT_PRICE .getFld().getName(),0);
					}
					if(s.get(i).getAggravatePrice().compareTo(BigDecimal.ZERO)<0)
					{
						throw LOG.err(ErrMsgs.lowPriceErr,PltFreightSellerLine.T.AGGRAVATE_PRICE.getFld().getName(),0);
					}
				}     	
		}	
	}
	public  static class Upd extends IduUpdLines<Upd,PltFreightSeller, PltFreightSellerLine>
	{
		...
		public void before() {
			...
			PropertyUtils.copyPropertiesWithout(dbBean, getB(),PltFreightSeller.T.PKEY,PltFreightSeller.T.SUPPLIER);
			SQL sql=new SQL() {{
		...
		public void before() {
			if( getLines(PltFreightSellerLine.T.MAIN,getB().getPkey())!=null){
				...
				for (PltFreightSellerLine  psl : sline) {
					Idu.delLine(BeanBase.list(PltCountryFreight.class,PltCountryFreight.T.REGION.getFld().getCodeSqlField() + "=? ",false,psl.getPkey()));
				}
	...
	public static void init(Integer supplier, boolean force) {
		if(!force && Query.SELECT(PltFreightSeller.class).WHERE(T.SUPPLIER, "=?", supplier).queryCount()>0)
			return;
	 ...
	 * @author zw
	 */
	public  static  Page<FreightSellerlistView> listfreightlike(Integer pkey ,String selectcompany,Integer getStart, Integer getLimit){
		BeanQuery<PltFreightSeller> q = Query
		.SELECT(PltFreightSeller.class)
		.WHERE(T.SUPPLIER, "=?", pkey)
		.WHERE(selectcompany!=null, T.COMPANY, "like ?", "%"+selectcompany+"%")
		.limit(getStart, getLimit);
		List<FreightSellerlistView> items = q.queryList().stream().map(bean->{
	...
	public static List<FreightSellerlistView> getFreightBySupplier(Integer supplier){
		return Query
				...
				.queryList()
				.stream()
				.map(bean -> new FreightSellerlistView(){{
					setId(bean.getPkey());
					setCompany(bean.getCompany());
				}})
				.collect(Collectors.toList());
