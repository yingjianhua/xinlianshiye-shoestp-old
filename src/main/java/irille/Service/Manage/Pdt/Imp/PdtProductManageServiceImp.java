...
public class PdtProductManageServiceImp implements IPdtProductManageService {
  ...
  public Integer saveProduct(String data, Integer supId) throws IOException, ExecutionException {
    ...
    if (pdtProductSaveView.getTieredPricing() != null && !pdtProductSaveView.getTieredPricing()
        .isEmpty()) {
    ...
    if (pdtProductSaveView.getNewSpec() != null && !pdtProductSaveView.getNewSpec().isEmpty()) {
      for (int i = 0; i < pdtProductSaveView.getNewSpec().size(); i++) {
        if (pdtProductSaveView.getSpecColor() != null && !pdtProductSaveView.getSpecColor()
            .isEmpty()) {
          if (!pdtProductSaveView.getSpecColor()
        ...
        try {
          ...
          JSONObject jsonColorName = new JSONObject(
              pdtProductSaveView.getNewSpec().get(i).getColorName());
          for (Language item : Language.values()) {
            String str = "";
            str += jsonColorName.get(item.toString()) + " " + pdtProductSaveView.getNewSpec().get(i)
                .getSizeName();
...
//        pdtProduct.stState(pdtProductSaveView.isState() ? Pdt.OState.ON : Pdt.OState.OFF);
    ...
    if (pdt.getProductType() == Pdt.OProductType.PrivateExpo.getLine().getKey()) {
      O2O_PrivateExpoPdt o2o_privateExpoPdt = O2O_PrivateExpoPdt
          .chkUniquePdt_Id(false, pdt.getPkey());
    ...
    if (pdtProductSaveView.getId() <= 0) {
      ...
      PdtTieredPricingDao
          .updByPdt(pdtProductSaveView.getId(), pdtProductSaveView.getTieredPricing());
    }
    return 1;
  }

  @Override
  public Page getWarehouse(Integer supplierm, String productName, String productNum, Integer status,
    ...
    List<WarehouseView> list = Query.sql(sql.LIMIT(start, limit)).queryMaps().stream()
        .map(o -> new WarehouseView() {{
          ...
          if (o.get(PdtProduct.T.CATEGORY.getFld().getCodeSqlField()) != null) {
            setProductCate(BeanBase.load(PdtCat.class,
                (Integer) o.get(PdtProduct.T.CATEGORY.getFld().getCodeSqlField())).getName());
          }
          if (o.get(PdtProduct.T.CATEGORY_DIY.getFld().getCodeSqlField()) != null) {
            setStoreCate(BeanBase.load(UsrProductCategory.class,
                (Integer) o.get(PdtProduct.T.CATEGORY_DIY.getFld().getCodeSqlField())).getName());
          }
