			...
			$('.btn_submit', reg_form).on('tap', function(e){
				if(reg_rq_mark){
					...
					setTimeout(function(){
						...
						//通过验证，提交数据
						reg_rq_mark=false;
						$.post('/home/usr_UsrPurchase_register', reg_form.serializeArray(), function (data){
							if(data.ret == 1) {
								//When a sign up is completed, such as signup for trial.
								//parseInt(ueeshop_config.FbPixelOpen)==1 && fbq("track", "Lead");
								$('html').tips_box(lang_obj.signIn.Register_Success, 'success');
								var em = $("#reg_form").find("input[name=Email]").val();
			...
			$('.btn_submit', login_form).on('tap', function (e){
				if (login_rq_mark){
					...
					setTimeout(function (){
						...
						//通过验证，提交数据
						login_rq_mark=false;
						$.post('/home/usr_UsrPurchase_signIn', login_form.serializeArray(), function (data){
							if(data.ret!=1){
								if (data.ret==-1){//刷新水印
								...
								var msg=data.msg[0];
								if(data.ret==0){
								msg=lang_obj.signIn.error_note;
								}
								else{
								msg=lang_obj.signIn.verification;
								}
								...
								if (val>=3){
									w_login_vcode.find('input[name=checkCode]').attr('notnull', 'notnull');
