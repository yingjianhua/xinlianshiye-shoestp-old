...
public class PdtAttrCatAction extends MgtAction<PdtAttrCat> {
  ...
  public void list() throws Exception {
    ...
    page.setWhere(where);
    page.commit();
    List<PdtAttrCat> list = page.getList();
    JSONObject lineJson = null;
    for (PdtAttrCat line : list) {
      if (!line.gtDeleted()) {
  ...
  public void delMulti() throws Exception {
    ...
    for (Serializable line : svs) {
      PdtAttrCatDAO.Del upd = new PdtAttrCatDAO.Del();
      upd.setBKey(line);
      upd.commit();
    }
    writeSuccess();
