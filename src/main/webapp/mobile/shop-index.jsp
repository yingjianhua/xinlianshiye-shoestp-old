...
<html lang="en" data-dpr="1">
    <head>
        ...
        <style>
          /* 轮播插件的样式 */
          .slideBox{ width:96%; height:13.5rem; margin: 0 auto;overflow:hidden; position:relative; border:1px solid #ddd;  }

          .slideBox .hd{ height:15px; overflow:hidden; position:absolute; left: 50%; transform: translateX(-50%);bottom:12px; z-index:1; }
          .slideBox .hd ul{ overflow:hidden; zoom:1; float:left;  }
          .slideBox .hd ul li{ float:left; margin-right:12px;  width:30px; height:6px; line-height:6px; font-size: 0;text-align:center; background:#ddd; cursor:pointer; border-radius: 6px;}
          .slideBox .hd ul li.on{ background:#fff; color:#fff; }

          .slideBox .bd{ position:relative; height:100%; z-index:0;   }
          .slideBox .bd li{ zoom:1; vertical-align:middle; }
          .slideBox .bd img{ width:100%; height:13.5rem; display:block; object-fit: cover;}
        </style>
    </head>
                            ...
                            <ul>
                              <%-- <li><img src="/home/static/themes/default/mobile/images/0af52b7816.jpg" /></li>
                    					...
                    					<c:set var="companyPhotoLink" value="''"/>
						                  <c:forTokens items="${supView.companyPhoto}" delims="," var="companyPhoto" varStatus="index">
						                  		<c:if test="${not empty supView.companyPhotoLink}">
								              		<c:forTokens items="${supView.companyPhotoLink}" delims="," var="companyLink" varStatus="index2">
								              			<c:if test="${index.index == index2.index}">
