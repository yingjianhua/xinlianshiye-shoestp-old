        ...
        Query.sql(sql.LIMIT(start, limit)).queryMaps().stream()
            .map(
                o -> {
                  ...
                  view.setContact(
                      String.valueOf(o.get(UsrSupplier.T.CONTACTS.getFld().getCodeSqlField())));
                  view.setContactInformation(
                      String.valueOf(o.get(UsrSupplier.T.EMAIL.getFld().getCodeSqlField())));
                  view.setWhetherCertification(
                      Byte.valueOf(
                          String.valueOf(o.get(UsrSupplier.T.IS_AUTH.getFld().getCodeSqlField()))));
                  view.setImage(
                      String.valueOf(o.get(UsrSupplier.T.CERT_PHOTO.getFld().getCodeSqlField())));
                  view.setStatus(
                      Byte.valueOf(
                          String.valueOf(
                              o.get(
                                  OrderMeetingAuditRelease.T.STATUS.getFld().getCodeSqlField()))));
                  return view;
                })
