...
public class UsrCartAction extends HomeAction<UsrCart> {
   ...
   * 加入购物车
   *
   * @author lijie@shoestp.cn @Description:加入购物车
   * @date 2018/8/8 10:56
   */
  ...
  /**
   * 放入购物车... Json 形式 { specid:qty}这样的形式
   *
  ...
  public String showAdvCart() throws JSONException {
    ...
    List<UsrCart> carts =
        BeanBase.list(
            ...
            false);
    List<irille.homeAction.usr.dto.SupplierView> supplierList =
        irille.homeAction.usr.dto.SupplierView.buildByCart(carts);
    String supId = "";
    ...
    List<PltFreightSeller> expressList =
        BeanBase.list(
            ...
            false,
            Sys.OEnabled.TRUE.getLine().getKey());
    List<irille.homeAction.usr.dto.ProductView> pdtViews =
        irille.homeAction.usr.dto.ProductView.build(carts);
    ...
    //        Integer sum = 0;
    //        List<UsrPurchaseLine> listUsrPurchaseLine =
    // Idu.getLines(UsrPurchaseLine.T.PURCHASE.getFld(), getPurchase().getPkey());
    //        List<PltCountry> listPltCountry = new ArrayList<PltCountry>();
    //        List<PltProvince> listProvince = new ArrayList<PltProvince>();
    ...
    //		Map<String, Map<PdtProduct, Map<PdtColor, List<PdtSize>>>> cart = new HashMap<String,
    // Map<PdtProduct, Map<PdtColor, List<PdtSize>>>>();
    //		Map<PdtProduct, Map<PdtColor, List<PdtSize>>> product = new HashMap<PdtProduct,
    // Map<PdtColor, List<PdtSize>>>();
    //		//Map<PdtProduct, Map<PdtColor, List<PdtSize>>> productB = new HashMap<PdtProduct,
    // Map<PdtColor, List<PdtSize>>>();
    ...
    //		            } else {
    //		              Map<PdtColor, List<PdtSize>> mapTest =
    // product.get(usrcart.gtSpec().gtProduct());
    ...
    //		            } else {
    //		              Map<PdtColor, List<PdtSize>> mapTest =
    // productB.get(usrcart.gtSpec().gtProduct());
    //		              if (mapTest.get(usrcart.gtSpec().gtColor()) == null) {
    //		                  List<PdtSize> sizeList = new ArrayList<PdtSize>();
    //		                  PdtSize sz=usrcart.gtSpec().gtSize();
    //			              sz.setPkey(usrcart.getQty());
  ...
  public void cartDateVerify() throws Exception {
    ...
    for (int i = 0; i < strSpecList.length; i++) {
      List<PdtSpec> specList =
          BeanBase.list(PdtSpec.class, PdtSpec.T.PKEY + " = " + strSpecList[i], false);
    ...
    for (int i = 0; i < pdtSpecList.size(); i++) {
      if (!productPkey.equals(pdtSpecList.get(i).getProduct())) {
        json.put("data", "-1");
        write(json.toString());
        return;
      }
  ...
  public String cartToDeliveryInformation() throws Exception {
    ...
    for (int i = 0; i < cartArray.length; i++) {
      ...
      List<PrmGroupPurchaseLine> list =
          BeanBase.list(
              PrmGroupPurchaseLine.class,
              PrmGroupPurchaseLine.T.PRODUCT + " = " + gtProduct.getPkey(),
              false);
      ...
      if (Integer.valueOf(loadCart.gtSpec().gtProduct().getProductType())
    ...
    if ((pitchOnCart == null || pitchOnCart.size() == 0)
        && (pitchOnCartPRM == null || pitchOnCartPRM.size() == 0)) {
    ...
    for (int i = 0; i < listSpes.size(); i++) {
      ...
      ps.setPdtProduct(
          translateUtil.getAutoTranslate(listSpes.get(i).gtProduct(), HomeAction.curLanguage()));
      ps.setPdtColor(
          translateUtil.getAutoTranslate(listSpes.get(i).gtColor(), HomeAction.curLanguage()));
      ps.setPdtSize(
          translateUtil.getAutoTranslate(listSpes.get(i).gtSize(), HomeAction.curLanguage()));
      psList.add(ps);
    }
    ...
    for (int i = 0; i < listSpesPRM.size(); i++) {
      ...
      ps.setPdtProduct(
          translateUtil.getAutoTranslate(listSpesPRM.get(i).gtProduct(), HomeAction.curLanguage()));
      ps.setPdtColor(
          translateUtil.getAutoTranslate(listSpesPRM.get(i).gtColor(), HomeAction.curLanguage()));
      ps.setPdtSize(
          translateUtil.getAutoTranslate(listSpesPRM.get(i).gtSize(), HomeAction.curLanguage()));
      psListPRM.add(ps);
    }
    ...
    List<UsrPurchaseLine> list =
        BeanBase.list(
            ...
            UsrPurchaseLine.T.PURCHASE
                + " = "
                + 2
                + "  and  "
                + UsrPurchaseLine.T.ISDEFAULT
                + " = 1 ",
            false);
    ...
    List<PltFreightSeller> freightSellerlist =
        BeanBase.list(
            PltFreightSeller.class, PltFreightSeller.T.SUPPLIER + " = " + supplier, false);
  ...
  public void getSpecColor() throws Exception {
    JSONObject json = new JSONObject();
    List<PdtSpec> Onelist =
        BeanBase.list(
            PdtSpec.class, PdtSpec.T.PKEY + " = " + Integer.parseInt(singleSpecNum), false);
    ...
    List<PdtSpec> list =
        BeanBase.list(
            ...
            PdtSpec.T.PRODUCT
                ...
                + spec.getProduct()
                + " and "
                + PdtSpec.T.STORE_COUNT.getFld().getCodeSqlField()
                + " > 0 and "
                + PdtSpec.T.COLOR
                + " = "
                + spec.getColor()
  ...
  public void updateCart() throws Exception {
    ...
    List<UsrCart> cartList =
        BeanBase.list(
            UsrCart.class,
            UsrCart.T.PURCHASE + " = " + 2 + " and " + UsrCart.T.SPEC + " = " + singleSpecNum,
            false);
    if (cartList.size() <= 0) {
      json.put("data", "-1");
      write(json.toString());
  ...
  //			List<PdtSize> pdtSize = BeanBase.list(PdtSize.class, PdtSize.T.PKEY + " = " +
  // listSpes.get(i).getSize(),
  //					false);
  ...
  public void getCartData() throws Exception {
    ...
    JSONObject json = new JSONObject();
    json.put(
        "suppliers", new JSONArray(irille.homeAction.usr.dto.SupplierView.build(cartList), false));
    json.put(
        "products", new JSONArray(irille.homeAction.usr.dto.ProductView.build(cartList), false));
    ...
    if (getCartPkeys() != null && !getCartPkeys().trim().equals("")) {
      BeanBase.executeUpdate(
          " DELETE FROM "
              + UsrCart.TB.getCodeSqlTb()
              + " WHERE "
              + UsrCart.T.PKEY.getFld().getCodeSqlField()
              + " in ("
              + getCartPkeys()
              + ") ");
    }
  ...
  /**
   * 将商品加入购物车 使用:传入产品pkey -> 属性名:pdtPkey
   *
   * @throws Exception
   * @author liyichao
   * @updatetime 2018-8-16
    ...
    if (specList == null || specList.size() <= 0) {
      ...
      if (existsCart == null) {
        ...
        existsCart.setAmtTotal(
            existsCart
                .getAmtTotal()
                .add(specList.get(0).getPrice().multiply(BigDecimal.valueOf(cartPdt.getMinOq()))));
        existsCart.upd();
      }
    } else {
  ...
  public String buyNow() throws JSONException {
    if (getPurchase() == null) {
      setResult(
          "/home/usr_UsrPurchase_sign?&jumpUrl=/home/pdt_PdtProduct_gtProductsInfo?id=" + pid,
          false);
      return RTRENDS;
    }
    List<UsrPurchaseLine> listUsrPurchaseLine =
