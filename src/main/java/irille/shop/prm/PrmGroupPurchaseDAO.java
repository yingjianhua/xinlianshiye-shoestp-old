...
public class PrmGroupPurchaseDAO {
  ...
  public static class InsGroup extends IduOther<InsGroup, PrmGroupPurchase> {
    ...
    public void before() {
      ...
      for (GroupPdtView view : getLines()) {
        ...
        PropertyUtils.copyPropertiesWithout(
            actProduct,
            sourcePdt,
            PdtProduct.T.CATEGORY_DIY,
            PdtProduct.T.SOURCE_PRODUCT,
        ...
        List<PdtSpec> sourceSpecList =
            BeanBase.list(
                PdtSpec.class,
                PdtSpec.T.PRODUCT.getFld().getCodeSqlField() + " = " + sourcePdt.getPkey(),
                false);
    ...
    public void valid() {
      ...
      List<PrmGroupPurchase> groupPurchaseList =
          BeanBase.list(
              PrmGroupPurchase.class,
              PrmGroupPurchase.T.CREATED_BY.getFld().getCodeSqlField()
                  + " = ? AND "
                  + PrmGroupPurchase.T.STATUS.getFld().getCodeSqlField()
                  + " <> ? ",
              false,
              getCreateBy(),
              Prm.OStatus.DELETE.getLine().getKey());
      for (PrmGroupPurchase act : groupPurchaseList) {
        boolean startFlag =
            belongCalendar(getB().getStartTime(), act.getStartTime(), act.getEndTime());
        boolean endFlag = belongCalendar(getB().getEndTime(), act.getStartTime(), act.getEndTime());
        boolean containFlag =
            getB().getStartTime().getTime() <= act.getStartTime().getTime()
                && getB().getEndTime().getTime() >= act.getEndTime().getTime();
  ...
  public static class Upd extends IduOther<Upd, PrmGroupPurchase> {
    ...
    public void before() {
      ...
      if (toPre(getB().getStartTime(), getB().getPreTime())
          && model.getStatus().equals(Prm.OStatus.NOTSTART.getLine().getKey())) {
        getB().setStatus(Prm.OStatus.SOONSTART.getLine().getKey());
      } else {
        getB().setStatus(Prm.OStatus.SOONSTART.getLine().getKey());
      }
      PropertyUtils.copyPropertiesWithout(
          model, getB(), PrmGroupPurchase.T.CREATED_BY, PrmGroupPurchase.T.CREATED_TIME);
      setB(model);
    }
    ...
    public void run() {
      ...
      List<PrmGroupPurchaseLine> existsLineList =
          BeanBase.list(
              PrmGroupPurchaseLine.class,
              PrmGroupPurchaseLine.T.MAIN.getFld().getCodeSqlField() + " = ? ",
              false,
              getB().getPkey());
      ...
      for (GroupPdtView line : getLines()) {
        if (line.getGroupLinePkey() == null) {
          ...
          PrmGroupPurchaseLine model =
              BeanBase.load(PrmGroupPurchaseLine.class, line.getGroupLinePkey());
          PdtProduct product = model.gtProduct();
          product.setCurPrice(line.getAmt());
          product.setMinOq(line.getMinOq());
          List<PdtSpec> specList = new ArrayList<PdtSpec>();
          List<PdtSpec> existsSpecList =
              BeanBase.list(
                  PdtSpec.class,
      ...
      for (int i = 0; i < backPkey.size(); i++) {
        ...
        Idu.delLine(
            BeanBase.list(
                PdtSpec.class,
                PdtSpec.T.PRODUCT.getFld().getCodeSqlField() + " = " + delLine.getProduct(),
                false));
  ...
  public static class GroupList extends IduOther<GroupList, PrmGroupPurchase> {
    ...
    public void after() {
      ...
      if (getFalg() != 1) {
        where +=
            " AND "
                + PrmGroupPurchase.T.STATUS.getFld().getCodeSqlField()
                + " <> "
                + Prm.OStatus.NOTSTART.getLine().getKey();
      }
      ...
      if (title != null && title.trim() != "") {
        where +=
            " AND "
                + PrmGroupPurchase.T.TITLE.getFld().getCodeSqlField()
                + " LIKE '%"
                + title
                + "%' ";
      }
      where += " ORDER BY start_time DESC";
      List<PrmGroupPurchase> allGroupData =
          BeanBase.list(
              PrmGroupPurchase.class,
              PrmGroupPurchase.T.STATUS.getFld().getCodeSqlField() + " <> ? ",
              false,
      ...
      for (int i = 0; i < allGroupData.size(); i++) {
        ...
        switch (state1) {
          ...
          case 4:
            group.setStatus(state1);
            if (Env.getSystemTime().getTime() - group.getEndTime().getTime()
                >= 15 * 24 * 60 * 60 * 1000) {
              group.setStatus(Prm.OStatus.DELETE.getLine().getKey());
              List<PrmGroupPurchaseLine> lineList =
                  Idu.getLines(PrmGroupPurchaseLine.T.MAIN, group.getPkey());
  ...
  public static class Del extends IduDel<Del, PrmGroupPurchase> {
    ...
    public void before() {
      ...
      for (PrmGroupPurchaseLine line : lineList) {
        List<PdtSpec> pdtList =
            BeanBase.list(
                PdtSpec.class,
                PdtSpec.T.PRODUCT.getFld().getCodeSqlField() + " = " + line.getProduct(),
                false);
        for (PdtSpec spec : pdtList) {
          List<UsrCart> cartList =
              BeanBase.list(
                  UsrCart.class,
                  UsrCart.T.SPEC.getFld().getCodeSqlField() + " = ? ",
                  false,
                  spec.getPkey());
          Idu.delLine(cartList);
        }
        List<PdtComment> commentList =
            BeanBase.list(
                PdtComment.class,
                PdtComment.T.PRODUCT.getFld().getCodeSqlField() + " = ? ",
                false,
                line.getProduct());
        List<UsrConsult> consultList =
            BeanBase.list(
                UsrConsult.class,
                UsrConsult.T.PRODUCT.getFld().getCodeSqlField() + " = ? ",
                false,
                line.getProduct());
        List<UsrFavorites> favoriteList =
            BeanBase.list(
                UsrFavorites.class,
                UsrFavorites.T.PRODUCT.getFld().getCodeSqlField() + " = ? ",
                false,
                line.getProduct());
  ...
  /**
   * 用于汇总各采购商在同一活动下的订单发送给相应供应商 没有使用
   *
   * @author liyichao
   */
  public static class Send extends IduOther<Send, PrmGroupPurchase> {
    ...
    /**
     * union -> 选中的活动 prmOrderList -> 对应活动下采购商的订单列表 staOrder -> 结构为<规格Pkey , 订单详情>的map supOdrMap ->
     * 结构为<供应商Pkey , 订单详情集合>的map allSupProMap -> 结构为<供应商Pkey , 活动产品的集合>的map
     */
    @Override
    public void before() {
      // super.before();
      ...
      // 然后按供应商的家数去生成订单
      // 1.获得PrmUnion对象
      PrmGroupPurchase union =
          loadAndLock(getB().getPkey()); // Main 采购商pkey-即联合采购活动创建人 PrmUnion下的createdBy
      // 采购商订单处理
      // 2.获得对应活动下的采购商订单列表
      List<PrmGroupOrder> prmOrderList =
          BeanBase.list(
              PrmGroupOrder.class,
              PrmGroupOrder.T.ACTIVITY.getFld().getCodeSqlField() + " in( ? )",
              false,
              union.getPkey());
      ...
      for (PrmGroupOrder purOrder : prmOrderList) {
        List<PrmGroupOrderLine> purOrderLineList =
            BeanBase.list(
                PrmGroupOrderLine.class,
                PrmGroupOrderLine.T.GROUP_ORDER.getFld().getCodeSqlField() + " in( ? )",
                false,
                purOrder.getPkey());
      ...
      List<PrmGroupPurchaseLine> prmUnionLineList =
          BeanBase.list(
              PrmGroupPurchaseLine.class,
              PrmGroupPurchaseLine.T.MAIN.getFld().getCodeSqlField() + " in( ? )",
              false,
              union.getPkey());
      // 6.获取prmUnionLineList中涉及到的商家
      // 将活动中每个供应商的PrmUnionLine通过Map放到对应供应商的PrmUnionLine集合中
      Map<Integer, List<PrmGroupPurchaseLine>> allSupProMap =
          new HashMap<Integer, List<PrmGroupPurchaseLine>>();
  ...
  public static class GetAll extends IduOther<GetAll, PrmGroupPurchase> {
    ...
    public void before() {
      String state =
          Prm.OStatus.SOONSTART.getLine().getKey()
              + " , "
              + Prm.OStatus.HAVEINHAND.getLine().getKey()
              + " , "
              + Prm.OStatus.SOONEND.getLine().getKey()
              + " , "
              + Prm.OStatus.END.getLine().getKey();
      unionList =
          BeanBase.list(
              PrmGroupPurchase.class,
              PrmGroupPurchase.T.CREATED_BY.getFld().getCodeSqlField()
  ...
  /** ————————————————分割线(新平台)————————————————————————— */
  public static Page getPrmGroupPurchases(
      Integer start, Integer limit, String title, Integer status, Integer supplier) {
    if (start == null) {
      start = 0;
    }
    if (limit == null) {
    ...
    List<PrmGroupPurchaseView> list =
        Query.sql(sql.LIMIT(start, limit)).queryMaps().stream()
            .map(
                o ->
                    new PrmGroupPurchaseView() {
                      {
                        setId((Integer) o.get(PrmGroupPurchase.T.PKEY.getFld().getCodeSqlField()));
                        setTitle(
                            (String) o.get(PrmGroupPurchase.T.TITLE.getFld().getCodeSqlField()));
                        setStartTime(
                            (Date) o.get(PrmGroupPurchase.T.START_TIME.getFld().getCodeSqlField()));
                        setEndTime(
                            (Date) o.get(PrmGroupPurchase.T.END_TIME.getFld().getCodeSqlField()));
                        setStatus(
                            Integer.valueOf(
                                String.valueOf(
                                    o.get(PrmGroupPurchase.T.STATUS.getFld().getCodeSqlField()))));
                        setPreTime(
                            Integer.valueOf(
                                String.valueOf(
                                    o.get(
                                        PrmGroupPurchase.T.PRE_TIME.getFld().getCodeSqlField()))));
                        setSupplier(
                            BeanBase.load(
                                    UsrSupplier.class,
                                    (Integer)
                        ...
                        setCreatedTime(
                            (Date)
                                o.get(PrmGroupPurchase.T.CREATED_TIME.getFld().getCodeSqlField()));
                        setUpdatedTime(
                            (Date)
                                o.get(PrmGroupPurchase.T.UPDATED_TIME.getFld().getCodeSqlField()));
                      }
                    })
                    ...
                    new SupplierView() {
                      {
                        setId(
                            (Integer)
                                o.get(PrmGroupPurchase.T.CREATED_BY.getFld().getCodeSqlField()));
