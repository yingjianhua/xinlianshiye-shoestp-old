...
public class UsrConsultMessageDAO {
	...
	public static class Send extends IduIns<Send, UsrConsultMessage> {
		...
		public void after() {
			if(p2s)
				relation.stPToSNewMsg(true);
			else
				relation.stSToPNewMsg(true);
			relation.upd();
...
//		delete from usr_consult_message where relation in (select pkey from usr_consult_relation where consult=?)|params:1
		SQL subSql = new SQL(){{
