...
public abstract class SetBeans {
  ...
  public static void set(Object get, Object set, Class label) {
    ...
    Field[] fields =
        label != null
            ? GetCalssFiledsAndMethods.getFidlds(label)
            : GetCalssFiledsAndMethods.getFidlds(set);
    for (Field field : fields) {
      try {
        ...
        do {
          ...
          if (setBean.OriginalField().length() > 0) {
            if (get instanceof Map) {
              ...
              if (label != null) {
                setMethodString =
                    GetCalssFiledsAndMethods.setMethodName(
                        set, setBean.OriginalField(), field.getType());
                getMethodString =
                    GetCalssFiledsAndMethods.getMethodName(get, field.getName(), field.getType());
              } else {
                getMethodString =
                    GetCalssFiledsAndMethods.getMethodName(
                        get, setBean.OriginalField(), field.getType());
              }
            }
          }
        ...
        if (setMethodString == null) {
          setMethodString =
              GetCalssFiledsAndMethods.setMethodName(set, field.getName(), field.getType());
        }
        do {
          ...
          Method setMethod =
              GetCalssFiledsAndMethods.getMethod(set, setMethodString, result.getSetType());
          if (setMethod != null) {
