...
public class PltPayAction extends SellerAction<PltPay> implements IPltPayAction {
  ...
  public void savePaySetting() throws IOException, JSONException {
    ...
    for (int i = 0; i < jsonArray.length(); i++) {
      ...
      pltPay.setPoundage(
          BigDecimal.valueOf(jsonObject.getDouble(PltPay.T.POUNDAGE.getFld().getCodeSqlField())));
      pltPay.setExtraCosts(
          BigDecimal.valueOf(
              jsonObject.getDouble(PltPay.T.EXTRA_COSTS.getFld().getCodeSqlField())));
      pltPay.stEnabled(jsonObject.getBoolean("enabled"));
            ...
            try {
              if (v == 2) {
                ...
                for (int i1 = 0; i1 < js.length(); i1++) {
                  ...
                  if (PltPaySettingEnum.TT_Pay.valueOf(temp.getString("name").toUpperCase())
                      != null) {
        ...
        case 5:
          {
            try {
              if (v == 2) {
                ...
                for (int i1 = 0; i1 < js.length(); i1++) {
                  JSONObject temp = js.getJSONObject(i1);
                  if (PltPaySettingEnum.Ooffline_Pay.valueOf(temp.getString("name").toUpperCase())
                      != null) {
                    save.put(temp.getString("name"), temp.getString("value"));
                  }
                }
              } else {
                for (PltPaySettingEnum.Ooffline_Pay offline_pay :
                    PltPaySettingEnum.Ooffline_Pay.values()) {
                  save.put(
                      offline_pay.getFldName(), jsonObject.getString(offline_pay.getFldName()));
                }
              }
            } catch (JSONException e) {
              e.printStackTrace();
