...
public class UsrSupplierAction extends SellerAction<UsrSupplier> implements IUsrSupplierAction {
	...
	/**
	 * 供应商列表页面 Created by IntelliJ IDEA. User: lijie@shoestp.cn Date: 2018/7/19
	 * Time: 15:46
	 */
	private int _curr;
	...
	public void regex() throws Exception {
		...
		valid.validNotEmpty(UsrSupplier.T.NAME, UsrSupplier.T.ENGLISH_NAME, UsrSupplier.T.COMPANY_ADDR,
				UsrSupplier.T.TARGETED_MARKET, UsrSupplier.T.PROD_PATTERN, UsrSupplier.T.CREDIT_CODE,
		...
		if (getBean().getRegisteredCapital() != null)
			regex.validRegexMatched("[A-Za-z0-9\\u4e00-\\u9fa5]+", "注册资本只能输入中文、英文和数字",
					UsrSupplier.T.REGISTERED_CAPITAL);
		...
		if (getBean().getDepartment() != null)
			regex.validRegexMatched("[A-Za-z\\u4e00-\\u9fa5]{1,15}", "联系人部门只能输入中文、英文，且个数在15个之内",
					UsrSupplier.T.DEPARTMENT);
		if (getBean().getJobTitle() != null)
			regex.validRegexMatched("[A-Za-z\\u4e00-\\u9fa5]{1,15}", "联系人职称只能输入中文、英文，且个数在15个之内",
					UsrSupplier.T.JOB_TITLE);
		if (getBean().getContactEmail() != null)
			regex.validEmail(UsrSupplier.T.CONTACT_EMAIL);
		if (getBean().getIdCard() != null)
			regex.validRegexMatched("(^\\d{15}$)|(^\\d{18}$)|(^\\d{17}(\\d|X|x)$)", "请输入正确的18位身份证号码",
					UsrSupplier.T.ID_CARD);
	...
	public void updShopbase() throws Exception {
		...
		try {
			...
			if (!StringUtil.hasValue(json.get("en")))
