...
public class PltFreight extends BeanInt<PltFreight> {
  public static final Tb TB =
      new Tb(PltFreight.class, "运费管理").setAutoIncrement().addActIUDL().addActEnabled();

  public enum T implements IEnumFld { // @formatter:off
    PKEY(TB.crtIntPkey()), // 主键
    //		SUPPLIER(UsrSupplier.fldOutKey().setNull()),
    USER_SYS(SYS.USER_SYS), // 平台用户外键 //重复字段 ,下次可以已删除了 @see created_by
  ...
  ;
    public static final Index IDX_COMPANY = TB.addIndex("company", true, COMPANY);
  ...
  // 方法----------------------------------------------
  public static PltFreight loadUniqueCompany(boolean lockFlag, String company) {
    return (PltFreight) loadUnique(T.IDX_COMPANY, lockFlag, company);
  }

  public static PltFreight chkUniqueCompany(boolean lockFlag, String company) {
    return (PltFreight) chkUnique(T.IDX_COMPANY, lockFlag, company);
  }
  ...
  public SysUser gtUserSys() {
    if (getUserSys() == null) return null;
    return (SysUser) get(SysUser.class, getUserSys());
  }

  public void stUserSys(SysUser userSys) {
    if (userSys == null) setUserSys(null);
    else setUserSys(userSys.getPkey());
  }
