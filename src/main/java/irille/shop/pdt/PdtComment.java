...
public class PdtComment extends BeanInt<PdtComment> {
  public static final Tb TB =
      new Tb(PdtComment.class, "评论").setAutoIncrement().addActList().addActDel();
  ...
  public UsrPurchase gtUsersPkey() {
    if (getUsersPkey() == null) return null;
    return (UsrPurchase) get(UsrPurchase.class, getUsersPkey());
  }

  public void stUsersPkey(UsrPurchase usersPkey) {
    if (usersPkey == null) setUsersPkey(null);
    else setUsersPkey(usersPkey.getPkey());
  }
  ...
  public JSONObject gtOthoerSatisfaction() throws JSONException {
    return getOthoerSatisfaction() == null
        ? new JSONObject()
        : new JSONObject(getOthoerSatisfaction());
  }
