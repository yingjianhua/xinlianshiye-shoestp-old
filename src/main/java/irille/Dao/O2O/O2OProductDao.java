...
public class O2OProductDao {
  ...
  public List<Map<String, Object>> getO2OActivityList(
      int start, int limit, Date startDate, Date endDate, String keyWord) {
    SQL sql = new SQL();
    sql.SELECT(
            O2O_Activity.T.PKEY,
            O2O_Activity.T.NAME,
  ...
  public List<Map<String, Object>> getO2OActivityPdtList(int start, int limit, Long id) {
    SQL sql = new SQL();
    sql.SELECT(
            O2O_Product.T.PKEY,
            PdtProduct.T.NAME,
        ...
        .LEFT_JOIN(O2O_JoinInfo.class, O2O_JoinInfo.T.ACTIVITY_ID, T.PKEY)
        .WHERE(T.PKEY, "=?", id)
        .LIMIT(start, limit);
    return Query.sql(sql).queryMaps();
  }
