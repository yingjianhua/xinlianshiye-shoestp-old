...
public class OrderMeetingdetailAction extends HomeAction<OrderMeetingProduct> {
  ...
  @Setter @Getter private Integer shoppingid;
  ...
  public void omtmeettingshop() throws Exception {
    ...
    ppt1.setDefaultReviewRating(
        new BigDecimal(Math.ceil(ppt.getDefaultReviewRating().doubleValue())));
    translateUtil.getAutoTranslate(ppt1, HomeAction.curLanguage());
    omdv.setProduct(ppt1);

    if (getPurchase() != null) {
      omdv.setFavorite(
          UsrFavorites.chkUniquePurchase_product(false, getPurchase().getPkey(), ppt.getPkey()));
    }
    // 获取该商品的商家信息
      ...
      for (int i = 0; i < 5; i++) {
        ...
        PdtProduct pdt =
            translateUtil.getAutoTranslate(
                newProList.get(r.nextInt(newProList.size())), HomeAction.curLanguage());
        view.setPdt(pdt);
