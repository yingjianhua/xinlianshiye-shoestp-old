...
public class CntAdDAO {
  ...
  public static class Ins extends IduInsLines<Ins, CntAd, CntAdLine> {
    ...
    public void valid() {
      ...
      // 验证数值不能为null
      //            Validate.validNotNull(new Object[]{getB().getCreatedTime()},
      // CntAd.T.CREATED_TIME.getFld().getName());
      //            for (CntAdLine cntAdvertisingLine : getLines()) {
      //                验证不能为null
      //                Validate.validNotNull(new Object[]{cntAdvertisingLine.getName(),
      // cntAdvertisingLine.getUrl()}, cntAdvertisingLine.getUrl());
      //            }
      super.valid();
    }
  ...
  public static class Upd extends IduUpdLines<Upd, CntAd, CntAdLine> {
    ...
    public void valid() {
      ...
      //
      //            //验证数值不能为null
      //            Validate.validNotNull(new Object[]{getB().getCreateTime()},
      // CntAd.T.CREATE_TIME.getFld().getName());
      //            for (CntAdLine cntAdvertisingLine : getLines()) {
      //                //验证不能为null
      //                Validate.validNotNull(new Object[]{cntAdvertisingLine.getName(),
      // cntAdvertisingLine.getUrl()}, CntAd.T.NAME.getFld().getName(),
      // CntAd.T.URL.getFld().getName());
      //            }
    }
    ...
    public void before() {
      ...
      PropertyUtils.copyPropertiesWithout(
          dbBean, getB(), CntAd.T.PKEY, CntAd.T.CREATED_BY, CntAd.T.CREATED_TIME);
      setB(dbBean);
   ...
   * @author yingjianhua
   */
  public static List<AdView> listViewBySignagePosition(
      OSIGNAGE signage, OAdLocation position, Integer start, Integer limit) {
    List<AdView> views = new ArrayList<>();
    Query.SELECT(T.PKEY, T.CATEGORY)
        .FROM(CntAd.class)
  ...
  public static class pageSelect extends IduOther<pageSelect, CntAd> {
     ...
     */
    public List<Map> getHasAdCatInfos(
        IduPage page, CntAd.OAdLocation adLocation, CntAd.OSIGNAGE osignage) {
      FormaterSql sql = FormaterSql.build(Debug);
      sql.select(CntAd.T.PKEY, CntAd.T.CATEGORY)
          .page(page)
          .eq(CntAd.T.ENABLED)
          .and()
          .eq(CntAd.T.AD_POSITION)
          .and()
          .eq(CntAd.T.SIGNAGE);
     ...
     * @date 2018/7/25 16:52
     */
    public List<Map> getAdsInfo(
        IduPage page, CntAd.OAdLocation adLocation, CntAd.OSIGNAGE osignage) {
      FormaterSql sql = FormaterSql.build(Debug);
      sql.select(CntAdLine.T.NAME, CntAdLine.T.IMAGE, CntAdLine.T.URL)
     ...
     * @date 2018/7/30 9:35
     */
    public CntAd_CategoryAdView getCatAdInfoList(
        IduPage page, long category, CntAd.OSIGNAGE osignage) {
      CntAd_CategoryAdView cntAd_categoryAdView = new CntAd_CategoryAdView();
      List<Map<String, Object>> mainList =
          getCatAdInfoListByIsMain(page, category, Sys.OYn.YES.getLine().getKey(), osignage);
      if (mainList.size() > 0) cntAd_categoryAdView.setMain(mainList.get(0));
      cntAd_categoryAdView.setItems(
          getCatAdInfoListByIsMain(page, category, Sys.OYn.NO.getLine().getKey(), osignage));
     ...
     */
    private List<Map<String, Object>> getCatAdInfoListByIsMain(
        IduPage page, long category, int isMain, CntAd.OSIGNAGE osignage) {
      FormaterSql sql = FormaterSql.build(Debug);
      List parmList = new ArrayList();
      parmList.add(Sys.OEnabled.TRUE.getLine().getKey());
      sql.select(CntAdLine.T.NAME, CntAdLine.T.IMAGE, CntAdLine.T.URL)
